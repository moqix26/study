...
> **对照**：[AIAgent 10 生态选型](../AIAgent/10-LLM原理与模型适配.md)；视觉算子见 [LLMInfra 05 GEMM](../LLMInfra/05-矩阵运算cuBLAS与GEMM优化入门.md)。
...
**直觉**：拉近配对、推远不配对的，类似 [06 RAG](../AIAgent/05-RAG基础-分块检索与引用.md) 里 Embedding 的「意思近则向量近」，但 CLIP 同时学 **两种模态**。
...
[AIAgent 07 Agent](../AIAgent/07-Agent编排-状态机与长任务.md) 若需「看 UI 截图操作」，视觉编码器多为 CLIP 或 SigLIP 衍生；文本 RAG 无法直接索引像素。
...
CLIP 继承网络数据偏见；生产需过滤与人工审核，对照 [AIAgent 08 安全](../AIAgent/08-评估可观测安全与成本.md)。
...
8. 多模态 RAG 与 [AIAgent 05](../AIAgent/05-RAG基础-分块检索与引用.md) 文本 RAG 差在哪？
...
并行：[09 CNN](09-视觉CNN入门-可选.md)、[AIAgent 10 多模态趋势](../AIAgent/10-LLM原理与模型适配.md)。
