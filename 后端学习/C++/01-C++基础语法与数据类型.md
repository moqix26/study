...
本章 C 风格数组是 [数据结构 02 数组](../数据结构/02-数组与字符串.md) 的裸内存版；04 章 `vector` 会替代手写数组。先学本章类型与循环，再刷数据结构更顺。
...
**Q：和 [数据结构 01](../数据结构/01-复杂度分析与学习方法.md) 怎么配合？**  
可并行：01 章练语法，数据结构 01 练大 O 直觉；04 章后刷 [数据结构 02](../数据结构/02-数组与字符串.md)。
...
- [数据结构 03 链表](../数据结构/03-链表.md) 节点里必有 `next` 指针。
