...
| `hello-qt/`（自练） | [17 Qt](../17-Qt入门与信号槽.md) | Qt6 桌面 | 按 17 章创建 |
| `kv-store/`（自练） | [35 KV-Store](../35-项目实战高性能KV-Store.md) | **C++ 核心简历项目** | Phase 按 35 章 |
| `MiniInfer/`（自练） | [LLMInfra 19](../LLMInfra/19-项目实战简易推理引擎.md) | LLM Infra **扩展**项目 | Phase A～F |

**C++ 核心主线** 优先 **35 KV-Store + mini-http**；CUDA 扩展用 MiniInfer；桌面轨用 hello-qt。
...
| 27 | gtest 接入 hello-cmake | `ctest` 绿灯 |
| 35 | kv-store/ | WAL + LRU + epoll |
| 33～34 | 口述 + 白板 | 不依赖单独 binary |
...
5. **`kv-store/`** — [35 章](../35-项目实战高性能KV-Store.md) 简历项目  

完成 2+5 后可在简历写：**「C++17 高性能 KV + HTTP 服务，gtest 覆盖，ab 压测 QPS xxx」**（数字用 12/35 章实测表填写）。
