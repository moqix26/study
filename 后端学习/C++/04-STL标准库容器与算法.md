...
用 **标准工具箱** 完成词频统计；LeetCode 用 `vector`+`unordered_map` 刷 [数据结构 02～05](../数据结构/README.md)；为 05 移动语义准备容器传参场景。
...
| STL | 数据结构章 |
|-----|------------|
| `vector` | [02 数组](../数据结构/02-线性结构/01-数组.md) |
| `stack`/`queue` | [04 栈与队列](../数据结构/02-线性结构/03-栈与队列.md) |
| `unordered_map` | [05 哈希表](../数据结构/03-哈希表/README.md) |
| `priority_queue` | [07 堆](../数据结构/04-树形结构/05-堆.md) |
...
9. 词频统计首选哪个容器？为什么？
10. 04 章与 [数据结构 05 哈希](../数据结构/03-哈希表/README.md) 分工？
...
9. **`unordered_map<string,int>`**，平均 `O(1)` 完成词频计数。
10. 数据结构资料讲冲突处理和负载因子；STL 章节讲 API、容器选型与迭代器失效。
...
3 分钟讲清“为什么 C++ 程序员优先使用 STL，而不是每次手写数组和容器”。

**提纲**：STL 容器经过充分测试与优化；`vector` 自动扩容；算法通过迭代器与容器解耦；刷题和工程代码都能减少重复实现。
...
能覆盖大量数组、哈希、栈队列题；树和图继续阅读 [树形结构](../数据结构/04-树形结构/README.md)、[图结构](../数据结构/05-图结构/README.md)，C++ 写法配合 [C++ 13](13-算法与数据结构C++实现.md)。
...
与 [数据结构 05 哈希](../数据结构/03-哈希表/README.md) 两数之和/字母异位词同一容器思维。
...
对照 [数据结构 04 栈](../数据结构/02-线性结构/03-栈与队列.md)：表达式求值、括号匹配、DFS 非递归均用 stack。
