...
> **文件编码**：UTF-8。  
> **定位**：Go 后端路线 **第 1 步**——在 Windows 上安装 Go 1.22+、配置 GOPROXY、理解 go mod、跑通 Hello World 与标准项目布局。  
...
1. 在 PowerShell 执行 `go version`，看到 `go1.22.x` 或更高。
...
| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | 下载安装 Go 1.22+ MSI | 安装向导完成 | 见 §8 报错表 #1 |
| 2 | 新开 PowerShell，`go version` | `go version go1.22.x windows/amd64` | 见 #2 PATH |
...
2. 下载 **Windows amd64 MSI**（例如 `go1.22.5.windows-amd64.msi`）
...
1. 新开终端执行 `go version`，显示 go1.22.x windows/amd64。
