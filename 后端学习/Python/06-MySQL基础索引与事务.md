...
**一句话**：05 章会写 ORM，本章学 **MySQL 怎么存、怎么查快、事务怎么保证不错账**——与 [Java 06](../Java/06-MySQL基础索引与事务.md) **概念相同**，Python 侧用 SQLAlchemy 连库。
...
| **表** | Excel 一张 sheet |
| **索引** | 书后目录，按关键字快速翻页 |
| **事务** | 银行转账：扣款和入账必须同时成功或同时失败 |
| **EXPLAIN** | 看 MySQL「打算怎么查」的路线图 |
...
| 水平 | 建议 |
|------|------|
| 学完 05 | 正常学习，Docker 起 study-mysql |
| 学过 Java 06 | **概念可速读**，重点 Python 连接串与 IntegrityError |
| 只会 ORM 不会 SQL | 本章必练客户端敲 SQL + EXPLAIN |

---

### 0.3 本章知识地图（学完后应能勾选全部 ☐→☑）

- [ ] 设计 user/product/order 三表，金额 DECIMAL
- [ ] 写 JOIN、分页 SQL，会用 EXPLAIN
- [ ] 理解 B+ 树、最左前缀、覆盖索引
- [ ] 口述 ACID、四种隔离级别
- [ ] 会写 `UPDATE ... WHERE stock >= ?` 防超卖
- [ ] 知道与 Java 06 考点一致
- [ ] 闭卷自测 ≥ 8/10
...
1. 对慢查询 **EXPLAIN**，说出 type、key、rows 含义。
2. 给订单表加 **联合索引**，对比加前后 rows。
3. 用 SQL **事务**完成扣库存，失败则 rollback。
4. 向面试官讲 **脏读、幻读** 与隔离级别（与 Java 06 同题）。
...
| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | Docker 启动 study-mysql，导入 §9 建表 SQL | 三表存在 | `SHOW TABLES;` |
| 2 | 无索引时 `EXPLAIN ... user_id=1` | type=ALL, key=NULL | 正常，待优化 |
| 3 | 建 `idx_user_status_time` | 索引创建成功 | 检查 DDL 语法 |
| 4 | 再次 EXPLAIN 同一条 SQL | type=ref, key=idx_... | 仍 ALL → 最左前缀 |
| 5 | 对比 rows 列 | 加索引后 rows 明显变小 | 数据太少时差异小 |

**与 Java 06 对照**：EXPLAIN 各列含义、B+ 树、最左前缀、隔离级别**面试答案完全一致**；Python 项目只是在 SQLAlchemy 里 `text("EXPLAIN ...")` 执行，优化思路不变。

### 14.2 电商三表建表 SQL 逐字段读（节选）

| 字段 | 表 | 含义 | 改错会怎样 |
|------|-----|------|------------|
| `price DECIMAL(10,2)` | product | 金额精确两位 | 用 float 丢精度 |
| `UNIQUE uk_username` | user | 用户名唯一 | 重复注册 IntegrityError |
| `KEY idx_user_status_time` | order | 联合索引 | 漏索引 → 订单列表慢 |
| `` `order` `` | order | 保留字需反引号 | 语法错误 |
...
| ACID / 隔离级别 | ✅ | ✅ | **相同** |
| 防超卖 SQL | ✅ | ✅ | **相同** |
| 接入方式 | JDBC / MyBatis | SQLAlchemy / aiomysql | 实现不同 |

**学习建议**：MySQL 概念**读一份深一份**即可；实操两边各做一遍 EXPLAIN 对比。Python 独有：`IntegrityError` 捕获、async 连接池（§19）。

---

## 26. FAQ

**Q1：06 和 Java 06 要都学吗？**  
概念重复；时间紧学一条线的 06，另一条速读对照表。

**Q2：为什么不用 float 存金额？**  
精度丢失；用 DECIMAL(10,2)。

**Q3：type=ALL 什么意思？**  
全表扫描，慢；应争取 ref/range。

**Q4：联合索引 (a,b,c) 能查 b 吗？**  
不能跳过最左 a；违反最左前缀。

**Q5：READ COMMITTED 和 REPEATABLE READ？**  
MySQL 默认 RR；面试常考脏读/不可重复读/幻读。

**Q6：乐观锁和悲观锁？**  
见 §17；初学 `UPDATE WHERE stock>=?` 够用。

**Q7：深分页为什么慢？**  
大 offset 扫描多；改 `WHERE id > last_id`。

**Q8：SQLAlchemy 和 raw SQL 关系？**  
ORM 底层仍是 SQL；优化要看 EXPLAIN 不看 ORM 语法。

**Q9：Docker MySQL 连不上？**  
查端口 3306、密码、容器 Up；Python URL 加 charset=utf8mb4。

**Q10：QueuePool timeout？**  
连接池耗尽；增大 pool 或检查 Session 未关。

**Q11：06 学完为何学 07 Redis？**  
MySQL 磁盘 IO 有上限；热点读用内存缓存。

**Q12：表名 order 为什么要反引号？**  
保留字；`` `order` `` 或改名 orders。
...
2. **概念**：最左前缀原则一句话。
3. **概念**：ACID 四个字母各代表什么？
4. **概念**：脏读、不可重复读、幻读各是什么？
5. **概念**：EXPLAIN 里 type=ref 和 type=ALL 谁更好？
6. **概念**：防超卖 `UPDATE stock` 为什么要带 `WHERE stock >= ?`？
7. **动手**：写查询用户 id=1 已支付订单，按时间倒序 LIMIT 10。
8. **动手**：写出给 `(user_id, status, create_time)` 建联合索引的 DDL。
9. **综合**：Python 06 与 Java 06 面试考点有何异同？
10. **综合**：索引是不是越多越好？为什么？

### 自测参考答案

1. 叶子节点有序链表，便于范围扫描。
2. 联合索引 (a,b,c) 查询必须从 a 开始连续匹配才高效用索引。
3. Atomicity 原子性、Consistency 一致性、Isolation 隔离性、Durability 持久性。
4. 脏读：读到未提交数据；不可重复读：同事务两次读不同；幻读：范围读出现新行。
5. ref 更好；ALL 全表扫描。
6. 保证库存够才扣；行数为 0 则超卖失败。
7. `SELECT ... FROM order WHERE user_id=1 AND status=1 ORDER BY create_time DESC LIMIT 10`
8. `CREATE INDEX idx_user_status_time ON order(user_id, status, create_time);`
9. **概念相同**；Python 侧多 ORM/连接池/IntegrityError 工程细节。
10. 不是；索引占空间、拖慢写入，只给高频 WHERE/ORDER BY 建。

---

## 28. 费曼检验

3 分钟解释：**「MySQL 索引和事务分别是干什么的？」**

**对照提纲**：

1. **索引**像目录，让 WHERE 不用翻全书（全表扫描）。
2. **事务**像转账：扣库存和写订单要一起成功，失败就撤销。
3. **EXPLAIN** 看 MySQL 打算怎么查，优化慢 SQL。
4. **和 Java 06 考的一样**；Python 项目用 SQLAlchemy 执行这些 SQL。
