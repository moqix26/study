...
> **读者假设**：你会复制粘贴、会用电脑；**不一定**写过代码。本章从「写第一个 Python 程序」到「用面向对象组织代码」。

### 0.1 用一句话弄懂本章

**一句话**：Python 是一门**面向对象**语言——你先写**类（蓝图）**，再创建**对象（实例）**，用**封装、继承、多态**组织代码，为后面 **FastAPI（≈ Spring Boot）** 写接口打底。

**生活类比——OOP = 建筑行业**（与 [Java 01 §0](../Java/01-Java基础语法与面向对象.md) 相同框架，便于双路线对照）：

| 概念 | Python 代码 | 生活类比 |
|------|-------------|----------|
| **类 class** | `class User:` | **户型蓝图** |
| **对象** | `u = User()` | **某一户具体的房** |
| **封装** | `_balance` + 方法 | **防盗门**，余额不能乱改 |
| **继承** | `class Dog(Animal):` | 子类复用父类行为 |
| **多态** | 子类重写 `speak()` | 同一句「叫一声」，猫狗不同 |

**为什么重要**：FastAPI 的 Router、Service、Pydantic Schema 全是类与函数；不会 OOP，04 章每一行都陌生。
...
| 你现在的水平 | 建议动作 |
|--------------|----------|
| 完全零基础 | 从 §3 第一个程序开始；00 路线图只看环境安装 |
| 学过 Java 01 | 重点看 §2 对照表、§10 OOP 语法差异（无分号、缩进、无 public/private） |
| 学过 JS | 注意 Python 缩进、无 `{}`、True/False 首字母大写 |
| 已会 OOP 想跳 FastAPI | 速读 §0～§14，闭卷自测 ≥8/10 后跳 [04 FastAPI](./04-FastAPI核心开发.md) |

**最低门槛**：能安装 Python 3.11+、创建 `.venv`；知道「变量 = 装数据的盒子」。

---

### 0.3 本章知识地图（学完后应能勾选全部 ☐→☑）

- [ ] 在虚拟环境里创建项目并运行 `python main.py`
- [ ] 使用 `if/for/while`、定义函数与 f-string
- [ ] 写带 `__init__`、`self` 的类，理解继承与方法重写
- [ ] 用 `try/except` 和 `raise` 处理异常
- [ ] 对照 Java 01 说出 3 处语法差异（缩进、类型、访问控制）
- [ ] 完成「Student + Course」或「银行账户」练习
- [ ] 闭卷自测 10 题正确 ≥ 8 题
...
| 阶段 | 建议时间 | 做什么 |
|------|----------|--------|
| §0 + §3～§9 语法 | 4～6 小时 | 变量、流程、函数 |
| §10～§14 OOP | 4～5 小时 | 类、封装、继承、多态 |
| §15～§18 练习 | 3～4 小时 | 分级练习 + 报错排查 |
| FAQ + 闭卷 + 费曼 | 1 小时 | 检验是否真懂 |

---

### 0.5 学完本章你能做什么（可验证的具体动作）

1. **创建** hello-python 项目，Run 出 `Hello Python`。
2. **编写** 含 `__init__`、`add_score` 的 `Student` 类。
3. **实现** BMI 计算器函数，非法输入抛 `ValueError`。
4. **用继承** 写 `Animal` + `Dog`/`Cat`，多态调用 `speak()`。
5. **对照** Java：说出 Python 为何用缩进代替 `{}`。
6. **排查** `IndentationError`、`NameError` 两类报错。
...
| 行号 | 代码 | 含义 | 改错会怎样 |
|------|------|------|------------|
| 1 | `def main():` | 定义函数 main，注意冒号 | 漏 `:` → SyntaxError |
| 2 | `print("Hello Python")` | 向控制台输出字符串 | 漏引号 → SyntaxError |
| 4 | `if __name__ == "__main__":` | 仅直接运行时进入 | 无影响运行，但被 import 时会误执行 |
| 5 | `main()` | 调用上面定义的函数 | 缩进不对 → IndentationError |

**术语（解释型语言）**：源码由解释器逐行/逐块执行，无需先编译成机器码再运行（CPython 内部会先编译字节码，初学可理解为「改完即跑」）。  
**生活类比**：像口头传菜谱现做现吃；Java 像先印成标准手册再按手册做。  
**为什么重要**：开发迭代快，适合 Web 原型；04 章改接口代码后 uvicorn --reload 立刻生效。  
**本章用到的地方**：§3、§15 执行流程图。
...
| 步骤 | Java | Python（本章） |
|------|------|----------------|
| 文件名 | `Main.java` | `main.py` |
| 入口 | `public static void main(String[] args)` | `if __name__ == "__main__":` |
| 输出 | `System.out.println("Hello Java");` | `print("Hello Python")` |
| 运行 | `javac` + `java Main` | `python main.py` |
| 块结构 | `{ }` | 缩进 |
...
| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | `python --version` | Python 3.11+ | 安装 Python 并加 PATH |
| 2 | `mkdir hello-python && cd hello-python` | 进入新目录 | 检查当前路径 |
| 3 | `python -m venv .venv` | 出现 `.venv` 文件夹 | 确认 python 命令可用 |
| 4 | `.\.venv\Scripts\Activate.ps1` | 提示符 `(.venv)` | RemoteSigned 策略 |
| 5 | 创建 `main.py` 并 Run | 输出 Hello Python | 查 IndentationError 行号 |
| 6 | `python main.py` | 同上 | 确认在 hello-python 目录 |
...
**术语（class 类）**：创建对象的模板，定义属性和方法。  
**生活类比**：楼盘**户型蓝图**——蓝图不是房子，但规定了几室几厅。  
**为什么重要**：04 章 `class UserCreate(BaseModel)` 本质是类；FastAPI Service 也是类。  
**本章用到的地方**：§10～§13。

**术语（object 对象）**：类的具体实例，各自有独立的数据。  
**生活类比**：按同一蓝图盖出来的**101 室、102 室**——户型相同，住户不同。  
**为什么重要**：`user1 = User("张三")` 与 `user2 = User("李四")` 是两个对象。  
**本章用到的地方**：§10.1、§17 练习。

### 10.1.2 封装/继承/多态与 Java 01 对照

| OOP 概念 | Python 写法 | Java 01 写法 |
|----------|-------------|--------------|
| 封装 | `_balance` + `@property` | `private` + getter/setter |
| 继承 | `class Dog(Animal):` | `class Dog extends Animal` |
| 重写 | 同名 `def speak(self):` | `@Override void speak()` |
| 多态 | 父类变量接子类对象 | `Animal a = new Dog()` |

**手把手：Student 类**

| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | 定义 `class Student` 含 `__init__` | 无 SyntaxError | 检查缩进与冒号 |
| 2 | 写 `add_score(course, score)` | 可存入 dict | scores 未初始化 |
| 3 | 写 `average()` | 返回均分 float | 空 dict 除零 → 先判断 |
| 4 | 创建两个 Student 实例 | 互不影响 | 用了类变量而非实例变量 |
...
**Q1：`if __name__ == "__main__"` 必须写吗？**  
小脚本可不写；项目里建议写，避免被 import 时误执行测试代码。

**Q2：Python 2 还要学吗？**  
不需要。Python 2 已停止维护，本资料全部基于 **Python 3.11+**。

**Q3：先学 Django 还是直接 FastAPI？**  
先完成 01～03 语言基础，04 章从 FastAPI 入手；Django 可在 15 篇索引延伸。

**Q4：和 Java 路线可以同时学吗？**  
可以，但建议**一条为主**。MySQL、HTTP 等通用知识可互参。

**Q5：Python 有 public/private 吗？**  
没有关键字。约定：**单下划线 `_field`** 表示内部使用；**双下划线 `__name`** 触发名称改写（了解即可）。对照 Java 的 `private` 是**约定而非强制**。

**Q6：Python 需要声明变量类型吗？**  
运行时动态推断；02 章会学 `typing` 注解。Java 必须写 `int x = 1`。

**Q7：缩进用 Tab 还是空格？**  
**4 个空格**（PEP 8）。混用 Tab/空格是 `IndentationError` 头号原因。

**Q8：`self` 是什么？**  
实例方法的第一个参数，指向**当前对象**，类似 Java 隐式的 `this`（Java 不用写 this 参数）。

**Q9：Python 的 `==` 和 `is` 区别？**  
`==` 比**值**；`is` 比**是否是同一对象**。字符串比较一般用 `==`。

**Q10：异常该捕获还是往上抛？**  
业务层可捕获转友好错误；未知错误记录日志后 re-raise。04 章 FastAPI 有全局异常处理器。

**Q11：学完 01 能写 Web 接口吗？**  
不能。01 只解决**语言**；HTTP 接口从 04 FastAPI 开始（≈ Java 04 Spring Boot）。

**Q12：和 Java 01 比，哪部分可以少花时间？**  
若已会 OOP：**继承、多态概念**可速读；**缩进、dict/list 语法、无分号**必须手敲适应。

---

## 21. 与 Java 01 平行对照速查

| 主题 | Java 01 | Python 01（本章） |
|------|---------|-------------------|
| 入口 | `public static void main` | `if __name__ == "__main__":` |
| 类型 | 强类型 `int x` | 动态类型 `x = 1` |
| 代码块 | `{ }` | **缩进 4 空格** |
| 访问控制 | public/private | `_` 约定 |
| 接口 | `interface` | 鸭子类型（02 章 Protocol） |
| 编译 | javac → .class | 解释执行，改完即跑 |
| OOP 核心 | 封装继承多态 | **相同思想**，语法更短 |

双路线学习建议：**同一周做相同练习**（Student 选课、银行账户），对比代码行数和语法差异，印象更深。

---

## 22. 闭卷自测

1. **概念**：Python 用缩进表示代码块，混用 Tab 和空格会导致什么错误？
2. **概念**：`self` 在实例方法里代表什么？Java 里对应什么？
3. **概念**：封装在 Python 里通常如何实现（关键字 vs 约定）？
4. **概念**：`if __name__ == "__main__":` 的作用是什么？
5. **概念**：继承与方法重写（override）解决什么问题？
6. **概念**：Python 解释执行与 Java 编译执行，对开发迭代有什么影响？
7. **动手**：写函数 `is_even(n)` 返回 bool，n 为偶数返回 True。
8. **动手**：写 `BankAccount` 类，`withdraw` 余额不足时抛自定义异常。
9. **综合**：对照 Java，列出 Python 01 你最不适应的 3 个语法点及原因。
10. **综合**：为什么 01 章要练 OOP 而不是只学 `print("hello")`？

### 自测参考答案

1. `IndentationError`。
2. 当前实例对象；Java 的 `this`（方法内隐式可用）。
3. 用 `_` 前缀表示内部字段 + 通过方法访问；无 `private` 关键字。
4. 仅当**直接运行本文件**时执行块内代码；被 import 时不执行。
5. 复用父类代码；子类可重写行为实现多态。
6. Python 改完即跑、迭代快；Java 需编译但运行时类型更严格。
7. `def is_even(n): return n % 2 == 0`
8. 见 §18 挑战参考答案 `InsufficientBalanceError`。
9. 开放题；常见：缩进、无分号、动态类型。
10. FastAPI/SQLAlchemy 全是类与对象；只会 print 无法读框架代码。

---

## 23. 费曼检验

请用 **3 分钟**向朋友解释：**「Python 的类与对象是什么？和 Java 比有什么不同？」**

**对照提纲**：

1. **类 = 蓝图，对象 = 按蓝图造出的实例**——和 Java 一样。
2. **Python 更简洁**：不用写分号和大括号，用**缩进**表示块。
3. **没有 public/private**，用 `_` 表示「别随便碰」；类型不用每行声明。
4. **后面写 FastAPI** 时，Router、Service 都是类——01 章是在练「造房子的能力」。
