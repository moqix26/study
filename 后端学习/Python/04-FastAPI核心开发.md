...
**一句话**：**FastAPI = Python 生态的 Spring Boot**——帮你对外提供 HTTP/JSON 接口，自带路由、依赖注入、参数校验和 Swagger 文档。

**生活类比**：

| FastAPI 概念 | Spring Boot 对照 | 生活类比 |
|--------------|------------------|----------|
| `@app.get` | `@GetMapping` | 菜单上的「宫保鸡丁」——顾客点什么菜 |
| `APIRouter` | `@RestController` | 分窗口：收银台、后厨 |
| `Depends()` | `@Autowired` | 需要啥工具自动递给你 |
| `BaseModel` | `@Valid` DTO | 点单格式不对直接拒单 |
| `/docs` | Swagger UI | 菜单说明书 |

**为什么重要**：03 章 asyncio 是发动机，**04 章是整车上路**——从这一章起你是「后端开发者」。
...
| 水平 | 建议 |
|------|------|
| 学完 03 | 正常学习，跟做 demo-api |
| 学过 Java 04 Spring Boot | 对照表速读，重点 Pydantic 与 Depends |
| 只会 Flask | 重点分层、类型校验、async 路由 |
...
- [ ] 写 Router + Service + Schema 三层
- [ ] 用 Pydantic 校验请求体，Swagger 调通 CRUD
- [ ] 配置 CORS，理解统一 Result 返回
- [ ] 知道 `async def` 路由与 03 章 asyncio 的关系
...
| 阶段 | 时间 | 内容 |
|------|------|------|
| 项目搭建 | 1 天 | §2～§3，跑通 Hello |
| 分层 CRUD | 2～3 天 | Router/Service/Schema |
| 进阶 | 2 天 | Depends、异常、CORS、JWT 入门 |
| 自测+联调 | 1 天 | Swagger + 可选前端联调 |

---

### 0.5 学完本章你能做什么

1. 启动 **demo-api**，浏览器打开 `/docs` 完成用户 CRUD。
2. 新增 `GET /api/users/count` 接口并返回统一 JSON。
3. 向面试官解释 **FastAPI 与 Spring Boot 的异同**。
4. 配置 CORS 让本地 Vue/React 能调你的 API。
...
**术语（FastAPI）**：基于 Starlette 与 Pydantic 的现代 Python Web 框架，可理解为 **Python 生态的 Spring Boot**。  
**生活类比**：餐厅前台系统——接单（路由）、验单（Pydantic）、派工（Depends 注入 Service）。  
**为什么重要**：04 章起你写的不再是脚本，而是**可被前端/移动端调用的服务**。  
**本章用到的地方**：§2 分层、§4 CRUD、§10 JWT。
...
| 行号 | 含义 | 改错会怎样 |
|------|------|------------|
| 1 | 导入 FastAPI 类 | ImportError：未 pip install |
| 3 | 创建应用实例，Swagger 标题 | 漏实例则无法挂路由 |
| 5 | GET 路由装饰器，等价 @GetMapping | 路径错 → 404 |
| 6 | **async def** 异步路由，配合 03 章 asyncio | 写 blocking IO 会卡 loop |
| 7 | 返回 dict 自动转 JSON | 返回不可序列化对象 → 500 |

### 1.2 启动 demo-api 手把手

| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | `cd demo-api` | 目录含 `app/main.py` | 路径错误 |
| 2 | `pip install "fastapi[standard]" uvicorn` | Successfully installed | 激活 venv |
| 3 | `uvicorn app.main:app --reload --port 8080` | Uvicorn running on 8080 | ModuleNotFoundError → 看根目录 |
| 4 | 浏览器开 `http://127.0.0.1:8080/docs` | Swagger UI | 防火墙/端口占用 |
| 5 | Try it out `/health` | `{"status":"ok"}` | 查路由注册 |
...
## 17. FastAPI ≈ Spring Boot 对照深化

| 能力 | Spring Boot（[Java 04](../Java/04-SpringBoot核心开发.md)） | FastAPI（本章） |
|------|-----------------------------------------------------------|-----------------|
| 启动 | `SpringApplication.run` | `uvicorn app.main:app` |
| 路由 | `@GetMapping("/users")` | `@router.get("/users")` |
| 路径参数 | `@PathVariable Long id` | `id: int` |
| 请求体 | `@RequestBody UserDTO` | `user: UserCreate`（Pydantic） |
| 校验 | `@NotBlank` 等 | Field 约束、`EmailStr` |
| DI | `@Autowired UserService` | `Depends(get_user_service)` |
| 异常 | `@ControllerAdvice` | `@app.exception_handler` |
| 文档 | springdoc | 内置 `/docs` |
| 异步 | WebFlux（另学） | 原生 `async def` + asyncio |

**术语（Pydantic BaseModel）**：基于类型的数据模型，自动校验 JSON 字段。  
**生活类比**：快递验货单——件数、重量不对直接拒收。  
**本章用到的地方**：§4 Schema、§6 创建用户接口。
...
本路线 **FastAPI**（≈ Spring Boot 定位）：类型校验 + async + 自动文档。

**Q2：必须用 async def 吗？**  
IO 密集推荐 async；纯 CPU 短逻辑可用 sync，FastAPI 会放线程池。

**Q3：422 是什么？**  
Pydantic 校验失败；看响应 body 的 `detail` 字段。

**Q4：Depends 和 Spring @Autowired 一样吗？**  
思想类似：声明依赖，框架注入；FastAPI 的 Depends 更显式、可测。

**Q5：为什么分层 Router/Service？**  
路由只接 HTTP；业务在 Service——与 Spring Controller/Service 相同。

**Q6：内存 dict 存用户够吗？**  
04 章练分层；**05 章换 MySQL**。

**Q7：CORS 报错怎么办？**  
`CORSMiddleware` 的 `allow_origins` 要含前端地址如 `http://localhost:5173`。

**Q8：Swagger 和生产环境？**  
生产可关 `/docs` 或加鉴权。

**Q9：JWT 必须学吗？**  
登录场景必须；04 章入门，10 章项目深化。

**Q10：和 Java 04 能对照学吗？**  
**强烈建议**：同一 CRUD 各写一版，REST 设计一致。

**Q11：uvicorn --reload 干什么的？**  
改代码自动重启，开发用；生产不用 reload。

**Q12：ModuleNotFoundError: app？**  
在项目根目录启动：`uvicorn app.main:app --reload`。

---

## 19. 闭卷自测

1. **概念**：为什么说 FastAPI ≈ Python 的 Spring Boot？
2. **概念**：Router、Service、Schema 各负责什么？
3. **概念**：422 和 400 在本项目中通常各表示什么？
4. **概念**：`Depends(get_db)` 在 05 章起什么作用？（预习）
5. **概念**：CORS 解决什么问题？
6. **概念**：`async def` 路由和 03 章 asyncio 的关系？
7. **动手**：写出启动 demo-api 的 uvicorn 命令（带 reload、端口 8080）。
8. **动手**：Pydantic 模型如何声明 `username` 必填且长度 3～32？
9. **综合**：对照 Spring Boot，列出 FastAPI 侧 3 个等价注解/写法。
10. **综合**：为何业务逻辑不应写在 `@router.get` 函数里直接堆几十行？

### 自测参考答案

1. 都是主流 Web 框架：路由、DI、校验、Swagger、JSON API。
2. Router 接 HTTP；Service 业务；Schema 入参/出参与校验。
3. 422 参数校验失败；400 业务错误（如用户名重复）。
4. 每个请求注入数据库 Session，用完关闭。
5. 浏览器跨域安全限制；后端声明允许的来源。
6. async 路由在 asyncio 事件循环中执行，await IO 不阻塞其他请求。
7. `uvicorn app.main:app --reload --port 8080`
8. `username: str = Field(..., min_length=3, max_length=32)`
9. 如 `@GetMapping`→`@router.get`，`@Valid`→Pydantic，`@Autowired`→`Depends`。
10. 难测试、难复用；违反分层，与 Spring 不写 Controller 大逻辑同理。

---

## 20. 费曼检验

3 分钟解释：**「FastAPI 是什么？和 Spring Boot 像在哪里？」**

**对照提纲**：

1. **对外提供 HTTP 接口**，前端用 JSON 调用——后端「餐厅前台」。
2. **路由**对应 `@GetMapping`；**Pydantic** 对应请求体验证；**Depends** 对应注入。
3. **自带 Swagger** `/docs`，不用额外配插件。
4. **区别**：Python 用 async/await 做高并发 IO；语法更短，生态是 pip 不是 Maven。
