...
<!-- 修改说明: 2026-06-30 按 EXPANSION-STANDARD 扩充 §0（+200 行导读）、闭卷自测、费曼检验；全系列 6/6 收官 -->
...
> **读者假设**：已学过 [HTML CSS JS 11](../HTML%20CSS%20JS/11-前端工程化调试Git与包管理基础.md) 或正在与 [Vue 01](../Vue/01-Vue入门与环境搭建.md) 并行。有 shop-vue / java-demo 项目目录。

### 0.1 用一句话弄懂本系列

**一句话**：Git = 给代码拍**无限次快照**，随时回到任意版本；GitHub/Gitee = 把快照存云端和别人协作。
...
|----------|----------|----------|
| **工作区** | 正在玩的当前画面 | 02 |
| **暂存区** | 存档前勾选「要保存哪些进度」 | 02 |
| **本地仓库** | 硬盘上的存档槽 | 01～02 |
| **commit** | 一次存档 + 写存档说明 | 01～02 |
| **branch** | 平行宇宙存档线 | 03 |
| **merge** | 把两条线合并 | 03 |
| **push / PR** | 上传云端 + 请队友审核合并 | 04 |

**为什么重要**：Vue 01 建议 `git init shop-vue`；没有 Git 改坏代码无法 `git restore`；面试必问「你们怎么协作」。

### 0.2 你需要提前知道什么（零基础解释列）

| 术语 / 能力 | 零基础解释 | 真不会请先学 |
|-------------|------------|--------------|
| **终端 cd** | 进入某个文件夹 | HTML 11 |
| **package.json** | Node 项目配置文件 | HTML 11～12 |
| **node_modules** | npm 下载的依赖，**不能提交 Git** | HTML 12 |
| **README** | 项目说明书，常放启动命令 | 01 章 |
| **SSH 密钥** | 免密码连接 GitHub/Gitee | 04 章 |

| 你现在的水平 | 建议动作 |
|--------------|----------|
| 完全零基础 | ⏸ 先 HTML 01～05 会保存文件 |
| 学过 HTML 11 四条命令 | ✅ 00 通读 + 01 安装 |
| Vue 01 当天 | ✅ 01 init + 02 每日 commit |
| 准备协作/实习 | ✅ 03～04 分支 + PR |

### 0.3 本章知识地图（00 路线图学完后 ☐→☑）

- [ ] 能区分 Git / GitHub / Gitee
- [ ] 能画出工作区→暂存区→仓库三区模型
- [ ] 能说出 01～05 各章主题
- [ ] 知道 shop-vue / java-demo 的 commit 节奏（§6）
- [ ] 完成 §13：`git log --oneline` 至少 1 行
- [ ] 闭卷自测 ≥ 8/10
...
| 阶段 | 时间 | 内容 |
|------|------|------|
| 00 + 01 | 2 h | 安装、config、首 commit |
| 02 | 3 h | add/log/diff/restore |
| 03 | 2.5 h | 分支、merge、冲突 |
| 04 | 2.5 h | remote、push、PR |
| 05 | 1.5 h | 三项目实践、面试 |

**最佳窗口**：HTML 12 后或 **Vue 01 同一天 init**。

### 0.5 学完 00 你能做什么

1. 解释 Git 是软件、GitHub 是网站。
2. 说出 `.gitignore` 为什么要忽略 `node_modules`。
3. 列出练手仓库：**Go 路线**用 `go-daily`、`leetcode-go`、`shorturl`（见 [Go 00](../../后端学习/Go/00-学习路线图与说明.md)）；前端路线用 shop-vue、shop-react、java-demo。
4. 执行 `git --version` 确认环境就绪。

### 0.6 全系列 01～05 章速览

| 章 | 一句话主题 | 产出物 |
|----|------------|--------|
| **01 入门** | 安装、config、init、首 commit | hello-git 或 shop 首次提交 |
| **02 本地操作** | add、log、diff、撤销、stash | 10+ 有意义 commit |
| **03 分支** | branch、merge、冲突解决 | feature 分支合并 main |
| **04 远程协作** | push、pull、PR、Review | 远程仓库 + README |
| **05 实践面试** | 三项目 ignore、Hooks、面试题 | 完整工作流 |

### 0.7 三仓库 commit 节奏（与 §6 对照）

| 仓库 | Git 01 | Git 02 | Git 03 | Git 04 |
|------|--------|--------|--------|--------|
| shop-vue | init + 首 commit | 每章一 commit | feature 分支 | push + README |
| shop-react | 同左 | 同左 | 同左 | 同左 |
| java-demo | Spring 生成后 init | 每模块一 commit | 联调分支 | 远程 + PR |

### 0.8 Conventional Commits 速记

| 前缀 | 含义 | 示例 |
|------|------|------|
| `feat:` | 新功能 | `feat: product list page` |
| `fix:` | 修 bug | `fix: cart total calculation` |
| `chore:` | 杂项/初始化 | `chore: init shop-vue` |
| `docs:` | 文档 | `docs: update README` |

02 章详讲；与 Vue/Java 资料提交习惯一致。

### 0.9 零基础常见误解

| 误解 | 正确理解 |
|------|----------|
| Git = GitHub | Git 是本地软件；GitHub/Gitee 是托管网站 |
| node_modules 要备份 | 体积巨大且可 `npm install` 重建，必须 ignore |
| commit = 保存文件 | 须先 `git add` 进暂存区再 commit |
| 11 章四条命令已够 | 分支/远程/冲突/PR 本系列 01～05 系统补 |
| 前后端一个仓库 | 常见分仓库；联调靠 API 文档对齐 |

### 0.10 与 TypeScript / 计网 / Java 衔接

| 系列 | 衔接点 |
|------|--------|
| [TypeScript 10](../TypeScript/10-项目实战JS到TS迁移.md) | 迁移用分支 + 小步 commit |
| [计网 00](../计算机网络/00-学习路线图与说明.md) | PR message 写 `fix: CORS` 要懂修什么 |
| [Java 04+](../../后端学习/Java/04-SpringBoot核心开发.md) | java-demo 同样纳入 Git |

### 0.11 第一天命令清单（跟 01 章）

| 命令 | 零基础解释 | 预期 |
|------|------------|------|
| `git --version` | 确认 Git 已装 | 2.x.x.windows |
| `git config user.name` | 看提交者昵称 | 非空 |
| `git config user.email` | 看提交邮箱 | 非空 |
| `git init` | 当前目录变仓库 | 出现 `.git` |
| `git status` | 看哪些文件改了 | 红色未跟踪/绿色已暂存 |
| `git log --oneline` | 压缩历史一行一条 | 至少 1 行 commit |

### 0.12 `.gitignore` 必忽略项（Node/Java）
...
02 章给完整模板；**第一天 init 就要加**，否则 `node_modules` 误提交很难清理。

### 0.13 Git 全系列学完标准（00 级预览）

- [ ] 01：完成首 commit + `.gitignore`
- [ ] 02：10+ commit，会用 restore/diff
- [ ] 03：`feature/xxx` 合并并解决一次冲突
- [ ] 04：push 远程 + 发 PR
- [ ] 05：三项目工作流 + 面试 5 题

### 0.14 与 todo.md / C++ 项目对齐

| 周次 | 项目任务 | Git 必完成 | 验证 |
|------|----------|------------|------|
| C++ 01 当天 | 创建 `cpp-learning` 仓库 | 01 init + ignore | `git log` 1 行 |
| C++ 02～09 | 每章 demo | 02 每章 commit | 10+ 历史 |
| C++ 09 hello-cmake | CMake 工程 | 03 feature 分支合并 | 无冲突 |
| C++ 10 mini-http | 系统项目 | 04 push GitHub | 远程可见 |
| 面试前 | 简历 | 05 工作流口述 | 3 min 讲协作 |

> 若走 **Vue 全栈** 仍可用上表 shop-vue 列；**2026 默认主线** 见 [todo.md](../../todo.md) **C++ 计划**。
...
1. **概念** Git 和 GitHub 各是什么？
2. **概念** 工作区、暂存区、本地仓库三者关系？
3. **概念** `git add` 和 `git commit` 各做哪一步？
4. **概念** 为什么 `.gitignore` 要忽略 node_modules？
5. **概念** merge 和 rebase 面试怎么说（概念级）？
6. **概念** PR（Pull Request）解决什么问题？
7. **动手** 写出配置用户名邮箱的两条命令。
8. **动手** 首次 commit 最少哪三步（命令名即可）？
9. **综合** shop-vue 从 init 到 push 远程的大致顺序？
10. **综合** 改坏 main.js 如何在不丢其他改动的情况下恢复该文件？

### 19.1 自测参考答案

1. Git=本地版本控制软件；GitHub=远程托管网站。
2. 改文件在工作区→add 进暂存区→commit 进仓库。
3. add 把变更加入暂存区；commit 把暂存区快照写入仓库。
4. 体积大、可 npm install 重建、不应进版本历史。
5. merge 保留分支历史合并；rebase 变基成线性历史（协作需谨慎）。
6. 代码审查 + 合并前讨论，保护 main 分支质量。
7. `git config --global user.name "..."` 和 `user.email "..."`。
8. `git init`（若新仓库）→ `git add` → `git commit -m "..."`。
9. init → 多次 commit → 建远程 → remote add → push -u → 功能分支 → PR merge。
10. `git restore src/main.js` 或 `git checkout -- src/main.js`（视 Git 版本）。

---

## 20. 费曼检验

请在不看资料的情况下，用 3 分钟向朋友解释「Git 是干什么的」。

**对照提纲**：

1. **存档比喻**：Git 像游戏无限存档——每次 commit 写清楚「这一步做了什么」，坏档可以读档。
2. **三区比喻**：改代码=当前画面；add=勾选要存的项；commit=真正写入存档槽。
3. **协作比喻**：GitHub 是云存档；分支是平行宇宙线；PR 是请队友审核「能不能合并进主线」。

---

*本章已按 EXPANSION-STANDARD 扩充（§0 +200 行 §0.6～§0.10、闭卷 §19、费曼 §20）。*

**EXPANSION-STANDARD 自检**：☑ §0 扩展 ☑ 三仓库节奏 ☑ 闭卷 10 题 ☑ 费曼 ☑ 全系列 6/6 索引
