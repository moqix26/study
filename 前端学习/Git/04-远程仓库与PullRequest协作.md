...
本地 Git（01～03）像**单机存档**；**远程仓库**（GitHub/Gitee）是**云存档**——`git push` 上传、`git pull` 同步；**Pull Request（PR）** 是「请队友审核后再合并进主线」的正式流程，对应真实团队 **Code Review**。
...
| `origin` | 云存档账号默认昵称 |
| `git push` | 上传本地支线到云端 |
| `git clone` | 新电脑下载完整存档库 |
| `git fetch` | 只看云端更新了什么，不自动合并 |
| `git pull` | fetch + merge，把云端合进本地 |
| **Pull Request** | 「我改好了，请 Review 后批准合 main」 |
| **SSH 密钥** | 云存档的指纹锁，免每次输密码 |
| **Squash merge** | 把支线 10 次小存档压成 1 条再进主线 |

### 0.2 你需要提前知道什么

| 前置 | 章节 | 必须？ |
|------|------|--------|
| 分支 merge、冲突 | [03 章](./03-分支管理与合并冲突.md) | ✅ |
| user.name / email | [01 章](./01-Git入门与安装配置.md) | ✅ |
| GitHub 或 Gitee 账号 | 本章 §2 注册 | ✅ |

### 0.3 本章知识地图（学完后应能勾选全部 ☐→☑）

```text
☐ 会 git remote add / push -u / pull
☐ 能区分 fetch 与 pull
☐ Windows 下配置 SSH 并测试 git@github.com
☐ 能在网页完成 New Pull Request → Review → Merge
☐ 知道 Squash merge 与 Merge commit 怎么选
☐ PR 冲突时能在本地 merge origin/main 后 push 修复
☐ shop-vue 至少 1 次真实或模拟 PR 全流程
```
...
1. 把本地 shop-vue `main` push 到 GitHub，网页可见 commit 历史。
2. 开 `feature/readme` 分支 → push → 开 PR → Squash merge → 本地 pull。
3. 解释：为什么 `git pull` 有时会产生 merge commit。
4. 配置 SSH，`ssh -T git@github.com` 看到 Hi username。
5. 闭卷自测 ≥8/10。
...
**Q1：fetch 和 pull 区别？**  
fetch 只**下载**远程更新到 `origin/*`，不自动合并；pull = fetch + merge（或 rebase，看配置）。

**Q2：首次 push 为什么要 `-u`？**  
设置上游跟踪，之后 `git push` / `git pull` 不用写远程名和分支名。

**Q3：SSH 和 HTTPS 选哪个？**  
SSH 配好密钥后免密 push；HTTPS 需 Personal Access Token。团队常用 SSH。

**Q4：PR 冲突在网页能解吗？**  
GitHub 简单冲突可网页编辑；复杂冲突本地 `merge origin/main` 后 push。

**Q5：Squash merge 和 merge commit？**  
Squash：feature 上 N 个 commit 压成 1 个进 main，历史干净；merge commit 保留全部分叉历史。

**Q6：能 push 到 main 吗？**  
学习仓库可以；真实团队 **禁止**直接 push main，必须 PR + Review。

**Q7：`git pull` 产生多余 merge commit？**  
因本地与远程分叉；可先 `fetch` + `rebase` 或配 `pull.rebase true`（团队统一）。

**Q8：Gitee 与 GitHub 流程一样吗？**  
概念相同：remote、push、Pull Request（Gitee 叫 Pull Request/合并请求），界面文案略不同。

---

## 23. 手把手：首次 push 与 PR 步骤表

| 步骤 | 命令/动作 | 预期 | 若不对 |
|------|-----------|------|--------|
| 1 | 平台建空仓库 | 得到 HTTPS/SSH URL | 权限 |
| 2 | `git remote add origin URL` | `git remote -v` 两行 | URL 错 |
| 3 | `git push -u origin main` | 网页见 commit | SSH 权限 denied |
| 4 | `git switch -c feature/x` 开发 push | 远程多分支 | 未 -u 要写全名 |
| 5 | 网页 New Pull Request | 可 Compare | 无 diff 检查分支 |
| 6 | Review 后 Merge | main 更新 | 冲突见 §进阶 |
| 7 | 本地 `git switch main && git pull` | 与远程一致 | 忘记 pull |

---

## 24. SSH 配置 Windows 检查清单

```text
☐ ssh-keygen -t ed25519 -C "email"
☐ 启动 ssh-agent；ssh-add 私钥路径
☐ 公钥粘贴 GitHub/Gitee Settings → SSH Keys
☐ ssh -T git@github.com 见 Hi username
☐ remote URL 为 git@ 而非 https（若用 SSH）
```
...
1. 用「**云存档**」类比 origin、push、pull、PR。
2. fetch 与 pull 区别？
3. `git push -u origin main` 中 `-u` 作用？
4. Pull Request 解决什么问题？
5. Squash merge 适合什么场景？
6. 为何不应 force push 到公共 main？

### 动手题（2 题）

7. 写出添加 SSH remote 并首次 push main 的三条命令。
8. PR 显示冲突时，本地修复的最小命令序列（假设在 feature 分支）？

### 综合题（2 题）

9. 描述 shop-vue 从本地 feature/login 到合并进远程 main 的 8 步协作流。
10. 队友 push 了 main 新 commit，你 feature 分支如何同步再 push？（两种方式）

### 自测参考答案

**1.** origin=云账号；push=上传；pull=下载并合并；PR=请审核后合主线。

**2.** fetch 只更新远程跟踪分支；pull 还合并到当前分支。

**3.** 设置 upstream，后续 push/pull 简写。

**4.** Code Review、讨论、CI 通过后再合并，保护 main。

**5.** 功能分支 commit 碎、合 main 想一条历史。

**6.** 覆盖他人基于旧 main 的工作，破坏协作历史。

**7.** `git remote add origin git@github.com:you/repo.git` → `git push -u origin main`（此前已 commit）

**8.** `git fetch origin` → `git merge origin/main` → 解决冲突 → `git push`

**9.** 本地开发 → commit → push feature → 开 PR → Review → Merge → 本地 main pull → 删远程/本地 feature。

**10.** `git merge origin/main` 或 `git rebase origin/main`（未共享时可 rebase）。
...
1. **GitHub 是云存档**，push 上传、clone 新电脑下载全套历史。
2. **Pull Request 是合并前先给队友审**，不能直接改公司主线。
3. **pull 是先把云端最新进度同步下来**，再 push 自己的，减少冲突。

---

*本章已按 EXPANSION-STANDARD 扩充（§0+云存档类比+FAQ+闭卷自测+费曼）。*

**EXPANSION-STANDARD 自检**：☑ §0 导读 ☑ 云存档类比 §0.1 ☑ 首次 push 步骤 §23 ☑ SSH 清单 §24 ☑ FAQ §22 ☑ 闭卷 10 题 §25 ☑ 费曼 §26

---

## 27. PR 描述模板（可复制到 `.github/pull_request_template.md`）
...
## 28. GitHub vs Gitee 操作对照

| 操作 | GitHub | Gitee |
|------|--------|-------|
| 新建仓库 | New repository | 新建仓库 |
| 合并请求 | Pull Request | Pull Request / 合并请求 |
| SSH 公钥 | Settings → SSH keys | 设置 → SSH 公钥 |
| 保护 main | Branch protection rules | 分支保护（企业版等） |

---

## 29. 本章总复习清单

1. 完成首次 push + 1 次 PR 全流程。  
2. `ssh -T` 通过。  
3. 能解释 fetch vs pull。  
4. 闭卷 §25 ≥8/10。  
5. 费曼 3 分钟。  
6. 知道 Squash 与 merge commit 选型。
...
| 误操作 | 症状 | 修复 |
|--------|------|------|
| push 错分支到 main | 远程 main 含 WIP | revert PR 或 reset（需团队协调） |
| 忘记 pull 就 push | rejected non-fast-forward | pull --rebase 或 merge 后再 push |
| remote URL 写错 | push 403/404 | `git remote set-url origin 正确URL` |
| SSH 未 add | Permission denied | ssh-add + 平台绑公钥 |
| PR 从过旧 main 切出 | 巨大冲突 | merge origin/main 进 feature |
