...
把 **能跑的 JS shop** 按 **types → api → stores → views** 顺序改成全 TypeScript，用 `allowJs` 渐进、最后 `strict: true`，让简历上的「熟悉 TS」有仓库可展示。

### 0.2 你需要提前知道什么

| 状态 | 动作 |
|------|------|
| shop 还不能跑 | 先完成 [Vue 08](../Vue/08-Axios网络请求与前后端联调.md) / [React 08](../React/08-Axios网络请求与前后端联调.md) |
| 不会 tsconfig | 先读 [09 章](./09-工程化与tsconfig深入.md) §1～5 |
| 不会框架 TS | 读 [07](./07-Vue3与TypeScript.md) 或 [08](./08-React与TypeScript.md) |
| 时间紧 | 跟 §14 一天 MVP 路径 |

### 0.3 本章知识地图（☐→☑）

- ☐ 能盘点 shop 目录并建 `feature/ts-migrate` 分支
- ☐ 建好 `src/types` 并与后端 Result 对齐
- ☐ 完成 `http<T>` 泛型封装
- ☐ Pinia/Zustand store 无显式 any
- ☐ 组件 `lang="ts"` / `.tsx` 完成
- ☐ 理解 allowJs 三阶段与修错优先级
- ☐ 能列举 §13 至少 5 个迁移陷阱
- ☐ 验收 §17 项目级标准
...
| 阶段 | 时长 | 内容 |
|------|------|------|
| §1～3 盘点 + tsconfig | 45 min | 建分支、记 error 基线 |
| §4～6 types + api + stores | 2 h | 核心 |
| §7～9 router + views + main | 1.5 h | |
| §14 MVP 或 §10 全清单 | 按时间选 | |
| 闭卷自测 | 30 min | §20 |

### 0.5 学完本章你能做什么

1. `src` 下业务文件无 `.js`（配置脚本除外）
2. `npm run typecheck` 0 errors + `strict: true`
3. 口述迁移顺序并解释「为何先 types 后 views」
4. 在面试中用 STAR 讲一次 shop JS→TS 迁移
...
**http 泛型封装**：业务 API 调用 `http<Product>(...)` 直接得到 `Product`，拦截器内统一解包 `ApiResult<T>`。
**生活类比**：像前台统一拆快递盒——你只管拿里面的 `Product`，不用每次 `.data.data`。
**为什么重要**：避免 Axios 响应「双重 data」和 any 泛滥。
**本章用到的地方**：§5.1～§5.3
...
| 行号/代码 | 含义 | 改错会怎样 |
|-----------|------|------------|
| `response.data as ApiResult<unknown>` | 拦截器层认后端统一 Result | 若后端字段不叫 code，需改 interface |
| `res.code !== 0` → reject | 业务错误走 catch | 漏判断会当成功数据 |
| `http<T>(config)` | 业务层 Promise\<T\> | 忘记泛型则调用方丢类型 |
| `body.data as T` | 解包 data 字段 | 与 Java DTO 不一致会运行时 undefined |
| 401 → logout + router.push | 鉴权失败跳转 | router 类型见 §7 |
...
| 13 | 迁移后 bundle 体积异常 | 误把 types 当值 import | import type | 见 TS 06 |
| 14 | Order 状态字符串对不上 | union 与后端 enum 不一致 | 对齐字面量 | `'pending' \| 'paid'` |
| 15 | composable 与 store 循环依赖 | 互相 import 值 | 拆 types 层 | types 不 import store |

---

## 16.1 深入解释：为何「先 types 后 views」

```mermaid
flowchart TB
    Types["src/types Product ApiResult"]
    API["api/product.ts 依赖 types"]
    Store["stores/cart.ts 依赖 types + api"]
    View["ProductList.vue 依赖 store + api + types"]

    Types --> API --> Store --> View
```

若先改 View 再补 types，会出现 **反复改 props 签名**、**store 字段名不一致** 的返工。types 是 **前后端 JSON 契约** 的前端落点，与 [Java DTO](../../后端学习/Java/04-SpringBoot核心开发.md) 对齐后，api/store/view 只做「消费者」。

---

## 16.2 迁移进度跟踪表（可打印）

| 周 | 目标 | typecheck 错误数 | strict | 备注 |
|----|------|------------------|--------|------|
| W1 | types + api | 记录基线 ___ | false | allowJs true |
| W2 | stores + router | ___ | false | noImplicitAny |
| W3 | components/views | ___ | 可选 true | |
| W4 | 删 .js + strict | 0 | true | CI check |

每周五跑 `npm run typecheck`，错误数应单调下降。

---

## 16.3 与后端 OpenAPI 对齐（进阶）

若 Java 项目暴露 Swagger/OpenAPI，可用 `openapi-typescript` 生成 `src/types/api-generated.ts`，再与手写 `types/` merge。**原则**：生成类型为「后端真相」，手写 domain 类型（Product）与之对齐字段名。

```bash
npx openapi-typescript http://localhost:8080/v3/api-docs -o src/types/openapi.d.ts
```

初学 shop 仍以手写 `Product` + 联调为准；入职后可接 codegen 流程。
...
| 提交 | 内容 | 好处 |
|------|------|------|
| commit 1 | types + env.d.ts | 可独立 review 契约 |
| commit 2 | api/*.ts | 泛型 http 一层 |
| commit 3 | stores/*.ts | 状态层类型 |
| commit 4 | views/components lang=ts | UI 层 |
| commit 5 | strict true + 删 .js | 最终态 |

每 commit 后 `npm run dev` + `npm run typecheck`，失败则 `git revert` 单 commit，避免大爆炸 diff。
...
1. 迁移顺序为何是 types → api → stores → views？
2. allowJs 三阶段各是什么？
3. `request.js` 与 `request.ts` 并存会怎样？
4. Pinia 解构 store 为何不更新 UI？
5. `http<T>` 与直接 `axios.get` 的类型差异？
6. 写 `ApiResult<T>` 三字段。
7. 写 `defineProps<{ product: Product }>()` 替代数组 props。
8. Zustand 拦截器为何用 `getState()`？
9. 修错优先级 P0～P4 各举一例。
10. §17 验收表最少通过几项算「迁移完成」？

### 自测参考答案

1. 先契约后消费者；views 依赖 store/api/types，倒序会反复改。
2. allowJs true + checkJs false → 可选 checkJs → allowJs false。
3. 重复模块、import 歧义；删 JS 并全局搜路径。
4. 未用 storeToRefs，解构了普通值非 Ref。
5. http 返回 T；axios 返回 AxiosResponse，需自己 as。
6. code, message, data（见 §4.2）。
7. 见 §8.1。
8. Hook 规则禁止模块顶层 useStore（React）。
9. P0 找不到模块；P1 implicit any；P2 not assignable；P3 undefined；P4 emit。
10. 建议 7 项全过；MVP 可暂留非核心 warning 但 strict 与 typecheck 必过。

---

## 21. 费曼检验

3 分钟解释：**如何把 JS 的 shop 迁到 TS？**

**提纲**：① allowJs 渐进；② 先 types 对齐后端；③ api 泛型 http；④ store/组件加类型；⑤ 关 allowJs、开 strict、CI typecheck。
