...
> **读者假设**：已完成 TS 01～05，会写 [JS 09 模块化](../HTML%20CSS%20JS/11-前端工程化调试Git与包管理基础.md) 的 import/export。本章让 **多文件 TS 项目** 和 **无类型的 npm 包** 都能通过类型检查。

### 0.1 用一句话弄懂本章

**一句话**：用 ES Module 组织 `src/types/`；用 **`.d.ts` 声明文件** 给纯 JS 库补「类型菜单」；配置 `@/` 别名与 `vite-env.d.ts`，打通到 [TS 07 Vue+TS](./07-Vue3与TypeScript.md) / [TS 08 React+TS](./08-React与TypeScript.md)。

**生活类比**：

| 概念 | 类比 |
|------|------|
| `import type` | 只借合同复印件——不留实物（编译后擦掉） |
| `.d.ts` | 外文餐厅 **菜单翻译**——厨房仍是 JS |
| `@types/lodash` | 官方译版菜单 |
| `declare module` | 自己写翻译贴墙上 |
| `paths @/*` | 仓库统一货位编号 |

**为什么重要**：shop 项目几十文件共用 `@/types`；axios/lodash 等靠声明文件才有补全。
...
| 缺什么 | 跳到哪 |
|--------|--------|
| import/export | [JS 09](../HTML%20CSS%20JS/11-前端工程化调试Git与包管理基础.md) |
| interface 合同 | [TS 03](./03-接口类型别名与联合交叉.md) |
| tsconfig 基础 | [TS 01 §5](./01-TypeScript入门与环境配置.md) |

---

### 0.3 本章知识地图（☐→☑）

- [ ] `import type` / `export type` / barrel
- [ ] `.d.ts` vs `.ts`
- [ ] `declare module` 与 `@types/xxx`
...
**约 5～6 小时**（含 shop-ts 目录搭建）。

---

### 0.5 可验证成果

1. `import type { Product } from '@/types'` Ctrl+点击跳转成功。
2. 为假想包写 `declare module` 后 IDE 有补全。
3. `import.meta.env.VITE_API_BASE` 有类型提示。

---

## 本章衔接

你在 [05-类、枚举与类型收窄](./05-类枚举与类型收窄.md) 里学会了 `class`、`enum` 和类型收窄——这些知识大多写在**单个 `.ts` 文件**里。真实项目却是**几十个文件互相 import**：
...
- 已完成 [HTML CSS JS 09-模块化](../HTML%20CSS%20JS/11-前端工程化调试Git与包管理基础.md)（`import`/`export` 语法）
...
### Q6：`.d.ts` 和 interface 合同区别？

`.d.ts` **只有类型声明无实现**；给 JS 库或环境补类型。业务合同仍在 `types/product.ts`。

### Q7：06 与 [JS 09 模块化](../HTML%20CSS%20JS/11-前端工程化调试Git与包管理基础.md) 关系？

JS 09 教 **运行时** import/export；06 教 **类型如何跨模块** + `import type`。

### Q8：Vue 单文件组件类型谁提供？

`vite-env.d.ts` 里 `declare module '*.vue'`；见 [TS 07](./07-Vue3与TypeScript.md)。

### Q9：`skipLibCheck: true` 什么意思？

跳过 node_modules 里 .d.ts 的检查，加快编译；团队项目常开。

### Q10：barrel `index.ts` 导太多会慢吗？

可能略影响类型解析；shop 规模可接受，注意避免循环 export。

### Q11：06 完成后的框架章？

Vue 主线 [TS 07](./07-Vue3与TypeScript.md) + [Vue 07 Pinia](../Vue/07-Pinia状态管理.md)；React 主线 [TS 08](./08-React与TypeScript.md)。

### Q12：找不到模块 `@/types` 先查什么？

`tsconfig paths`、`vite.config alias` 是否一致；06 §12 报错表。

---

## 14.1 DevTools 与 tsc 验证模块

| 步骤 | 动作 | 预期 | 若不对 |
|------|------|------|--------|
| 1 | `npx tsc --noEmit` | 0 errors | paths/include |
| 2 | Ctrl+点击 `@/types` | 跳到 product.ts | 同步 vite alias |
| 3 | 删 `@types/node` 试 `@types` 包 | 看缺什么类型 | npm i -D @types/xx |
| 4 | 故意 `declare module` 错签名 | 调用处类型错 | 改声明 |
| 5 | `vue-tsc --noEmit`（Vue 项目） | SFC 类型通过 | TS 07 |
...
1. **概念**：`.d.ts` 与 `.ts` 区别？
2. **概念**：`import type` 为何有利于 tree-shaking？
3. **概念**：`declare module 'xxx'` 何时用？
4. **概念**：`@/` 别名需哪两处配置一致？
5. **概念**：**.d.ts 菜单翻译** 类比含义？
6. **动手**：写 `types/index.ts` barrel 导出 Product/User。
7. **动手**：扩展 `ImportMetaEnv` 声明 `VITE_API_BASE`。
8. **综合**：axios 类型从哪来（自带 vs @types）？
9. **综合**：06 如何衔接 [Vue 08 联调](../Vue/08-Axios网络请求与前后端联调.md)？
10. **综合**：06 与 [TS 07](./07-Vue3与TypeScript.md) 分工？

### 自测参考答案

1. .d.ts 仅声明无实现；.ts 含可编译实现。
2. 编译后完全移除，不生成空 require。
3. 无类型 JS 包、自研 JS 模块需补类型时。
4. `tsconfig.json` paths 与 `vite.config.ts` resolve.alias。
5. 让 TS 认识 JS 库的 API 形状，不改变运行时。
6. `export type { Product } from './product'` 等。
7. `interface ImportMetaEnv { readonly VITE_API_BASE: string }`
8. axios 现代版自带类型；老库用 @types/xxx。
9. `@/api/request` 导入类型化 request，响应 ApiResult。
10. 06 搭模块与声明基础设施；07 把类型搬进 .vue 与 Pinia。

---

## 14.3 费曼检验

**3 分钟解释：什么是 .d.ts、为什么 Vite 项目还需要 tsconfig paths。**

**对照提纲**：

1. **菜单翻译**：JS 库没类型，.d.ts 告诉 TS 参数和返回值。
2. **货位编号**：`@/types` 避免 `../../../` 地狱。
3. **与框架**：07 章组件 `import type` 都依赖本章模块结构。
