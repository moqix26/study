...
> **读者假设**：已完成 [TS 02](./02-基本类型与类型注解.md) 基本类型，会写 `interface Product` 雏形。本章把 **interface 当合同**——一次约定、处处复用。

### 0.1 用一句话弄懂本章

**一句话**：用 **interface（合同）** 和 **type（别名）** 描述整块对象形状，用 `|` 联合、`&` 交叉组合类型，并定义 **`ApiResult<T>`** 对齐 Java 后端——Vue [08 联调](../Vue/08-Axios网络请求与前后端联调.md) 的类型基石。

**生活类比**：

| 概念 | 类比 |
|------|------|
| **interface** | **合同**——Product 必须有 id/name/price，缺一项违约（编译报错） |
| **type 联合** | 支付方式三选一：微信 \| 支付宝 \| 卡 |
| **type 交叉** | 商品 + 时间戳 = 带审计字段的商品 |
| **ApiResult\<T\>** | 标准快递外包装——code/message/data 格式固定 |

**为什么重要**：[Java 04 Result](../../后端学习/Java/04-SpringBoot核心开发.md) JSON 与 `ApiResult<T>` 一一对应；[TS 07 Vue+TS](./07-Vue3与TypeScript.md) 组件 props 导入 `@/types`。
...
| 缺什么 | 跳到哪 |
|--------|--------|
| 基本类型、对象 | [TS 02](./02-基本类型与类型注解.md) |
| JS 对象、解构 | [JS 06](../HTML%20CSS%20JS/06-JavaScript基础语法与数据类型.md) |
| fetch / JSON | [JS 07](../HTML%20CSS%20JS/07-JavaScript流程控制函数对象数组与ES6基础.md) |

---

### 0.3 本章知识地图（☐→☑）

- [ ] interface 描述 Product/User/Cart/Order
- [ ] `?` 可选与 `readonly`
...
| 阶段 | 时间 |
|------|------|
| §1～§6 interface/type 基础 | 2 小时 |
| §7～§9 联合/交叉/字面量 | 2 小时 |
| §10～§12 ApiResult 实战 | 2 小时 |
| 手把手 + 练习 | 1.5 小时 |
| **合计** | **约 7～8 小时** |
...
1. 独立写出与 curl 后端 JSON 一致的 `Product` interface。
2. `mockProductPage` 故意拼错字段时 TS 报错。
3. 向同学解释 **interface = 合同** 类比。
...
### Q7：interface 合同改了，谁受影响？

所有 `import type { Product }` 的文件——IDE 会列出引用，编译器强制同步修改。

### Q8：交叉类型 `A & B` 和 `extends` 怎么选？

实体扩展用 `interface extends`；type 别名组合、工具类型用 `&`。

### Q9：03 章与 [Vue 08](../Vue/08-Axios网络请求与前后端联调.md) 如何衔接？

08 章 axios 响应类型即 `ApiResult<T>`；拦截器里 `res.data` 需先按合同解析整包。

### Q10：`import type` 和 `import` 混用？

值用 `import`；纯类型用 `import type`，编译后擦掉，避免循环依赖。

### Q11：联合类型 `Product | User` 不收窄会怎样？

访问 `.price` 报错——编译器不知道你是哪种合同。

### Q12：03 章学完能跳过 04 吗？

**不能。** 04 章 **泛型万能模具** 让 `request<T>()` 消费本章的 `ApiResult<T>`。

---

## 15.1 DevTools 与 tsc 验证类型合同

| 步骤 | 动作 | 预期 | 若不对 |
|------|------|------|--------|
| 1 | shop-ts 里 `npm run typecheck` 或 `npx tsc --noEmit` | 0 errors | 对照 §14 报错表 |
| 2 | `mockProductPage.data.list[0].priice = 1` | Property 'priice' 不存在 | interface 合同 |
| 3 | `order.status = 'PENDING'` | 若 type 是小写则报错 | 对齐后端 JSON |
| 4 | Ctrl+点击 `import type { Product }` | 跳到 `types/product.ts` | 检查 paths `@/*` |
| 5 | 改 `ApiResult` 的 `message`→`msg` | 全项目报错 | 合同变更影响面 |

---

## 15.2 闭卷自测

1. **概念**：interface 与 type 各适合什么场景？
2. **概念**：`ApiResult<T>` 中 `T` 是什么？
3. **概念**：可选属性 `?` 与 `\| undefined` 关系？
4. **概念**：字面量联合比 `string` 安全在哪？
5. **概念**：**interface 合同** 类比含义？
6. **动手**：写 `PageResult<Product>` 含 list/total/pageNum/pageSize。
7. **动手**：写 `isApiSuccess` 类型守卫。
8. **综合**：Java `Result<UserVO>` 对应前端什么类型？
9. **综合**：`Product & Timestamps` 与 `extends` 等价写法？
10. **综合**：03 类型如何进入 [TS 07](./07-Vue3与TypeScript.md) 的 `defineProps`？

### 自测参考答案

1. interface：对象实体、可合并；type：联合、交叉、工具类型、别名。
2. 成功时 `data` 字段的业务类型，如 `Product` 或 `PageResult<Product>`。
3. `field?: T` 等价 `field: T | undefined`（strict 下）。
4. 只能赋 `'pending'` 等枚举值，拼写错误编译期拦截。
5. 约定对象必须有哪些字段，违反则编译失败。
6. `interface PageResult<T> { list: T[]; total: number; pageNum: number; pageSize: number }`
7. `return res.code === 0` 配合 `res is ...` 谓词。
8. `ApiResult<User>` 或 `ApiResult<User[]>`。
9. `interface ProductWithMeta extends Product, Timestamps {}`
10. `defineProps<{ product: Product }>()` 从 `@/types` 导入。

---

## 15.3 费曼检验

**3 分钟解释：interface 为什么是「合同」、ApiResult 解决什么问题。**

**对照提纲**：

1. **合同**：Product 规定字段，组件/API 层都签同一份，改字段全项目知晓。
2. **外包装**：ApiResult 统一 code/message/data，axios 不再 any。
3. **与后端**：Java Result 的 JSON 镜像，联调少猜字段。
