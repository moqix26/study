...
> **读者假设**：已完成 [TS 04](./04-函数类型与泛型.md) 泛型与 `request<T>()`。本章用 **class** 封装有状态服务，用 **类型收窄** 在联合类型上安全分支。

### 0.1 用一句话弄懂本章

**一句话**：`class` 组织 CartService 等状态+方法；`enum`/字面量联合描述订单状态；用 `typeof`/`in`/`instanceof` **收窄** 让编译器在 `if` 分支里知道具体类型。

**生活类比**：

| 概念 | 类比 |
|------|------|
| class | 带锁抽屉的服务台——状态和方法封在一起 |
| enum | 固定菜单编号——订单状态只能选几项 |
| 类型收窄 | 安检分流——验完身份证走对应通道 |
| 可辨识联合 | 快递单上 `kind: 'fixed'` 决定走折扣还是满减通道 |
| interface 合同 | class `implements ICartService` 必须履约 |

**为什么重要**：[Vue 07 Pinia](../Vue/07-Pinia状态管理.md) store 可类式组织；[TS 07](./07-Vue3与TypeScript.md) 组件里处理 `Product | null`。
...
| 缺什么 | 跳到哪 |
|--------|--------|
| JS class 基础（若有） | [JS 07](../HTML%20CSS%20JS/07-JavaScript流程控制函数对象数组与ES6基础.md) |
| 联合类型 | [TS 03](./03-接口类型别名与联合交叉.md) |
| typeof 运算符 | [JS 06](../HTML%20CSS%20JS/06-JavaScript基础语法与数据类型.md) |

---

### 0.3 本章知识地图（☐→☑）

- [ ] class 访问修饰符、constructor、implements
- [ ] 数字/字符串 enum vs 字面量联合
- [ ] typeof / instanceof / in 收窄
- [ ] 可辨识联合、类型谓词
- [ ] null/undefined 严格处理
- [ ] CartService / Order 状态机
- [ ] 闭卷自测 ≥ 7/10
...
**约 6～8 小时**（含 CartService 与练习）。

---

### 0.5 可验证成果

1. `CartService` 通过 `implements ICartService` 编译。
2. `switch (coupon.kind)` 漏分支时 `never` 报错。
3. `handleShopError(err: unknown)` 无 any。
...
### Q7：05 章 class 和 Vue 07 Pinia 关系？

Pinia 是响应式 store；class 可封装 **纯业务逻辑** 或测试 mock，二者可并存。

### Q8：类型收窄与 [JS 06 typeof](../HTML%20CSS%20JS/06-JavaScript基础语法与数据类型.md) 区别？

运行时 typeof 返回字符串；TS 收窄是 **编译期** 控制流分析，两者常配合。

### Q9：`private` 在 DevTools 里能看见吗？

能——TS private 仅编译期；运行时是普通 JS 属性。不要靠 private 藏秘密。

### Q10：enum 与后端 JSON 数字不一致怎么办？

以 curl 为准；改 enum 或 adapter，见 [Vue 08](../Vue/08-Axios网络请求与前后端联调.md)。

### Q11：05 完成后为何学 06 模块？

类型与 class 要拆文件 `import`；三方库要 `.d.ts`——06 搭桥梁到 [TS 07](./07-Vue3与TypeScript.md)。

### Q12：instanceof 收窄对 interface 有效吗？

**无效**——interface 编译后不存在；instanceof 用于 class 与内置类型。

---

## 17.1 DevTools 与 tsc 验证收窄

| 步骤 | 动作 | 预期 | 若不对 |
|------|------|------|--------|
| 1 | 联合 `Product \| User` 直接 `.price` | 报错 | 先 in/typeof 收窄 |
| 2 | switch 漏 `cancelled` 分支 | default 里 never 报错 | 补分支 |
| 3 | `catch (e: unknown)` 直接 `e.message` | 报错 | instanceof Error |
| 4 | `tsc --noEmit` | 0 errors | §16 报错表 |
| 5 | Console 运行时 | 类型已擦除；靠逻辑正确 | 编译期是安全网 |
...
1. **概念**：类型收窄是什么？
2. **概念**：可辨识联合 discriminant 作用？
3. **概念**：string enum 比数字 enum 适合 JSON 的原因？
4. **概念**：`implements` 与 interface 合同关系？
5. **概念**：`unknown` 在 catch 中为何优于 any？
6. **动手**：写 `'price' in item` 区分 Product/User。
7. **动手**：写 `assertNever(x: never): never` 穷尽检查。
8. **综合**：CartService private items 如何测？
9. **综合**：OrderStatus 用 enum 还是字面量联合？
10. **综合**：05 如何支撑 Vue 组件里 `product: Product | null`？

### 自测参考答案

1. 用 typeof/in/instanceof 等让联合类型变窄，分支内访问专属属性。
2. 共同字面量字段让 switch 推断各分支具体类型。
3. JSON 序列化为可读字符串，与后端约定一致。
4. class 必须实现 interface 全部成员，否则编译失败。
5. unknown 强制先判断再访问；any 跳过检查。
6. `if ('price' in item) { item.price }`
7. `throw new Error(String(x))`
8. 测 public 方法或通过 `getItems()` 只读暴露。
9. 团队统一；联调优先字面量联合或 string enum。
10. `if (product !== null)` 后 TS 知 product 为 Product。

---

## 17.3 费曼检验

**3 分钟解释：什么是类型收窄、为什么联合类型需要它。**

**对照提纲**：

1. **安检分流**：`Product | User` 像两种乘客，检票后才知道走哪个通道。
2. **可辨识联合**：`kind` 字段像登机牌颜色，switch 一眼分流。
3. **null 检查**：API 的 `data` 可能空，先用 if 收窄再访问字段。
