...
| Agent 上线前 | AgentGo 加固 | **07** + [AIAgent 08](../../后端学习/AIAgent/08-评估可观测安全与成本.md) | 限流、审计、Key 不进 Git | 只做 demo 无限流 |
...
AgentGo / 业务 AI 模块（LLM）
  07 Prompt 注入 → AIAgent 04 Tool 设计 → AIAgent 08 限流/审计/密钥
...
**与 [AIAgent 08](../../后端学习/AIAgent/08-评估可观测安全与成本.md) 的推荐顺序**：

```text
Web安全 00（地图）→ 01～06（Web 基线）→ 07（LLM 威胁模型）
  → AIAgent 01～03（会调 API）
  → AIAgent 04、07（Tool + 有界状态机）
  → AIAgent 03、05～06（会话 + RAG）
  → AIAgent 08（Go 落地：评估、限流、脱敏、熔断）
...
| 07 LLM | 本地 AgentGo、Network 无 Key | 注入测试、Key 不进前端 |
...
| 「AI 安全毕业再学」 | Agent 04 就要设计 Tool | 07 与 Agent 04 §11 同步 | 07、AIAgent 08 |
...
| 07 Prompt | [AIAgent 04 Tool](../../后端学习/AIAgent/04-ToolCalling与安全工具设计.md) | strict Schema + Tool Registry 最小权限 |
| 07 生产化 | **[AIAgent 08 评估与安全](../../后端学习/AIAgent/08-评估可观测安全与成本.md)** | 限流、LogSanitizer、熔断 |
| 07 RAG 注入 | [AIAgent 05 RAG](../../后端学习/AIAgent/05-RAG基础-分块检索与引用.md) | tenantId 过滤 |
| 07 会话 | [AIAgent 03 记忆](../../后端学习/AIAgent/03-流式对话-SSE与会话管理.md) | session 绑 userId |

**深入解释：为何 07 必须链接 AIAgent 08？**  
07 章建立 **威胁模型与 Checklist**（Prompt 注入、Tool 滥用、输出 XSS）；[AIAgent 08](../../后端学习/AIAgent/08-评估可观测安全与成本.md) 提供 **Go 后端落地**（身份上下文、Tool Registry、分层限流、出网与熔断边界）。两章 **同一套标准、不同层次**——面试答「怎么防 Prompt 注入」应先讲威胁模型，再讲 Go 工程控制点。
...
| **L4** | 全栈威胁模型 + AI 安全 | 06～07 + **AIAgent 08** | 15 min 讲 shop + Agent 纵深防御 |
...
AI：用户/RAG 不可信、Tool 绑当前用户、输出 DOMPurify、Redis 限流（07 + AIAgent 08）
```
...
| 出路 | 下一步资料 | 本系列如何支撑 |
|------|------------|----------------|
| **全栈 notehub 上线** | [todo.md](../../todo.md) 第 5 周部署、[Linux 07](../../后端学习/Linux/03-网络端口DNS防火墙与curl.md) | 04 HTTPS、06 授权审计 |
| **Java 后端深化** | Java 05～08、14 面试 | 06 与 MyBatis/Redis 限流衔接 |
| **AI Agent 生产化** | **AIAgent 08** → 12 面试总表 | 07 Checklist 与 AIAgent 08 Checklist 合并勾选 |
...
**本系列（`前端学习/Web安全/`）要做的事**：把 Web 安全从「听说过 XSS」变成「能设计登录态、能配 CSP、能排查 CORS 误配、能跟后端对齐 CSRF/JWT 方案」——为 [Vue 08](../Vue/08-Axios网络请求与前后端联调.md) / [React 08](../React/08-Axios网络请求与前后端联调.md) 联调、[Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) JWT 鉴权、以及 [AIAgent 08](../../后端学习/AIAgent/08-评估可观测安全与成本.md) 生产化安全打底。
...
| AI 聊天被「忽略上文指令」 | 07 章 + [AIAgent 08](../../后端学习/AIAgent/08-评估可观测安全与成本.md) |
...
| AI | 07 | Prompt 注入、Tool 滥用 | 与 AIAgent 08 对齐清单 |
...
| AIAgent 04、07 Tool | 安全 07 | Prompt 注入与 Tool 权限 |
| AIAgent 08 | 安全 07 复习 | 生产化安全双向链接 |
...
| Prompt 注入 / Tool | [07](./07-LLM应用安全与Prompt注入防护.md) | [AIAgent 04 Tool](../../后端学习/AIAgent/04-ToolCalling与安全工具设计.md)、[AIAgent 08](../../后端学习/AIAgent/08-评估可观测安全与成本.md) |
...
07 章偏 **概念与前端展示层**；[AIAgent 08](../../后端学习/AIAgent/08-评估可观测安全与成本.md) 偏 **Go 后端评估、限流、审计与 Tool 权限**。建议 07 → AIAgent 04、07 → AIAgent 08。
...
- [ ] 读过交叉链接表（§6），知道普通后端鉴权与 AIAgent 08 的对应关系
...
**VMware Ubuntu 与安全的关系**：后端部署在 Linux（[Linux 01](../../后端学习/Linux/00-学习路线与环境选择.md)）后，HTTPS 证书、Nginx 头、CSP 多在 **服务器配置** 落地——前端懂威胁，后端懂响应头，联调才不断档。
...
**4 分及以上**：可开始在现有 Web 项目或 AgentGo 做安全加固练习。
...
*本章已按 EXPANSION-STANDARD 扩充（§0 +200 行 §0.6～§0.14、闭卷 §22、费曼 §23、AIAgent 08 衔接）。*

**EXPANSION-STANDARD 自检**：☑ §0 扩展 ☑ notehub 里程碑 ☑ AIAgent 08 链接 ☑ 闭卷 10 题 ☑ 费曼 ☑ 全系列 8/8 索引
