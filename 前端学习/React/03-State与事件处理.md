...
<!-- 修改说明: 2026-06-30 按 EXPANSION-STANDARD 扩充 §0 导读、受控表单步骤表、FAQ≥10、闭卷自测、费曼检验 -->

## 0. 读前导读（零基础也能跟上）

> **读者假设**：02 章 ProductList 能展示。本章深入 **useState**、**受控组件**、**状态提升**，让页面能搜索、能登录。

### 0.1 用一句话弄懂本章

**一句话**：用 `useState` 管「会变的数字和文字」，用 **value + onChange** 让输入框和 JS 数据同步，把搜索词 **提升到父组件** 让兄弟组件共用——升级 shop-react 为「可搜索、可登录」。

**生活类比**：

| 概念 | 生活类比 | 代码 |
|------|----------|------|
| **useState** | 乐高上的可翻数字牌 | `[keyword, setKeyword]` |
| **受控组件** | 价签与电脑库存实时同步 | `value={x} onChange={...}` |
| **不可变更新** | 换价签贴新纸，不涂改旧纸 | `[...arr, item]`、`{...obj, k: v}` |
| **状态提升** | 搜索词写在总控台，各柜台看同一块屏 | keyword 放 App，子只 callback |
| **useEffect 预习** | 定时去仓库核对库存（副作用） | `useEffect(fn, [])` |

与 [Vue 03 计算属性/表单](../Vue/03-计算属性侦听器与表单绑定.md) 对照：Vue 有 `v-model` 语法糖；React 显式受控组件。

**为什么重要**：03 章是 08 章 **登录对接 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md)** 的表单基础；07 章 Zustand 之前，局部 state 都靠本章。
...
| 水平 | 建议 |
|------|------|
| 02 章未完成 | 先完成 ProductList |
| 会 Vue v-model | 对照受控 value+onChange |
| 已做 LoginForm | 重点不可变更新与自测 |

---

### 0.3 本章知识地图（☐→☑）

- [ ] setState 与函数式 updater
- [ ] 对象/数组不可变更新（spread、map、filter）
- [ ] 受控 input/textarea/select/checkbox
- [ ] onSubmit + preventDefault + 简单校验
- [ ] 状态提升：keyword 在父、SearchBar 只 callback
- [ ] useMemo 派生 filteredProducts（或 render 内 filter）
- [ ] useEffect 预习：挂载读 localStorage、清理函数概念
- [ ] 对照 Vue 03 说 v-model vs 受控组件
- [ ] 闭卷自测 ≥ 8/10

---

### 0.4 建议时长：2～3 天（含 LoginForm 跟敲）

### 0.5 可验证成果

1. 输入搜索词，列表实时过滤。
2. 登录表单空提交有错误提示；成功 console 或 alert 用户名。
3. 口述：为什么 props 不能直接改、子如何通知父。
...
**Q：登录密码能存 useState 吗？**  
可以但**不要 persist 密码**；token 放 07 章 Zustand + localStorage；密码只用于提交 [Java 04 登录接口](../../后端学习/Java/04-SpringBoot核心开发.md)。

**Q：和 Vue v-model 怎么对应？**  
`<input value={x} onChange={e => setX(e.target.value)} />` 等价于 `v-model="x"`。

**Q：多个 input 要多个 useState 吗？**  
可以一个 `form` 对象 + 按 name 更新，或各字段独立 state。

**Q：filter 写 render 里还是 useMemo？**  
列表小可 render 内；大列表或 deps 复杂用 `useMemo`（05 章）。

**Q：兄弟组件共享 keyword 怎么办？**  
**状态提升**到父，或 07 章 Zustand「全局抽屉」。

**Q：useEffect 空依赖 [] 做什么？**  
挂载后跑一次（如读 localStorage）；05 章详讲 cleanup。

**Q：表单 submit 刷新页面？**  
`e.preventDefault()` 必须写在 onSubmit 里。

---

## 15.1 闭卷自测

1. 为什么 `count++` 不更新 UI？正确更新方式？
2. 连续两次 `setCount(count+1)` 同一轮里结果是什么？如何 +2？
3. 受控 input 必须哪两个 prop？
4. 不可变更新：给 items 数组追加一项怎么写？
5. 什么是状态提升？搜索词应放哪？
6. onSubmit 为什么要 preventDefault？
7. useEffect 与 useMemo 各解决什么问题？（一句）
8. **动手**：写受控 input 绑定 `keyword` 的一行 onChange。
9. **动手**：filter 商品名包含 keyword（忽略大小写）的 predicate 一行。
10. **综合**：对照 [Vue 03](../Vue/03-计算属性侦听器与表单绑定.md)，说明 computed 与 useMemo 的相似点。

### 15.2 自测参考答案

1. 未走 React 调度；`setCount(count+1)` 或 updater。
2. 只 +1；`setCount(c=>c+1)` 两次。
3. `value` + `onChange`。
4. `setItems(prev => [...prev, newItem])`。
5. 共享 state 移到共同父；keyword 在 App/ProductList 父级。
6. 阻止浏览器默认表单提交刷新。
7. effect=副作用；memo=缓存计算。
8. `onChange={e => setKeyword(e.target.value)}`。
9. `p.name.toLowerCase().includes(keyword.trim().toLowerCase())`。
10. 都缓存依赖 data 的派生结果；deps 变才重算。
...
1. **keyword** 是父组件手里的数字牌（useState）。
2. 输入框 **受控**：显示 keyword，每次按键 onChange 更新 keyword。
3. **filter** 从 products 算出 filteredProducts 再 map——数据单向流，和 [Vue 03 computed](../Vue/03-计算属性侦听器与表单绑定.md) 思路类似。
