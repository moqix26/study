...
<!-- 修改说明: 2026-06-30 按 EXPANSION-STANDARD 扩充 §0 导读、乐高组件类比、FAQ≥10、闭卷自测、费曼检验 -->

## 0. 读前导读（零基础也能跟上）

> **读者假设**：03 章 App.jsx 已有搜索与登录。本章把大文件拆成 **乐高组件**，练 **props 下传、callback 上传、children 组合**。

### 0.1 用一句话弄懂本章

**一句话**：页面 = 多块乐高拼成——**SearchBar、ProductCard、LoginForm** 各管一块，父组件握有 state，子组件通过 **props 只读展示 + onXxx 回调** 与父协作。

**生活类比——React = 乐高组件**：

| 概念 | 生活类比 | 代码 |
|------|----------|------|
| **组件** | 标准积木块 | `ProductCard.jsx` |
| **props** | 包装盒规格（不能撕改标签） | `product={p}` |
| **callback** | 子按铃叫父来收快递 | `onAddToCart(id)` |
| **children** | 积木中间空槽放内容 | `<Layout>{page}</Layout>` |
| **组合 vs 继承** | 拼积木而非把积木熔成一块 | 用 children 而非 class extends |

与 [Vue 04 组件通信](../Vue/04-组件基础与组件通信.md) 对照：Vue `props + emit`；React `props + 回调函数`。

**为什么重要**：工程化 React 的核心；05 章 Hooks 抽逻辑前必须先会拆组件。
...
|------|------|
| 03 章未完成 | 先完成搜索+登录 |
| 会 Vue emit | 把 emit 想成 `onXxx` props |
| 已拆四组件 | 做闭卷+组合模式复习 |
...
- [ ] 单向数据流：父 state → props → 子展示
- [ ] callback props 命名 `onXxx`
- [ ] 受控组件在子、state 可在子或父（LoginForm 模式）
- [ ] children、复合组件概念
- [ ] 目录：`components/SearchBar.jsx` 等
- [ ] PropTypes 或 TS 选型（了解）
- [ ] 完成 shop-react 四组件拆分
- [ ] 对照 Vue 04 emit vs callback
- [ ] 闭卷自测 ≥ 8/10
...
1. `App.jsx` 明显变短，组件各 <120 行。
2. 改 ProductCard 样式只需开一个文件。
3. LoginForm 可在另一页面复用（06 章路由挂）。
...
**单一职责**：ProductCard 只管一张卡片；SearchBar 只管搜+分类。一个文件 50～120 行为宜；见 §0 乐高类比。

### Q9：和后端分层像吗？

父组件像 **Controller 协调**；子组件像 **View 片段**；将来 08 章 **api/** 像 Service 调 [Java 04 REST](../../后端学习/Java/04-SpringBoot核心开发.md)——前后端分离但思路类似。

### Q10：children 和 Vue slot 一样吗？

默认插槽 ≈ `children`；具名 slot 需 04 章复合组件或 props 传节点。

### Q11：能传整个 store 吗？

能但不推荐；07 章 Zustand 用 `useUserStore()` 按需订阅更好。

### Q12：兄弟组件不传 props 怎么通信？

共同父 **状态提升**，或 07 **全局抽屉** Zustand。

---

## 19.1 闭卷自测

1. props 只读，违反会怎样（概念上）？
2. 子传父标准模式是什么？命名惯例？
3. 受控 LoginForm 的 value/onChange 谁持有 state 较常见？
4. children 适用场景举一例。
5. 为什么「在 render 里定义子组件」不推荐？
6. key 在兄弟列表中的作用（与 02 章呼应）？
7. 组合优于继承在 React 里指什么？
8. **动手**：写出 ProductCard 接收 `product` 和 `onAddToCart` 的函数签名。
9. **动手**：父里 `handleAdd(id)` 传给子的 prop 名建议叫什么？
10. **综合**：把 Vue 04 的 `emit('add')` 改写成 React callback 写法。

### 19.2 自测参考答案

1. 破坏单向数据流，难调试；应 callback 让父改 state。
2. **callback props**；`onAddToCart`、`onKeywordChange`。
3. 表单 state 常在 **LoginForm 内**；提交结果 `onLoginSuccess(user)` 给父。
4. Layout 包页面、Card 包正文。
5. 每次父 render 创建新组件类型，子树卸载重建丢 state。
6. 稳定 identity，diff 正确复用 DOM。
7. 用 props/children 拼装，少 `extends Component`。
8. `function ProductCard({ product, onAddToCart })`。
9. `onAddToCart={handleAdd}`。
10. 父 `<Child onAdd={() => ...} />`，子 `onAdd?.()`。

---

## 19.3 费曼检验

3 分钟解释「为什么拆 ProductCard」：

1. **乐高块**复用：列表 map 同一块积木 N 次。
2. **props** 贴规格；**callback** 子按铃父改购物车。
3. 父握总 state，和 [Vue 04](../Vue/04-组件基础与组件通信.md) 一样——数据往下、事件往上。
