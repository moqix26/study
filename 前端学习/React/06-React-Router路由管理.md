...
<!-- 修改说明: 2026-06-30 按 EXPANSION-STANDARD 扩充 §0 导读、FAQ≥10、闭卷自测、费曼检验；链 Vue 06 -->

## 0. 读前导读（零基础也能跟上）

> **读者假设**：05 章 shop-react 仍用 Tab 切换。本章用 **React Router** 让 URL 驱动页面——像商场 **楼层指示牌**，换 URL 换视图不拆大楼（SPA）。

### 0.1 用一句话弄懂本章

**一句话**：安装 `react-router-dom`，配置 `/`、`/products`、`/login`、`/cart` 等路径，用 **Link** 导航、**ProtectedRoute** 拦未登录——浏览器前进后退也能用。

**生活类比**：

| 概念 | 生活类比 | 代码 |
|------|----------|------|
| **SPA** | 同一栋商场，不换大楼 | 一个 index.html |
| **Router** | 楼层指示牌 | `/products` → ProductListPage |
| **Outlet** | 楼层里的展示位 | Layout 里 `<Outlet />` |
| **Link** | 室内导航电梯 | 不整页刷新 |
| **ProtectedRoute** | 会员区闸机 | 无 token 跳 login |

与 [Vue 06 Router](../Vue/06-Vue-Router路由管理.md) 对照：`router-view` ≈ `Outlet`，`router-link` ≈ `Link`。

**为什么重要**：真实项目必有多 URL；07 章守卫读 Zustand；08 章按页面调 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) 不同 API。
...
| 水平 | 建议 |
|------|------|
| 05 章未完成 | 先完成 useProducts 重构 |
| 会 Vue Router | 对照 §23 FAQ 与映射 |
| 已有六页面 | 重点 lazy + 自测 |
...
1. 地址栏 `/products/1` 进详情，刷新 dev 不 404。
2. 未登录访问 `/cart` 跳 `/login?redirect=/cart`。
3. 六页面 + Layout 可切换。
...
### Q7：BrowserRouter 和 HashRouter 怎么选？

生产常用 **History**（createBrowserRouter）+ 服务器 fallback；Hash（`#/cart`）免配服务器但 URL 丑，仅特殊场景。

### Q8：动态路由 id 变了组件不刷新？

同一组件实例复用，需在 `useEffect` 或 loader 里依赖 `id` 重新拉数据。

### Q9：和 Vue 06 最大区别？

API 名不同（Outlet/Link/useNavigate）；概念一致。对照 [Vue 06](../Vue/06-Vue-Router路由管理.md) 同周做六页面。

### Q10：loader 必须学吗？

v6.4+ 推荐；本章可先 element + useEffect，进阶用 loader 预取 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) 商品。

### Q11：404 怎么配？

路由表末尾 `{ path: '*', element: <NotFoundPage /> }`。

### Q12：Link 和 a 标签？

SPA 内用 **Link**；外链或下载用 `<a href>`。

---

## 23.1 闭卷自测

1. SPA 是什么？为什么还需要 Router？
2. `<Outlet />` 做什么？没有会怎样？
3. `useParams` 取什么？`/products/:id` 呢？
4. ProtectedRoute 典型判断逻辑？
5. 生产环境刷新子路径 404 原因与解决（一句）？
6. lazy 为什么要包 Suspense？
7. `navigate('/login', { replace: true })` 与 push 区别？
8. **动手**：写一行 Link 到 `/cart`。
9. **动手**：未登录 redirect 带 query `redirect=/cart` 怎么读？
10. **综合**：对照 Vue 06，写出 Vue 的 `router-view` 在 React 的等价物。

### 23.2 自测参考答案

1. 单 HTML 壳 + JS 换视图；Router 映射 URL→组件。
2. 渲染子路由出口；子路由区域空白。
3. 路径动态段；`const { id } = useParams()`。
4. 无 token/isLoggedIn 则 `<Navigate to="/login" />`。
5. 服务器找物理文件失败；`try_files ... /index.html`。
6. lazy 组件加载中需 fallback UI。
7. replace 不留历史记录。
8. `<Link to="/cart">购物车</Link>`。
9. `useSearchParams()` → `searchParams.get('redirect')`。
10. **Outlet**（或 Routes 匹配的 element 位置）。

---

## 23.3 费曼检验

3 分钟解释「点购物车 URL 怎么变页」：

1. **Link** 改地址栏路径，**不刷新**整页（SPA 同一商场）。
2. **Router** 看路径表，在 **Outlet** 位换对应页面组件。
3. 进 **/cart** 闸机（ProtectedRoute）查 07 **全局抽屉** login 态；08 章再拉 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) 订单 API。
