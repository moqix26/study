...
<!-- 修改说明: 2026-06-30 按 EXPANSION-STANDARD 扩充 §0 导读、Hook=插件类比、FAQ≥10、闭卷自测、费曼检验 -->

## 0. 读前导读（零基础也能跟上）

> **读者假设**：04 章已拆组件。本章学 **Hook = 给函数组件装的插件**：useEffect、useRef、自定义 useProducts。

### 0.1 用一句话弄懂本章

**一句话**：Hooks 是函数组件的 **插件槽**——`useState` 计数、`useEffect` 挂载后拉数据、`useRef` 拿 DOM 或存不触发渲染的值；自定义 Hook 把逻辑抽成 `useXxx` 复用。
...
|------|----------|----------|
| **useState** | 计数插件 | 表单、开关 |
| **useEffect** | 联网/定时插件 | 拉商品、订阅、清理 |
| **useRef** | 便签本（改了不闪屏） | DOM focus、保存 prev |
| **useMemo/useCallback** | 缓存插件 | 大列表、稳定回调 |
| **useProducts**（自定义） | 业务插件包 | 封装 loading/error/data |

与 [Vue 05 组合式 API](../Vue/05-组合式API与script-setup.md) 对照：composable ≈ 自定义 Hook；React **Hooks 必须顶层同步调用**。

**为什么重要**：05 章后 App.jsx 只拼 UI；08 章 Axios 常写在 useEffect 或自定义 Hook 里，对接 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) API。
...
|------|------|
| 04 章未拆组件 | 先拆再学 Hook |
| 会 Vue onMounted | 对照 useEffect `[]` |
| 已写 useProducts | 重点 Rules of Hooks 与自测 |

---

### 0.3 本章知识地图（☐→☑）

- [ ] Rules of Hooks：顶层、不在条件/循环里
- [ ] useEffect deps、cleanup、异步取消
- [ ] useRef 两种用途（DOM vs 可变盒子）
- [ ] useMemo / useCallback 适用场景（不过度用）
- [ ] 自定义 Hook：`useProducts`、`useCart`（单例模式）
- [ ] Strict Mode 开发双调用与 cleanup
- [ ] 对照 Vue composable 异同
- [ ] 闭卷自测 ≥ 8/10
...
1. `hooks/useProducts.js` 提供 loading/data/error。
2. 切换路由或卸载时 effect cleanup 无 warning。
3. 口述：为什么 Hook 不能写在 if 里。
...
### Q8：useEffect 里能 async 吗？

应写 `useEffect(() => { async function load() {...}; load() }, deps)`，不要直接把 effect 回调标 async（返回值应是 cleanup 或 undefined）。

### Q9：自定义 Hook 必须以 use 开头吗？

**必须**。eslint 靠此前缀检查 Rules of Hooks。

### Q10：useProducts 何时换真 API？

[08 章](08-Axios网络请求与前后端联调.md) 把 fetch 目标改为 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) `/api/products`；Hook 结构不变。

### Q11：useCart 单例和 Zustand 区别？

本章 useCart 模块单例理解原理；**多页共享 + 持久化**用 07 Zustand「全局抽屉」。

### Q12：Profiler 面板何时用？

性能问题再开；初学优先 Components 看 props/state。

---

## 19.1 闭卷自测

1. Rules of Hooks 两条核心规则？
2. useEffect 依赖数组 `[]` 表示什么？省略 deps 呢？
3. cleanup 函数何时执行？举一例场景。
4. useRef 改 `.current` 为何不重渲染？
5. useMemo 与 useCallback 区别一句。
6. 自定义 Hook 与普通函数区别？
7. Strict Mode 下 effect 为什么跑两次（开发）？
8. **动手**：写 `useEffect` 挂载时 `document.title='shop'`、卸载还原。
9. **动手**：`useProducts` 返回哪三个常见字段？
10. **综合**：对照 [Vue 05](../Vue/05-组合式API与script-setup.md)，onMounted 对应什么？

### 19.2 自测参考答案

1. 只在顶层调用；只在函数组件/自定义 Hook 里调用；不在条件/循环里。
2. `[]` 仅挂载+卸载；省略则**每次 render** 后都跑（慎用）。
3. 卸载前或 deps 变重新 effect 前；如 clearInterval、abort fetch。
4. React 不订阅 ref 变化。
5. memo 缓存值；callback 缓存函数引用。
6. 名字 `use*` 且内部可调用其他 Hooks。
7. 故意暴露缺少 cleanup 的 bug；生产不会。
8. `useEffect(() => { const t=document.title; document.title='shop'; return () => { document.title=t } }, [])`。
9. `loading`、`data`/`products`、`error`。
10. `useEffect(fn, [])` 或 `onMounted` → effect 空依赖。
...
1. 函数组件本来是纯函数；**Hook** 像 **USB 插件** 给它加 state、副作用、Ref。
2. **useEffect** = 联网插件，挂载后拉数据，离开要 **cleanup** 拔线。
3. **useProducts** 把拉商品打包成插件；08 章换接 [Java 04 API](../../后端学习/Java/04-SpringBoot核心开发.md) 线头即可。
