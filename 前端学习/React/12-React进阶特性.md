...
<!-- 修改说明: 2026-06-30 按 EXPANSION-STANDARD 扩充 §0 导读、memo/lazy 逐行读、FAQ 12 题、闭卷自测、费曼检验 -->

> **文件编码**：UTF-8。本章在掌握 01～11 主线后学习；不影响 MVP 交付，但影响面试深度与复杂组件设计能力。

## 0. 读前导读（零基础也能跟上）

> **读者假设**：11 章 shop-react MVP 能跑。本章补 **Context、memo、lazy、Portal、Error Boundary** 等进阶能力，应对面试深挖。

### 0.1 用一句话弄懂本章

**一句话**：主线技术够做业务；本章教你在**性能、跨层传值、错误隔离、按需加载**场景下用 React 进阶 API，并知道何时**不必**过度优化。

**生活类比**：

| 概念 | 类比 |
|------|------|
| **Context** | 商场广播：深层店铺直接听，不用一层层传话 |
| **React.memo** | 免打扰牌：props 没变就不重新装修 |
| **lazy + Suspense** | 按需进货：进详情页才加载详情模块 |
| **Error Boundary** | 防火分区：一个柜台着火不烧整栋楼 |
...
| 水平 | 建议 |
|------|------|
| 05 章 Hooks 不熟 | 回 [05-Hooks核心与自定义Hooks](./05-Hooks核心与自定义Hooks.md) |
| 07 章 Zustand 已会用 | Context 与之对比见 §1.4 |
| MVP 未交付 | **先完成 11 章**，本章作选修 |
...
- [ ] memo / useMemo / useCallback 使用场景
- [ ] React.lazy + Suspense 路由拆分
- [ ] createPortal Modal
- [ ] Error Boundary（class 组件）
- [ ] 闭卷自测 ≥ 8/10
...
| 阶段 | 时间 |
|------|------|
| Context + 对比 Zustand §1 | 1.5 小时 |
| memo 三件套 §3～§4 | 2 小时 |
| lazy/Portal/ErrorBoundary §6～§8 | 2 小时 |
| React 18 并发了解 §9 | 1 小时 |

---

### 0.5 可验证成果

1. ProductCard 加 memo 后 Profiler render 次数减少。
2. 路由 lazy 后首屏 JS 体积变小（Network 可见 chunk）。
3. 模拟子组件 throw，ErrorBoundary 显示降级 UI。
...
### 1.2 ThemeContext 逐行读

| 行号/字段 | 含义 | 改错会怎样 |
|-----------|------|------------|
| `createContext(null)` | 创建 Context 对象 | 每文件 create 多次则实例不同 |
| `Provider value={value}` | 向下提供值 | value 每次新对象则全树刷新 |
| `useContext(ThemeContext)` | 消费 Context | 在 Provider 外为 null |
| `throw new Error(...)` | 自定义 Hook 守卫 | 漏则静默 undefined 难调试 |
| `useMemo(() => ({ theme, setTheme }), [theme])` | 稳定 value 引用 | 不 memo 则任意父 render 触发子树 |
...
useMemo 缓存**计算结果**；useCallback 缓存**函数引用**，常配合 memo 子组件。

### Q5：lazy 路由白屏怎么办？

外层必须包 `<Suspense fallback={...}>`。

### Q6：Portal 事件为什么还能冒泡到 React 树？

Portal 只改 DOM 挂载点；React 合成事件仍按组件树委托。

### Q7：useTransition 解决什么问题？

标记低优先级更新（如搜索结果），保持输入框流畅（React 18 并发）。

### Q8：与 Vue KeepAlive 对比？

React 无内置；需社区方案或自行缓存路由 outlet。
...
不必；面试能讲 2～3 个真实用过的即可。

---

## 19. 闭卷自测

1. Context value 变化时谁会 re-render？
2. memo 比较 props 默认策略是什么？
3. Suspense fallback 显示时机？
4. Error Boundary 捕不到哪些错误？
5. Context vs Zustand 选型一句话？
6. **动手**：给 ProductCard 加 memo，Profiler 对比 render 次数。
7. **动手**：一条路由改为 lazy import。
8. **综合**：说明 inline `{ onClick: () => {} }` 为何导致 memo 失效。

### 19.1 自测参考答案

1. 该 Context 下所有 useContext 消费者。
2. 浅比较 props 各字段。
3. lazy 组件尚未 resolve 时。
4. 事件处理器内异步错误、SSR、Boundary 自身错误。
5. 少变配置 Context；频繁全局状态 Zustand。
6. 父 state 变时子 props 不变则 render 减少。
7. `const Cart = lazy(() => import('./CartPage'))` + Suspense。
8. 每次 render 新建函数引用，memo 浅比较认为 props 变了。
...
1. **先量后优**：Profiler 找慢组件，再 memo/useMemo，忌全员 memo。
2. **状态放对地方**：频繁变的放 Zustand；主题放 Context 并 useMemo 稳定 value。
3. **隔离与按需**：Error Boundary 防一崩全崩；lazy 减首屏体积。
