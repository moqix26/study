...
<!-- 修改说明: 2026-06-30 按 EXPANSION-STANDARD 扩充 §0 导读、MVP 验收表、FAQ 12 题、闭卷自测、费曼检验 -->

> **文件编码**：UTF-8。本章假设你已完成 01～10 章，拥有可运行的 `shop-react` 骨架。

## 0. 读前导读（零基础也能跟上）

> **读者假设**：01～10 章技术点已学，但还缺一个**能演示、能写进简历**的完整项目。本章是 shop-react 的**总装车间**。

### 0.1 用一句话弄懂本章

**一句话**：把 Router、Zustand、Axios、Ant Design 串成 **shop-react MVP**，对接 [Java 04～10 后端](../../后端学习/Java/04-SpringBoot核心开发.md)，并准备 **15～20 分钟**面试项目介绍。

**生活类比**：

| 概念 | 类比 |
|------|------|
| **MVP** | 最小可营业版商城：能逛、能买、能下单 |
| **四周计划** | 施工排期：先骨架再装修再开业 |
| **STAR 故事** | 面试时的「我遇到过 X，我做了 Y，结果是 Z」 |
| **API 清单** | 前后端对接合同，避免各说各话 |
...
| 水平 | 建议 |
|------|------|
| 08 章联调未完成 | 先打通 [08-Axios](./08-Axios网络请求与前后端联调.md) |
| 09 章 UI 未接入 | 至少完成 Login + Table |
| 后端 demo 未跑 | 启动 [Java 04 DemoApplication](../../后端学习/Java/04-SpringBoot核心开发.md) |

---

### 0.3 本章知识地图（☐→☑）

- [ ] shop-react MVP 8+ 页面可演示
- [ ] docs/API.md 与后端字段对齐
- [ ] 401 双保险（拦截器 + ProtectedRoute）
- [ ] 3 分钟项目介绍稿
- [ ] 2 个 STAR 难点故事
- [ ] 闭卷自测 ≥ 8/10
...
| 阶段 | 时间 |
|------|------|
| Week 1 路由+假数据 | 8～10 小时 |
| Week 2 UI+状态 | 8～10 小时 |
| Week 3 联调 | 10～12 小时 |
| Week 4 部署+面试稿 | 6～8 小时 |

---

### 0.5 可验证成果

1. 录屏：登录→浏览→加购→下单全流程 ≤ 3 分钟。
2. README 含技术栈、截图、启动步骤。
3. 能白板画：Browser → Nginx → [Java 04 Controller](../../后端学习/Java/04-SpringBoot核心开发.md) → DB。
...
| 周 | 交付物 | 验收动作 | 链后端 |
|----|--------|----------|--------|
| W1 | 路由+假数据 8 页 | 每页可导航、无 console 红错 | — |
| W2 | Ant Design Layout+Form+Table | 登录校验、Table loading | — |
| W3 | Axios 全链路 | Network 200、token 持久化 | [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) |
| W3 | CORS/proxy 无红字 | Console 无 CORS | [计网 06 §10](../计算机网络/06-缓存Cookie与会话机制.md) |
| W4 | build+preview | dist 可演示完整流程 | Java 04 8080 |
| W4 | README+3min 稿 | 录音自述不卡壳 | — |

### 3.5 docs/API.md 模板（与 Java 04 对齐）

```markdown
## POST /api/login
- Request: { "username": "string", "password": "string" }  ← LoginDTO
- Response: Result<{ token: string }>
- 前端: src/api/auth.js login()

## GET /api/users
- Response: Result<UserVO[]>
- 前端: getProductList() → usersToProducts 适配
- 后端: [Java 04 UserController](../../后端学习/Java/04-SpringBoot核心开发.md)
```
...
## 21. 联调与跨域速查（链计网06 + Java04）

| 场景 | 方案 | 文档 |
|------|------|------|
| 本地 dev 5173→8080 | Vite proxy | [08 §4](./08-Axios网络请求与前后端联调.md) |
| 浏览器 CORS 红字 | CorsConfig 或改 proxy | [计网 06 §10](../计算机网络/06-缓存Cookie与会话机制.md)、[Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) |
| 生产同域 API | Nginx `/api` 反代 | [10 章](./10-Vite构建与项目部署.md)、[计网 06 §11.3](../计算机网络/06-缓存Cookie与会话机制.md) |
| 登录 token | POST /api/login | [Java 04 LoginController](../../后端学习/Java/04-SpringBoot核心开发.md) |

---

## 22. 闭卷自测

1. shop-react MVP 必做模块有哪些？
2. 为什么 Axios 拦截器用 `getState()` 而不是 Hook？
3. ProtectedRoute 和 401 拦截器各管什么？
4. 项目介绍四段式（30s+60s+60s+30s）各说什么？
5. STAR 四个字母分别代表什么？
6. Week 3 联调验收标准是什么？
7. 简历上 React 项目应写哪些技术关键词？
8. **动手**：列出 shop-react 至少 8 个页面路径。
9. **动手**：写 1 段 401 双保险的 STAR（50 字情境 + 100 字行动）。
10. **综合**：对照 [Java 04 Result](../../后端学习/Java/04-SpringBoot核心开发.md) 说明前端拦截器如何解析。

### 22.1 自测参考答案

1. 登录、商品列表/详情、购物车、订单（MVP）；注册/个人中心可选。
2. 拦截器在组件外，违反 Hooks 规则；Zustand getState 可在任意模块调用。
3. ProtectedRoute：未登录进不了页；拦截器：token 过期调接口时全局 logout。
4. 项目定位；核心功能；技术亮点；难点收获。
5. Situation Task Action Result。
6. 真实 API 四态正常、token 持久化、401 跳登录带 redirect。
7. React 18、Vite、Router v6、Zustand、Axios、Ant Design 5、Spring Boot 联调。
8. /、/products、/products/:id、/cart、/login、/orders 等。
9. 示例：联调时 token 过期反复 401 → 拦截器统一 logout+redirect → 用户重新登录回跳。
10. HTTP 200 时读 body.code，!==0 reject；成功 return res.data。
...
1. **是什么**：React 商城前台，对接 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) REST API。
2. **怎么串**：页面调 api 模块 → Axios 拦截器带 token → Zustand 管用户和购物车 → Router 守卫鉴权。
3. **踩过什么坑**：CORS/proxy（[计网 06](../计算机网络/06-缓存Cookie与会话机制.md)）、SPA 部署 404、拦截器读 token。
