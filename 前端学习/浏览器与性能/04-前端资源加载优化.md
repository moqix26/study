...
[Vue 10](../Vue/10-Vite构建与项目部署.md) 的 `build`、`preview`、环境变量 `VITE_*` 与本章优化在 **preview/staging** 验收。生产 Nginx 配 gzip/brotli、静态长缓存（[Linux 13](../../后端学习/Linux/13-Nginx与Web服务部署.md)）。
...
部署细节见 [Linux 13 Nginx](../../后端学习/Linux/13-Nginx与Web服务部署.md)。
