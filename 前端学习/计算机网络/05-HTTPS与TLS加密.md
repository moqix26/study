...
**与 04 章关系**：04 讲「信怎么写」（HTTP 报文）；本章讲「信怎么**安全寄**」（TLS + 证书）。Go 客户端发 `Authorization: Bearer <token>` 时，**没有 HTTPS 时该头明文可被 Wi-Fi 嗅探**（**嗅探** = 在网络上偷听明文数据）。

**本章缩写先认一遍**

| 词 | 是什么 |
|----|--------|
| **TLS** | Transport Layer Security，传输层安全协议，HTTPS 实际干活的加密层 |
| **CA** | Certificate Authority，证书颁发机构，像「公安局」给网站发数字身份证 |
| **AES-GCM** | 一种对称加密算法，握手后用来加密 HTTP 正文 |
| **ECDHE** | 椭圆曲线密钥交换，握手时安全商量出只有双方知道的密钥 |
| **HSTS** | HTTP Strict Transport Security，告诉浏览器「这个站以后只许走 HTTPS」 |
| **嗅探** | 在链路上偷听未加密流量（公共 Wi-Fi 上尤其危险） |
| **反代 / TLS 终结** | 在 Nginx 上解密 HTTPS，再以 HTTP 转发给内网 Go 8080 |
...
| **完整性** | **消息认证码 MAC**（Message Authentication Code，此处勿与网卡 **MAC 地址**混淆）/ **AEAD** 校验 | 被改包解密失败 |
...
2. **部署架构**：Go 服务通常监听 8080 的明文 HTTP；**对外 443 由 Nginx/Caddy 做 TLS 终结**——即在 Nginx 上解密 HTTPS，再以 HTTP 转发给内网 Go（这叫**反向代理 / 反代**）
...
| 字段 | 示例 | 作用 |
|------|------|------|
| **Subject** | `CN=api.example.com`（**CN** = Common Name，证书绑定的主域名） | 证书绑定的域名 |
...
| **Let's Encrypt 等 DV** | 生产免费证书（**DV** = Domain Validation，只验证你控制该域名） | ✅ 验证通过 |
| **OV/EV** | 企业证书（**OV** = 组织验证；**EV** = 扩展验证，浏览器曾显示绿色公司名，现已少见） | 更严格验证 |
