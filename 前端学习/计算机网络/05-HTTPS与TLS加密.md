...
**HTTPS** = 在 HTTP 外面套一层 **TLS 加密隧道**——像把挂号信放进**带密码锁的保险箱**再寄出；同时用 **CA 证书**证明「收件人确实是 api.shop.com 本人」，不是钓鱼冒充。
...
| CA 证书链 | 公安局盖章的身份证，浏览器认「真服务器」 |
| 443 端口 | 银行专用通道号（HTTP 是 80 号普通窗口） |
| 混合内容拦截 | HTTPS 页面里不许偷偷塞 HTTP 明信片 |
| HSTS | 银行贴告示：「本域只走加密通道，别走旁门」 |

**与 04 章关系**：04 讲「信怎么写」（HTTP 报文）；本章讲「信怎么**安全寄**」（TLS + 证书）。Axios 的 `Authorization: Bearer` 在 HTTP 头里，**没有 HTTPS 时头明文可被 Wi-Fi 嗅探**。
...
| TCP 三次握手 | [02 章](./02-TCP与UDP.md) | ✅ TLS 在 TCP 之后 |
| DNS 域名解析 | [03 章](./03-IP地址与DNS解析.md) | 建议有 |
| shop-vue 登录 token | [Vue 08](../Vue/08-Axios网络请求与前后端联调.md) | 建议有，便于关联 |
| 对称/非对称加密名词 | 本章 §2 从零讲 | 见过即可 |
...
☐ 能口述证书链：根 CA → 中间 CA → 叶子证书，浏览器如何验证
☐ 能对比 TLS 1.2（~2 RTT）与 TLS 1.3（~1 RTT）
☐ 知道 443 端口、Nginx TLS 终结、内网 HTTP 反代架构
☐ 能解释混合内容 Active/Passive 及 shop-vue baseURL 误配后果
☐ 能说明 HSTS 防 SSL Stripping 的原理
☐ 会用 DevTools Security + curl -v 查看 TLS 版本与证书
☐ 能口述 Cookie Secure/HttpOnly 与 JWT localStorage 的安全权衡
☐ 知道 HTTPS 防传输层，不防 XSS（还需 CSP）
```
...
|------|------|------|
| 动机 + 加密基础 | §0～§3 | 45 分钟 |
| TLS 握手 + 端口部署 | §4～§6 | 60 分钟 |
| 混合内容 + HSTS + Cookie | §7～§9 | 45 分钟 |
| 实操 DevTools/curl | §10～§11 | 30 分钟 |
| 自测复盘 | 章末闭卷 + 费曼 | 30 分钟 |

**动手优先**：读完 TLS 1.3 握手后，立刻 `curl -v https://httpbin.org/get`，找 `TLSv1.3` 与 `SSL connection using` 两行。

### 0.5 学完本章你能做什么（可验证的具体动作）

1. 打开任意 HTTPS 站 → DevTools **Security** → 说出 Issuer、有效期、TLS 版本。
2. 解释：shop-vue 生产若 `VITE_API_BASE=http://` 而页面是 HTTPS，Console 报什么错。
3. 白板画 TLS 1.3 简化握手（ClientHello → ServerHello+证书+Finished → Finished）。
4. 写 Nginx 三行核心：`80→301 https`、`443 ssl`、`proxy_pass` 反代 8080。
5. 面试 2 分钟口述 HTTPS 完整稿（见 §31，可对照 §40 费曼）。
...
不是。证书只解决**身份**与**传输加密**；应用层 XSS 仍能偷 localStorage 里的 JWT，还需 CSP、输入转义。内网 Nginx→Java 段若是 HTTP，要依赖**内网隔离**，不能暴露到公网。

**Q2：自签名证书开发能用吗？**  
可以。浏览器会警告，点继续或装 **mkcert** 本地信任根。生产必须用正规 CA（Let's Encrypt 免费 DV 即可）。

**Q3：TLS 1.3 一定比 1.2 快吗？**  
握手通常 **1-RTT vs 2-RTT**，首包更快；但小 API 响应时 TTFB 仍可能 dominated 后端逻辑。用 DevTools Timing 区分 SSL 阶段与 Waiting。

**Q4：为什么 HTTP/2 浏览器要求 HTTPS？**  
...
Header 方案不依赖 Cookie，但生产仍应全站 HTTPS；Secure 针对 Set-Cookie 场景。

**Q7：Nginx 443 终结 TLS，Java 8080 HTTP 安全吗？**  
**浏览器到 Nginx** 必须 HTTPS；Nginx 到 Java 在内网/VPC 可 HTTP，前提是网络隔离，外网打不进 8080。
...
| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | `curl -v https://httpbin.org ??/get 2>&1` | `TLSv1.3 (OUT), Client hello` | 网络/防火墙 |
| 2 | 继续往下看 | `Server hello` + 证书信息 | 证书过期见 §13 表 |
| 3 | 找 `SSL connection using` | 如 `TLS_AES_256_GCM_SHA384` | 服务器只开老 TLS |
| 4 | Chrome F12 → Security | Overview: secure | Mixed content 见 §7 |
| 5 | Network 启用 Protocol 列 | `h2` 或 `http/1.1` | ALPN 协商失败少见 |

---

## 42. Nginx 响应头逐行读（>10 行示例）
...
| 行/头 | 含义 | 改错会怎样 |
|-------|------|------------|
| HSTS max-age | 1 年内强制 HTTPS | 过短频繁降级窗口 |
| includeSubDomains | 子域也强制 | 误配导致 dev 子域 http 失败 |
| nosniff | 禁止 MIME 嗅探 | 少一层 XSS 载体 |
| SAMEORIGIN | 禁止被外站 iframe | 第三方嵌入需改 DENY/SAMEORIGIN 策略 |

---

## 43. 模拟面试：HTTPS 八连问

1. **HTTP 三类风险？** → 窃听/篡改/冒充；TLS+证书应对。  
2. **对称 vs 非对称分工？** → 握手 ECDHE 协商；传输 AES-GCM。  
3. **证书链验证失败浏览器为何阻断？** → 无法确认服务器身份，中间人可冒充。  
4. **TLS 1.3 几 RTT？** → 约 1-RTT（不含 TCP）。  
5. **混合内容 Active？** → 脚本/XHR http 被 block。  
6. **HSTS 防什么？** → SSL Stripping 降级 HTTP。  
7. **shop-vue token 为何必须 HTTPS？** → Authorization 头防嗅探。  
8. **HTTPS 防 XSS 吗？** → 不防；靠 CSP/转义/HttpOnly Cookie 方案。
...
6. JWT 在 localStorage vs HttpOnly Cookie 的 XSS/CSRF 权衡？

### 动手题（2 题）

7. 写出 Nginx `80→301 https` 与 `443 ssl` 反代 `8080` 的最小 server 块骨架（各 3 行核心）。
8. DevTools 如何查看当前页 TLS 版本与证书 Issuer？

### 综合题（2 题）

9. shop-vue 生产 `.env` 误配 `VITE_API_BASE=http://api.shop.com`，页面是 HTTPS，会发生什么？如何修？
10. 2 分钟口述：浏览器访问 `https://api.shop.com/api/login` 的 TCP+TLS+HTTP 分层过程。
...
**6.** localStorage：CSRF 低、XSS 可读 token；HttpOnly Cookie：XSS 难读、CSRF 需 SameSite/Token。

**7.** `listen 80; return 301 https://$host$request_uri;` + `listen 443 ssl; ssl_certificate ...; proxy_pass http://127.0.0.1:8080;`

**8.** F12 → Security → View certificate / Main origin TLS 信息。

**9.** Mixed Content blocked，Axios 失败；改 `https://` 或同域 `/api` 反代。

**10.** DNS→TCP 443→TLS 1.3 握手验证书→AES 加密 HTTP POST→Nginx 解密→反代 Java。
...
1. **HTTP 像明信片谁都能看**；HTTPS 把信放进**带锁保险箱**，钥匙是握手商量出来的。
2. **CA 证书像身份证**，浏览器只认公安局（根 CA）盖章的，假网站会被拦。
3. **小锁不等于万能**：防路上被偷听，**防不了**你电脑里病毒脚本偷 token，还要防 XSS。

---

*本章已按 EXPANSION-STANDARD 扩充（§0+保险箱类比+FAQ+闭卷自测+费曼）。*

**EXPANSION-STANDARD 自检**：☑ §0 导读 ☑ 保险箱类比 §0.1 ☑ TLS 观测步骤 §41 ☑ Nginx 头逐行 §42 ☑ FAQ §40 ☑ 闭卷 10 题 §44 ☑ 费曼 §45

| 扩充项 | 所在 § |
|--------|--------|
| 保险箱+身份证类比 | §0.1 |
| DevTools Security | §10 |
| 混合内容 / HSTS | §7～§8 |
| 闭卷自测 | §44 |
| 费曼检验 | §45 |
...
**TLS（Transport Layer Security）**：HTTP 与 TCP 之间的加密协议层。  
**生活类比**：寄信前先套密码锁信封。  
**为什么重要**：防窃听、篡改、冒充服务器。  
**本章用到的地方**：§4 握手、§10 DevTools。

**AEAD（Authenticated Encryption with Associated Data）**：同时加密与校验完整性的算法族（如 AES-GCM）。  
**生活类比**：带封蜡的信，拆开即知是否被拆过。  
**为什么重要**：TLS 1.3 仅允许 AEAD，废弃弱 cipher。  
**本章用到的地方**：§2.3、§4.2。

**OCSP Stapling**：服务器在握手时附带证书吊销状态。  
**生活类比**：身份证上直接贴「未注销」条，不用再去派出所查。  
**为什么重要**：减少客户端额外查询、加快握手。  
**本章用到的地方**：§21。

---

## 47. shop-vue HTTPS 上线 Checklist（可复制）

```text
□ VITE_API_BASE=https://（或同域 /api）
□ Nginx 443 证书有效 + 链完整
□ 80 → 301 → 443
□ 静态 CDN 与 API 无 http:// 引用
□ Cookie 若用：Secure + HttpOnly + SameSite
□ DevTools Security 无 Mixed Content
□ curl -v 见 TLS 1.2+ 
□ 云安全组开放 443
```
