...
| 词 | 是什么 |
|----|--------|
| **CCPC / ACM** | 大学生算法竞赛名称；本文档用它们指「你有算法基础」，与计网题本身无关 |
| **QPS** | Queries Per Second，每秒处理的请求数 |
| **INCR** | Redis 命令：对某个 key 原子 +1，常用于计数、限流 |
| **BFF** | Backend for Frontend，给前端单独开的同源 API 层，用来避免浏览器跨域 |
| **wrk** | 命令行 HTTP 压测工具 |
| **P99 延迟** | 99% 的请求响应时间都低于该值，衡量「尾部慢请求」 |
| **Cache Aside** | 旁路缓存：先读缓存，没有再去读数据库并回填缓存 |
| **SETNX** | Redis「仅当 key 不存在时才设置」，常用来做分布式锁 |
| **布隆过滤器** | 概率数据结构：判断元素「可能存在」或「一定不存在」，用来减少打穿数据库 |
| **SLB** | Server Load Balancer，云上的负载均衡，把流量分给多台机器 |
| **K8s** | Kubernetes，容器编排平台，用来部署和管理一堆服务实例 |
