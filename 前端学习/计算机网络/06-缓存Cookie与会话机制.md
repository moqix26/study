...
> **前置**：[05-HTTPS与TLS加密](./05-HTTPS与TLS加密.md)、[04-HTTP协议深入](./04-HTTP协议深入.md)、[HTML 10 浏览器 HTTP 基础](../HTML%20CSS%20JS/10-浏览器HTTP网络与Web基础.md)、[Vue 08 Axios 联调](../Vue/08-Axios网络请求与前后端联调.md)。
...
浏览器和 CDN 会**记住**看过的文件（**缓存**），服务器用 **Cookie** 让浏览器「下次自动出示会员证」，前后端分离项目常用 **JWT Token** 代替 Session——本章把这些机制和 **跨域 CORS**、**shop 登录**串成一条你能亲手验收的链路。

**核心类比表**

| 概念 | 生活类比 |
|------|----------|
| **强缓存** | 冰箱里的便当，保质期内**不打电话问饭店**能不能吃 |
| **协商缓存（304）** | 便当过期了，**打电话问**「还是昨天那批吗？」——是则不用重送 |
| **CDN** | 连锁便利店在各小区设分店，不用每次都去总仓 |
| **Cookie** | 健身房**手环**，进门自动识别会员 |
| **Session** | 前台**登记簿**（服务端），手环只存编号 |
| **JWT Token** | 自带信息的**电子票**（localStorage），进门要手动出示 |
| **CORS** | 浏览器保安：「别的店的 JS **不许读**我们店 API 的响应」 |
| **Vite proxy** | 开发时让前端「假装」和后端同店，保安不拦 |
...
| Axios 拦截器基础 | [Vue 08](../Vue/08-Axios网络请求与前后端联调.md) | 建议有 |
| Pinia 存 token | [Vue 07](../Vue/07-Pinia状态管理.md) | 建议有 |
...
☐ 能说明 SPA：index.html no-cache，hash JS 长缓存
☐ 能对比 Session+Cookie、JWT Header、JWT HttpOnly Cookie
☐ 能解释 localStorage token 为何不自动出现在请求里
☐ 能排查 OPTIONS 预检失败与 CORS 红字
☐ 能讲清 Vite proxy（开发）vs Nginx 同域（生产）
☐ 能独立验收 shop 登录链路（§12.7 表全过）
☐ DevTools 能区分 memory/disk cache 与 304
```
...
| CORS + shop 登录实操 | §10～§12 | 75 分钟 |
| 自测复盘 | 章末闭卷 + 费曼 | 30 分钟 |

### 0.5 学完本章你能做什么（可验证的具体动作）

1. DevTools Network 勾选 Disable cache，对比同一 JS 第二次请求的 `(disk cache)` 与 304。
2. 登录 shop-vue 后，在 Request Headers 里找到 `Authorization: Bearer`。
3. 向非技术朋友解释：「发版后用户看到旧页面，多半是 HTML 被强缓存了。」
4. 口述 CORS 预检：为何 `application/json` + `Authorization` 要先 OPTIONS。
5. 完成 §12.7 验收表 5 步全绿。
...
| Redis Session | [06-Redis](../../后端学习/Java/07-Redis核心原理与缓存实战.md) |
| 网关统一鉴权 | [12 架构](../../后端学习/Java/12-高并发与分布式系统基础.md) |
...
**Q2：发版后用户旧页面，先查什么？**  
`index.html` 的 `Cache-Control` 是否被 CDN/浏览器长期 `max-age`；构建 JS 是否带 hash。

**Q3：304 和 200 (from disk cache) 区别？**  
304 **有网络往返**（只下响应头）；强缓存命中**零请求**。

**Q4：为什么 JWT 不自动出现在请求里？**  
HTTP 规范只自动带 Cookie；localStorage 需 Axios 拦截器加 `Authorization`。

**Q5：Session 和 JWT 哪个更好？**  
无绝对答案：Session 易注销、有状态；JWT 易扩展、跨域 Header 简单。shop-vue 主线用 JWT Header。

**Q6：CORS 是后端拦请求吗？**  
不是。请求常**已到服务器**；浏览器因响应头不合规**不把 body 交给 JS**。

**Q7：开发为何推荐 Vite proxy 而非 CORS？**  
浏览器认为 5173→5173 同源，无预检；生产 Nginx 同域同理。

**Q8：CDN 会缓存带 Authorization 的 API 吗？**  
不应配置缓存；默认 API 用 `private, no-store`。

---

## 21. 手把手：DevTools 验证缓存四场景

| 步骤 | 你的动作 | 预期 | 若不对 |
|------|----------|------|--------|
| 1 | Network 勾选 **Disable cache**，硬刷新 | 全部 200 完整下载 | 查离线/Service Worker |
| 2 | 取消 Disable，再刷新同一 hash.js | `(disk cache)` 或 `(memory cache)` | 响应头无 max-age |
| 3 | 改 HTML 为 `no-cache`，刷新 HTML | 304 或 200 新 HTML | CDN 误配长缓存 |
| 4 | 登录后看 profile 请求 | Request 有 `Authorization` | 拦截器未读 store |
| 5 | Application → Local Storage | 有 `token` 键 | login 未写入 |

---

## 22. Set-Cookie 逐行读（Session 方案）
...
2. **SPA 为何 HTML no-cache、JS 长缓存？** → HTML 是指针；hash 变即新 URL。  
3. **Cookie vs localStorage token？** → 自动携带 vs 拦截器 Bearer。  
4. **OPTIONS 何时出现？** → 非简单请求（JSON + Authorization）。  
5. **401 前端做什么？** → 清 token、跳登录、带 redirect。  
...
5. 简单请求与需预检请求各举 shop-vue 一例。
6. HttpOnly、Secure、SameSite 各防什么？

### 动手题（2 题）

7. 写出 Vite `server.proxy` 把 `/api` 转到 `8080` 的最小配置片段。
8. DevTools 如何确认某 JS 是 304 还是强缓存？

### 综合题（2 题）

9. 设计 shop 登录：JWT + localStorage + 401 logout，画 5 步时序（文字即可）。
10. 生产前后端不同域时，CORS 需配哪些响应头？`credentials: true` 时 Allow-Origin 能写 `*` 吗？

### 自测参考答案

**1.** 强缓存=保质期内直接吃；协商=过期后问是否还是同一批，是则 304 不重送 body。
...
**5.** 简单：GET 商品列表；预检：POST login JSON + Bearer。

**6.** HttpOnly 防 XSS 读 Cookie；Secure 仅 HTTPS；SameSite 减 CSRF。

**7.** `proxy: { '/api': { target: 'http://localhost:8080', changeOrigin: true } }`

**8.** Status 304 vs Size 列 `(disk cache)` 且无 304 状态。

**9.** POST login→存 token→GET profile 带 Bearer→401 logout→路由守卫 redirect。
...
1. **缓存像冰箱**：没过保质期不用问；过期了打电话问（304），没变就不重送整盒。
2. **Cookie 像手环自动进门**；JWT 在 localStorage 像**票在口袋里**，检票要自己掏（拦截器）。
3. **跨域是浏览器保安**，Postman 没有保安；开发用 proxy 假装同一家店。

---

*本章已按 EXPANSION-STANDARD 扩充（§0+冰箱类比+FAQ+闭卷自测+费曼）。*

**EXPANSION-STANDARD 自检**：☑ §0 导读 ☑ 冰箱/手环类比 §0.1 ☑ DevTools 四场景 §21 ☑ Set-Cookie 逐行 §22 ☑ FAQ §20 ☑ 闭卷 10 题 §24 ☑ 费曼 §25

---

## 26. Cache-Control 组合决策表
...
| 登录/支付 API | `private, no-store` | 禁止任何层缓存 |
| 公共 GET 列表（短） | `public, max-age=60` | 需业务同意 |

---

## 27. CORS 响应头最小集（Spring 对照）
...
1. DevTools 制造强缓存 / 304 / 全量 200 各一次。  
2. 跑通 §12 shop 登录验收表 5 步。  
3. 口述 Session vs JWT 对比表（§8.4）。  
4. 闭卷自测 §24 ≥8/10。  
5. 费曼 3 分钟录音自评。  
6. 能解释为何 API 默认 `no-store`。  
7. 对照 Vue 10 部署章核对 Nginx 缓存 location。
