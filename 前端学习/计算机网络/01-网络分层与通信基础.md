...
| 本章概念 | 后续文档 |
|----------|----------|
| HTTP 报文细节 | [02 HTTP 与 HTTPS 协议深入](./02-HTTP与HTTPS协议深入.md)（待写） |
| DNS 解析流程 | [03 DNS 与域名解析](./03-DNS与域名解析.md)（待写） |
| TCP 三次握手、端口 | [04 TCP 与 UDP](./04-TCP与UDP.md)（待写） |
| 同源、CORS、Vite proxy | [05 跨域 CORS 与开发代理](./05-跨域CORS与开发代理.md)（待写） |
| TLS、证书 | [06 Web 安全与 HTTPS 实战](./06-Web安全与HTTPS实战.md)（待写） |
| 面试总表 | [07 面试专题与知识点总表](./07-面试专题与知识点总表.md)（待写） |
...
下一章（**02 HTTP 与 HTTPS 协议深入**）会聚焦**应用层里最常用的 HTTP**：请求行/响应行、常用 Method 与 Header、Keep-Alive 与 HTTP/2 多路复用、HTTPS 与 TLS 在 TCP 之上如何衔接。学完后你能**手工读懂** Network 里 Headers 的每一行，并用 `curl -v` 看到完整往返报文——直接服务 [Vue 08](../Vue/08-Axios网络请求与前后端联调.md) 的接口调试。

---

*上一章：[00 学习路线图与说明](./00-学习路线图与说明.md)*  
*下一章：[02 HTTP 与 HTTPS 协议深入](./02-HTTP与HTTPS协议深入.md)（待写）*
