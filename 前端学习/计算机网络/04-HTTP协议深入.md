...
**HTTP（HyperText Transfer Protocol，超文本传输协议）** = 浏览器和服务器之间「写信的格式」——信封上写清楚：什么操作（GET/POST）、找谁（Host）、内容类型（Content-Type）、是否带登录凭证（Authorization）。
...
| 信纸正文 | **Body**（JSON、表单、HTML） |
| 邮局回执「已签收 / 查无此人」 | **状态码** 200 / 404 / 500 |
| 挂号信必须走可靠通道 | 跑在 **TCP** 上（02 章「打电话」） |

**Git 无关本章**；版本管理类比 **游戏存档** 见 Git 系列。你要建立的是：**TCP 接通电话 → HTTP 在电话里说的内容格式**。
...
| fetch/Axios 发过请求 | HTML 09～10 | 建议 |
| Spring Boot `Result<T>` | [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) | 联调时建议 |

**最低门槛**：知道「浏览器发请求、服务器回 JSON」即可；报文结构本章从零教。

### 0.3 本章知识地图（学完后应能勾选全部 ☐→☑）

```text
☐ 能画出 HTTP 请求/响应结构（起始行、头、空行、Body）
☐ 能解释 Host、Content-Type、Accept、Authorization
☐ 能区分 GET/POST/PUT/DELETE/PATCH 语义与幂等性
☐ 能列举 1xx～5xx 每类至少 2 码，区分 401 与 403
☐ 能对比 HTTP/1.1、HTTP/2、HTTP/3 核心差异
☐ 能说明 Keep-Alive 与队头阻塞
☐ 能设计 RESTful URI 并对照 Spring 注解
☐ 能用 curl -v 完成 GET/POST + Bearer Token
☐ 能在 Network 面板完成一次登录请求全链路分析
☐ 能口述 Result JSON 与 HTTP 状态码的配合（2 分钟）
```
...
| 报文与方法 | §1～§4 | 60 分钟 |
| 状态码 + REST | §5～§8 | 50 分钟 |
| curl + DevTools | §9～§10 | 60 分钟 |
| 版本与进阶 | §6～§7、§17～§21 | 45 分钟 |

**强烈建议**：本地启动 Java 04 demo（8080），本章每个 `curl` 命令都跟做一遍。

### 0.5 学完本章你能做什么（可验证的具体动作）

1. 读 `curl -v` 输出，区分 `>`（发出）与 `<`（收到）行。
2. 设计一套「商品 CRUD」REST 接口表（Method + URI），无动词 URI。
3. shop-vue 登录失败时，同时看 **HTTP status** 与 **body.code** 再下结论。
4. DevTools 里 **Copy as cURL**，在终端复现浏览器请求。
5. 面试 2 分钟：HTTP 无状态、幂等、401 vs 403、HTTP/2 多路复用。
...
**Q：HTTP 和 HTTPS 是一回事吗？**  
不是。HTTPS = HTTP + TLS 加密；本章讲明文 HTTP 报文结构，05 章讲 TLS。

**Q：GET 和 POST 哪个更安全？**  
都不加密时都不安全；安全靠 HTTPS，不是 Method。

**Q：为什么后端总返回 HTTP 200，错误也在 body 里？**  
国内常见 `Result<T>` 风格，便于前端统一拦截；面试要能说出利弊（§8.4）。

**Q：HTTP/2 我要在前端配置吗？**  
一般不用，Nginx/CDN 自动协商；DevTools Protocol 列可看 `h2`。

**Q：OPTIONS 请求是我写的吗？**  
浏览器 CORS 预检自动发；后端需正确响应 Allow-Origin 等头。

---

## 29.1 HTTP 请求报文逐行读（>10 行示例）
...
| 行 | 含义 | 改错会怎样 |
|----|------|------------|
| 1 | 方法 + 路径 + HTTP 版本 | 404/405 |
| 2 | 虚拟主机路由（HTTP/1.1 必填） | 虚拟主机路由错 |
| 3 | Body 是 JSON | 415 或解析失败 |
| 4 | 期望响应 JSON | 可能返回 HTML |
| 5 | Body 字节数 | 截断或等待超时 |
| 7 | 登录凭证 JSON | 400 校验失败 |

---

## 29.2 curl 实操步骤表（Java 04 demo）

| 步骤 | 命令 | 预期 | 若不对 |
|------|------|------|--------|
| 1 | 启动 demo 8080 | Started ... | Connection refused |
| 2 | `curl -v http://localhost:8080/api/users` | `< HTTP/1.1 200` + JSON | 路径/端口 |
| 3 | POST login 拿 token | `data.token` 存在 | 账号密码 |
| 4 | `-H "Authorization: Bearer ..."` | 200 用户详情 | 401 token |
| 5 | DevTools 对照 Headers | 与 curl 一致 | proxy 改 Host |

---

## 29.3 状态码与 shop 前端处理对照

| HTTP | body.code | 前端动作 |
|------|-----------|----------|
| 200 | 0 | 正常渲染 data |
| 200 | ≠0 | Toast message，业务失败 |
| 401 | — | 清 token，跳 /login |
| 403 | — | 无权限页 |
| 404 | — | 空状态 / 检查 URL |
| 5xx | — | 友好错误 + 可选上报 |
| 无 status | — | 查 TCP/CORS/后端是否启动 |

---

## 29.4 REST 设计 checklist（上线前自检）

- [ ] URI 用名词复数 `/api/users`，无动词 `getUser`
- [ ] GET 无副作用，改数据用 POST/PUT/PATCH/DELETE
- [ ] POST 创建返回 201 或 200+Result 团队统一
- [ ] 错误信息可读（400 message 或 Result.message）
- [ ] 需要登录的接口校验 Authorization
...
1. **无状态** + 登录态方案？→ Cookie/Session/JWT。  
2. **幂等**？→ GET/PUT/DELETE 通常幂等；POST 创建非幂等。  
3. **401 vs 403**？→ 未认证 vs 无权限。  
4. **Keep-Alive**？→ HTTP/1.1 默认复用 TCP，减握手。  
5. **HTTP/2 多路复用**？→ 单 TCP 多 Stream 并行。  
6. **Result 始终 200** 利弊？→ 前端统一拦截 vs REST 纯语义。

---

## 29.6 本章总复习清单

1. 手写 HTTP 请求/响应报文各一例（含 JSON Body）。  
2. curl 完成 GET/POST/login/token/DELETE 全流程。  
3. DevTools 分析一次 login：Headers + Timing + Preview。  
4. 设计商品 REST 接口表 6 行。  
5. 闭卷自测 ≥8/10；费曼 3 分钟。  
6. 背诵附录速查卡。  
7. 说明 HTTP/1.1 vs h2 vs HTTP/3 一句差异。  
8. 配置 Axios 拦截器同时看 status 与 body.code（§21 示例）。

---

## 29.7 附录：HTTP 一句话口语总结

- **HTTP** = 寄信格式：请求行是地址，Header 是标签，Body 是正文。  
- **无状态** = 服务器不记你；token 自己每次带上。  
- **200** = 邮局说收到了；**404** = 查无此人；**500** = 邮局内部乱了。  
- **401** = 没出示证件；**403** = 有证件但不让进。  
- **GET** = 查阅；**POST** = 提交新东西；**PUT** = 整份替换。  
- **REST** = URI 用名词，动词交给 Method。  
- **Keep-Alive** = 一封信邮筒里连发多封信（同 TCP）。  
- **HTTP/2** = 一个信封里装多封信（多路复用）。  
- **Result JSON** = 信封上写 200，信里另说业务成败（code 字段）。

*本章已按 EXPANSION-STANDARD 扩充（§0+HTTP=寄信类比+逐行读表+自测+费曼）。*

**EXPANSION-STANDARD 自检**：☑ §0 导读 ☑ 报文逐行读 §29.1 ☑ curl 步骤表 §29.2 ☑ REST checklist §29.4 ☑ 闭卷 10 题 ☑ 费曼 ☑ 状态码对照 §29.3
...
1. 用「**寄信格式**」类比：HTTP 请求行、Header、Body 各对应信的哪部分？
2. HTTP **无状态**是什么意思？登录态靠什么补？
3. GET 与 POST 在语义、幂等、参数位置上各写一点区别。
4. **401** 与 **403** 各表示什么？shop-vue 场景各举一例。
5. HTTP/1.1 队头阻塞是什么？HTTP/2 如何缓解？
6. RESTful 为什么反对 `POST /api/getUserById`？应改成什么？

### 动手题（2 题）

7. 写出带 JSON Body 的 POST 登录 curl 命令（含 Content-Type）。
8. DevTools 里如何看到 Protocol 是 `http/1.1` 还是 `h2`？

### 综合题（2 题）

9. Spring 返回 HTTP 200 但 `code:1` 表示业务失败——前端 Axios 应如何处理？
10. 用 2 分钟对比 HTTP/1.1、HTTP/2、HTTP/3 的传输层与多路复用差异。

### 自测参考答案

**1.** 请求行+Host 像信封地址；Header 像附加标签；Body 像信纸正文。

**2.** 服务器默认不记上次请求；Cookie/Session/JWT 在后续请求里自带凭证。

**3.** GET 查询、幂等、参数在 Query；POST 提交、非幂等、Body 常见。

**4.** 401 未认证（无 token）；403 已认证无权限（普通用户访问 admin）。

**5.** 同 TCP 连接上响应须按序返回，慢请求阻塞后续；HTTP/2 多 Stream 并行。

**6.** URI 应用名词表资源；应 `GET /api/users/{id}`。

**7.** `curl -X POST http://localhost:8080/api/login -H "Content-Type: application/json" -d "{\"username\":\"admin\",\"password\":\"123456\"}"`

**8.** Network 表头右键勾选 **Protocol** 列。

**9.** 响应拦截器：HTTP 2xx 仍检查 `body.code === 0`，否则 reject + Toast。

**10.** 1.1+TCP 串行/Keep-Alive；2+TCP 多路复用+HPACK；3+QUIC(UDP) 减 TCP 队头阻塞、连接迁移。
...
1. **HTTP 是信纸格式**：写清楚要什么（GET/POST）、找谁（Host）、带什么（JSON Body）、服务器用状态码回信（200/404）。
2. **HTTP 跑在 TCP 上**：像挂号信必须走可靠通道；无状态所以要自己带 token。
3. **联调看两层码**：HTTP status + Result 里 `code`；401/403 别混。
...
| TCP 传输层 | [02 TCP 与 UDP](./02-TCP与UDP.md) |
| DNS 解析 | [03 IP 地址与 DNS 解析](./03-IP地址与DNS解析.md) |
