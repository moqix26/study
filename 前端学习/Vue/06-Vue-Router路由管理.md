...
<!-- 修改说明: 2026-06-30 按 EXPANSION-STANDARD 扩充 §0 导读、router 逐行读、DevTools、FAQ 12 题、闭卷自测、费曼检验 -->

## 0. 读前导读（零基础也能跟上）

> **读者假设**：05 章 composable 已抽好逻辑，但 `shop-vue` 仍是单页 Tab 切换。本章把「页面」变成 **URL + 独立视图**，为 07 Pinia 跨页共享、08 Axios 按页调接口打基础。

### 0.1 用一句话弄懂本章

**一句话**：**Vue Router** 把浏览器地址栏路径映射到 Vue 组件——点导航只换 `<router-view>` 里的内容，不整页刷新，像商场里换展区不换大楼。

**生活类比**：

| 概念 | 类比 |
|------|------|
| **路由（Route）** | 商场楼层指示牌：3F → 数码区 |
| **router-view** | 展区玻璃门：当前楼层只展示对应展区 |
| **router-link** | 室内导览屏按钮，不走室外旋转门（不刷新整站） |
| **导航守卫** | 保安：进 VIP 区（/cart）要先出示会员卡（token） |

**为什么重要**：真实商城必有首页/列表/详情/登录/购物车多 URL；后端 [Java 04 REST](../../后端学习/Java/04-SpringBoot核心开发.md) 按资源路径设计接口，前端路由与之对齐（`/products/:id` ↔ `GET /api/products/{id}`）。
...
| 水平 | 建议 |
|------|------|
| 05 章 composable 不熟 | 先完成 [05-组合式API与script-setup](./05-组合式API与script-setup.md) |
| 不懂 URL 路径/query | 补 [计网 04 HTTP](../计算机网络/04-HTTP协议深入.md) §路径与查询串 |
| 已会 05 章 | **从 §3 安装 Router 跟做 §5～§9** |

**后端衔接（可选预习）**：

| Java 章 | 与路由的关系 |
|---------|--------------|
| [04 Spring Boot 核心](../../后端学习/Java/04-SpringBoot核心开发.md) | `@GetMapping("/api/products/{id}")` 与前端 `/products/:id` 一一对应 |
| [06 MySQL](../../后端学习/Java/06-MySQL基础索引与事务.md) | 详情页 id 来自数据库主键 |
| [07 Redis](../../后端学习/Java/07-Redis核心原理与缓存实战.md) | 08 章登录 token 可存 Redis，守卫读 Pinia token |

---

### 0.3 本章知识地图（☐→☑）

- [ ] 理解 SPA 与 MPA 区别，知道为何用 router-link
- [ ] 配置 `createRouter`、`routes`、`<router-view>`、`<router-link>`
- [ ] 使用动态路由 `:id`、query 传参，理解 params vs query
- [ ] 编写 `beforeEach` 登录鉴权 + redirect 回跳
- [ ] 配置懒加载与 404 兜底路由
- [ ] 理解 History 模式部署 fallback（10 章 Nginx）
- [ ] DevTools 验证路由切换与激活 class
- [ ] 闭卷自测 ≥ 8/10
...
|------|------|
| SPA 概念 + 安装 §1～§3 | 45 分钟 |
| 完整 routes + 六页面 §4～§9 | 2.5 小时 |
| params/query/守卫 §10～§16 | 2 小时 |
| 懒加载/keep-alive/生产案例 §17～§22 | 1.5 小时 |
| DevTools + 自测 | 45 分钟 |
...
1. 浏览器访问 `http://localhost:5173/products/2` 显示对应商品详情。
2. 未登录访问 `/cart` → 跳 `/login?redirect=/cart`，登录后自动回购物车。
3. 访问 `/abc` 显示 404 页，不白屏。
4. DevTools → Vue → Router 面板可见当前 route.name、params。

---

### 0.6 核心术语三件套

**术语（SPA 单页应用 Single Page Application）**：整站只加载一次 HTML/JS，后续「换页」由前端路由替换组件，无整页刷新。
**生活类比**：商场不换大楼，只换楼层展区——用户感觉在逛不同房间，其实还在同一栋 SPA 里。
**为什么重要**：Vue 商城、后台的标准形态；与 [计网 04 HTTP](../计算机网络/04-HTTP协议深入.md) 的请求仍走同一域名，只是路径变。
**本章用到的地方**：§1、§18 History 模式。

**术语（Vue Router 路由）**：官方库，维护 path → component 映射表，提供导航 API 与守卫。
**生活类比**：商场总控台——地址栏说「去 3F 数码」，它就打开对应 Vue 组件。
**为什么重要**：06 章之后每个功能模块独立 URL，08 章按路由页调不同 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) 接口。
**本章用到的地方**：§2～§5 全文。

**术语（导航守卫 Navigation Guard）**：路由跳转前/后执行的钩子，用于鉴权、改标题、埋点。
**生活类比**：展区入口保安——没会员卡（token）不能进购物车区。
**为什么重要**：07 章从 Pinia 读 `isLoggedIn`；与后端 [Java 04 JWT](../../后端学习/Java/04-SpringBoot核心开发.md) 登录配套。
**本章用到的地方**：§16 beforeEach。
...
### 5.1 `router/index.js` 逐行读（>10 行必配）

| 行号/字段 | 含义 | 改错会怎样 |
|-----------|------|------------|
| `createWebHistory()` | HTML5 History，URL 无 `#` | 改 Hash 则 URL 变 `/#/products` |
| `() => import(...)` | 懒加载，首屏不下载该页 JS | 改静态 import 首屏变大 |
| `path: '/products/:id'` | 动态段，`:id` 匹配任意字符串 | 漏 `:` 则只匹配字面量 `id` |
| `props: true` | params 自动作组件 props | false 时需 `useRoute().params` |
| `meta.requiresAuth` | 自定义元信息，守卫可读 | 漏设则 `/cart` 不鉴权 |
| `/:pathMatch(.*)*` | 404 兜底，**必须放最后** | 放前面会吞掉所有路由 |
| `scrollBehavior` | 切换路由滚到顶部 | 删掉则详情返回列表还在底部 |
| `beforeEach` 返回值 | 重定向对象替代旧 `next()` | 用 `next()` 在 VR4 已不推荐 |
| `query: { redirect }` | 登录后回跳原 URL | 漏则登录总回首页 |

### 5.2 安装 Router 手把手步骤表

| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | `npm install vue-router@4` | package.json 有依赖 | 在 shop-vue 根目录执行 |
| 2 | 新建 `src/router/index.js` | 文件存在 | 路径拼写 |
| 3 | main.js `app.use(router)` | 启动无报错 | import 路径错 |
| 4 | App.vue 加 `<router-view />` | 切换路由有内容 | 忘记 router-view |
| 5 | 访问 `/products` | 显示 ProductList | routes 未配置 |
| 6 | 访问 `/xxx` | 404 页 | 兜底路由缺失 |
| 7 | `npm run dev` 控制台 | 无 Router warn | 见 §25 报错表 |
...
## 24.5 DevTools 调试路由（步骤表）

| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | `npm run dev`，装 Vue DevTools 扩展 | F12 有 **Vue** 面板 | 用 Chrome/Edge；刷新页面 |
| 2 | 点导航「商品」 | Vue → **Router** 里 `path: '/products'` | App.vue 缺 `<router-view>` |
| 3 | 点某商品「详情」 | `params.id` 有值；`name: 'product-detail'` | ProductCard 的 `:to` 路径错 |
| 4 | 看导航链接 DOM | 当前项有 `router-link-active` class | scoped CSS 覆盖 active 样式 |
| 5 | 未登录访问 `/cart` | Router 记录先到 login 再 redirect | `meta.requiresAuth` 未设 |
| 6 | 登录成功回跳 | 最终 `path: '/cart'` | LoginView 未读 `query.redirect` |
| 7 | 访问 `/not-exist` | `name: 'not-found'` | 404 路由不在 routes 末尾 |

**与 08 章 Network 配合**：路由切到 `/products` 后，Network 应出现 `GET /api/users`（经 Vite 代理）；路由只负责换页，数据请求在 08 章 Axios 完成。

**后端对照**：详情页 `params.id` 将来对应 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) `GET /api/products/{id}` 或本章模拟的 `/api/users/{id}`。

### 24.6 与 Java 04/06/07 路由—接口对照表

| 前端路由 | 页面职责 | 后端接口（Java 路线） | 数据层 |
|----------|----------|----------------------|--------|
| `/` | 首页入口 | 可选 `GET /api/products?limit=8` | [Java 06 product 表](../../后端学习/Java/06-MySQL基础索引与事务.md) |
| `/products` | 商品列表 | `GET /api/users`（08 模拟）→ `GET /api/products` | List / 分页 |
| `/products/:id` | 商品详情 | `GET /api/users/{id}` → `GET /api/products/{id}` | 主键查询 |
| `/login` | 登录 | `POST /api/login` | [Java 04 LoginController](../../后端学习/Java/04-SpringBoot核心开发.md) |
| `/cart` | 购物车（需登录） | MVP 本地 Pinia；进阶 `GET/POST /api/cart` | [Java 06 order 相关表](../../后端学习/Java/06-MySQL基础索引与事务.md) |

**守卫与 token**：06 章 `meta.requiresAuth` + 07 章 `userStore.isLoggedIn`；token 校验逻辑与 [Java 07](../../后端学习/Java/07-Redis核心原理与缓存实战.md) 服务端 session 可并存（JWT 无状态 + Redis 黑名单进阶）。

### 24.7 shop-vue 06 章完成自检

```text
☐ src/router/index.js 含 6+ 路由（含 404 兜底）
☐ 懒加载 () => import('../views/...')
☐ App.vue 有 <router-view />，AppHeader 用 router-link
☐ /cart 需登录，login 带 redirect 回跳
☐ DevTools Router 面板可查看当前 route
☐ 读过 Java 04：08 章 Axios 按路由页调 REST
```
...
可以但不推荐阻塞导航太久。常见做法：守卫只检查本地 token，异步权限在组件 `onMounted` 里拉取（见 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) 接口）。
...
`main.js` 里顺序一般不影响，但守卫里用 Pinia store 时，必须 `app.use(createPinia())` 在 `app.mount` 之前（07 章）。

### Q7：`<a href="/products">` 和 `<router-link>` 有何区别？

`<a href>` 触发整页刷新，SPA 重新初始化、Pinia 内存丢失；`<router-link>` 客户端导航，只换 `<router-view>` 内容。

### Q8：History 模式为什么部署后要配 Nginx？

服务器按 URL 找物理文件，`/cart` 没有 `cart.html` 会 404；需 `try_files $uri /index.html` 把请求交给 Vue（10 章详解）。

### Q9：从 `/products/1` 跳到 `/products/2` 为什么数据不刷新？

同一组件实例被复用，`onMounted` 不再次执行；需 `watch(() => route.params.id)` 或 `onBeforeRouteUpdate`（§10.3）。

### Q10：query 和 params 怎么选？

params 是路径一部分（RESTful id）；query 是可选筛选（`?page=1&keyword=vue`）。列表搜索、分页用 query；详情 id 用 params。

### Q11：路由 meta 能放什么？

任意自定义字段：`title`、`requiresAuth`、`roles`、`keepAlive`。守卫和组件通过 `route.meta.xxx` 读取。

### Q12：06 章路由和 Java 后端 URL 如何对齐？

前端 `/products/:id` ↔ 后端 `@GetMapping("/api/products/{id}")`（[Java 04](../../后端学习/Java/04-SpringBoot核心开发.md)）；08 章 Axios 按 id 调接口，06 章只管 URL 与组件映射。
...
1. SPA 和传统多页（MPA）在「换页时浏览器行为」上有何区别？
2. `<router-view>` 和 `<router-link>` 各负责什么？
3. 动态路由 `:id` 和 query `?page=1` 分别适合什么场景？
4. `beforeEach` 里 `return { name: 'login', query: { redirect } }` 做了什么？
5. 为什么 404 路由要写在 `routes` 数组最后？
6. History 模式与 Hash 模式 URL 长什么样？生产更推荐哪种？
7. 同一详情组件从 id=1 切到 id=2，为什么要 watch params？
8. **动手**：给 `/products` 加 `meta: { title: '商品列表' }`，守卫里改 `document.title`。
9. **动手**：DevTools Router 面板确认访问 `/cart` 未登录时 route 先到 login。
10. **综合**：描述 06→07→08 三章协作：守卫鉴权 → Pinia token → Axios 带 Bearer 调 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md)。

### 28.1 自测参考答案

1. SPA 只替换 `<router-view>` 区域不整页刷新；MPA 每次整页请求新 HTML。
2. `router-view` 渲染当前匹配组件；`router-link` 声明式客户端导航。
3. `:id` 用于 RESTful 资源标识；query 用于可选筛选、分页、搜索词。
4. 中断原导航，改跳登录页，并把原目标 URL 存 query 供登录后回跳。
5. `/:pathMatch(.*)*` 会匹配一切路径，放前面会拦截正常路由。
6. History `/products/1`；Hash `/#/products/1`；生产推荐 History + Nginx fallback。
7. 组件实例复用，`onMounted` 不重复执行，需监听 id 变化重新拉数据。
8. `router.beforeEach((to)=>{ document.title=to.meta.title||'shop-vue' })`。
9. 清 token → 访问 `/cart` → Router 面板 path 应为 `/login`，query 含 redirect。
10. 06 守卫拦未登录 → 07 Pinia 存 token → 08 拦截器读 token 调 `/api/login`、`/api/users`。

---

## 29. 费曼检验

请用 3 分钟向没学过编程的朋友解释 Vue Router：

1. **地址栏 = 遥控器**：输入 `/products` 就像按「商品区」按钮，Vue 只换屏幕中间内容，整站不重启。
2. **保安在门口**：进购物车要先登录——守卫检查 token，没有就带去登录页，登完送回原来想去的地方。
3. **和后端配对**：网址里的商品编号，将来就是问 Java 服务器「给我 id=2 的商品」用的（08 章）。
