...
> **读者假设**：04 章已拆分组件，但 `useCart`、`filteredProducts` 逻辑仍散落。本章系统学 **Composition API、生命周期、composables**，把逻辑抽到 `useProducts.js`、`useCart.js`。

### 0.1 用一句话弄懂本章

**一句话**：`<script setup>` 里用 `ref/computed/watch/onMounted` 组织逻辑；重复逻辑抽到 `composables/useXxx.js`，组件只负责拼 UI——这是 Vue 3 的默认工程风格。

**生活类比**：

| 概念 | 类比 |
|------|------|
| **script setup** | 开放式厨房，按功能分区操作 |
| **composable** | 预制菜包：useCart 到处热一下就能用 |
| **onMounted** | 开业铃声：DOM 好了再调接口 |
| **Options API** | 老式厨房：data/methods 分房间（Vue 2 风） |

**为什么重要**：05 章结构是 08 章 Axios 调 [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) 的标准姿势——`useProducts` 里 `onMounted` fetch，组件只管展示。
...
| 水平 | 建议 |
|------|------|
| 04 章 props/emit 不熟 | 先完成 [04-组件基础与组件通信](./04-组件基础与组件通信.md) |
| 不懂 ES module export | 补 [06-JS §3](../HTML%20CSS%20JS/06-JavaScript基础语法与数据类型.md) |
| 已会 04 章 | **从 §2 script setup 跟做 §6～§9** |
...
- [ ] 熟练使用 `<script setup>` 与 defineProps/defineEmits/defineExpose
- [ ] 使用 onMounted、onUnmounted 及清理副作用
- [ ] 编写 composables/useProducts.js、useCart.js 并复用
- [ ] 理解 composable 单例（模块级 ref）vs 实例隔离
- [ ] 配置 Vite `@` 路径别名
- [ ] DevTools 查看 composable 返回的 ref 状态
- [ ] 闭卷自测 ≥ 8/10
...
| 阶段 | 时间 |
|------|------|
| script setup + 生命周期 §2～§4 | 2 小时 |
| useProducts + useCart §6～§9 | 3 小时 |
| 对照 Options API §12 | 1 小时 |
| 自测 | 45 分钟 |
...
1. `src/composables/useCart.js` 存在，App 与多个组件共享 cartCount。
2. `useProducts` 返回 products、loading（08 章接 Java 04 API 时只改此文件）。
3. `@/composables/useCart` import 无路径报错。

---

### 0.6 核心术语三件套

**术语（Composition API 组合式 API）**：用 `setup` / `<script setup>` + 组合函数组织逻辑，替代 Options 的 data/methods 分块。
**生活类比**：开放式厨房按功能分区——切配、炒制、装盘各一块，而不是按「data 抽屉」「methods 抽屉」找东西。
**为什么重要**：Vue 3 默认写法；composable 是大型项目复用逻辑的标准方式。
**本章用到的地方**：全文；§2 script setup。

**术语（composable 组合式函数）**：以 `use` 开头的函数，内部用 Vue API，return 暴露 state 与方法。
**生活类比**：预制菜包——`useCart()` 热一下就能在多个组件用同一套购物车逻辑。
**为什么重要**：05 章 `useProducts` 08 章接 Java 04 API 只改这一处。
**本章用到的地方**：§5～§9 useProducts/useCart。

**术语（onMounted 生命周期）**：组件挂载到 DOM 后执行的钩子，适合调接口、操作 DOM。
**生活类比**：店铺开业铃声——装修完（DOM 就绪）才迎客、上货。
**为什么重要**：浏览器环境 fetch 产品列表的标准位置；setup 顶层直接 fetch 在 SSR 会双跑。
**本章用到的地方**：§4、§16 异步 composable。
...
### 7.1 useCart 逐行读（单例 composable）

| 行号/代码 | 含义 | 改错会怎样 |
|-----------|------|------------|
| `const items = ref([])` 模块顶层 | 全应用共享一份购物车 | 放函数内则每次 useCart 独立 |
| `computed totalCount` | 派生总件数 | 写 method 每次渲染都重算 |
| `items.value.find(i => i.id === product.id)` | 合并同商品 qty | 08 章与后端 cart 表 id 对齐 |
| `exist.qty++` | 改 reactive 数组元素 | 应用 ref 包数组，改 `.value` 内对象 |
| `items.value.filter(...)` 删除 | 新数组引用触发更新 | 仅改属性有时也可，filter 更直观 |
| `return { items, add, ... }` | 暴露给组件的 API | 漏 return 模板 undefined |

**08 章接 Java 04**：`add` 后可 `POST /api/cart`；`useProducts` 的 `onMounted` 请求 [Java 04 `/api/products`](../../后端学习/Java/04-SpringBoot核心开发.md)——组件仍只 `import { useCart } from '@/composables/useCart'`。
...
## 15.1 DevTools 调试 composable 状态（步骤表）

| 步骤 | 动作 | 预期 | 若不对 |
|------|------|------|--------|
| 1 | 完成 §9 重构 App | useCart/useProducts 已接入 | — |
| 2 | F12 → Vue → App | setup 里见 cartItems、products | composable 是否 return |
| 3 | 加购 | cartCount 或 cartItems 变 | useCart 单例 ref |
| 4 | 刷新页面（未 persist） | 购物车清空 | 07 Pinia persist |
| 5 | 08 章预览 | onMounted fetch 后 products 变 | Network 见 Java 04 API |

**08 章衔接**：在 `useProducts.js` 的 `onMounted` 里请求 `/api/products`，后端见 [Java 04 Spring Boot 核心开发](../../后端学习/Java/04-SpringBoot核心开发.md)；DevTools + Network 同时验证。
...
**Q7：composable 文件名为什么 use 开头？**  
社区约定，一眼看出是组合式函数；如 `useCart.js`。

**Q8：useProducts 里 mock 数据何时换成 API？**  
08 章把 mock 换成 `onMounted` + axios；URL 与 [Java 04 Controller](../../后端学习/Java/04-SpringBoot核心开发.md) 一致即可。

**Q9：onUnmounted 必须写吗？**  
有 setInterval、事件监听、watch 副作用时要在 onUnmounted 清理，防内存泄漏。

**Q10：为什么 composable 要 return 对象？**  
解构仍保持 ref 响应式；return 列表即组件可用的 API。

**Q11：jsconfig paths 和 vite alias 都要配吗？**  
Vite alias 负责打包解析；jsconfig 让 VS Code 跳转和补全；05 章 §8 两个都配。

**Q12：Options API 还要学吗？**  
维护老项目要**能读**；新代码用 script setup + composable。
...
1. 组合式 API 相对 Options API 的主要优势是什么？
2. `<script setup>` 里为什么没有 `this`？
3. onMounted 和 setup 顶层写 fetch 各有什么注意点？
4. composable 模块级 ref 为什么能实现购物车跨组件共享？
5. defineExpose 的使用场景？
6. `@/` 别名需要改哪两个配置文件？
7. composable 和 Pinia 如何选型？
8. **动手**：新建 `useTheme.js` 返回 darkMode ref 和 toggle 方法。
9. **动手**：DevTools 确认 useCart 加购后 cartItems 长度变。
10. **综合**：描述 08 章 useProducts 如何从 Java 04 拉列表（onMounted → axios → ref）。

### 26.1 自测参考答案

1. 逻辑按功能聚合、易复用、TS 友好、无 mixin 命名冲突。
2. script setup 编译为 setup() 作用域，无组件实例 this。
3. onMounted 保证 DOM 存在、适合浏览器 API；setup 顶层 SSR 会双跑需注意。
4. 模块只 evaluate 一次，模块级 ref 是单例，多处 useCart() 共享同一 ref。
5. 父组件 template ref 调用子组件内部方法/属性。
6. vite.config.js alias + jsconfig.json paths。
7. 页面内/小项目 composable；多路由共享+DevTools 用 Pinia。
8. `const dark=ref(false); const toggle=()=>dark.value=!dark.value; return {dark,toggle}`。
9. 加购 → F12 Vue → cartItems.length 增加。
10. onMounted → axios.get('/api/products') → 解析 Result → products.value = data → 组件 v-for。
...
1. **script setup**：逻辑和模板在一个文件，但按功能写 ref/函数，不按 data/methods 分抽屉。
2. **composable**：把 useCart 抽成 JS 文件，多个组件 import 同一份购物车。
3. **接后端**：请求写 composable 的 onMounted，UI 组件只管展示——08 章连 Java 04。
...
| 级别 | 任务 | 验收 |
|------|------|------|
| 基础 | 抽 `useTheme.js` darkMode toggle | 多组件共享 |
| 进阶 | useProducts 加 loading/error ref | 模板三态展示 |
| 挑战 | onUnmounted 清理 setInterval | 无泄漏警告 |
| 联调 | useProducts.onMounted fetch Java 04 mock | CORS 08 章配 vite proxy |

### 27.2 Options API 与 Composition 对照（扩展）

| Options API | Composition + script setup |
|-------------|------------------------------|
| `data()` | `ref` / `reactive` |
...
### 27.3 shop-vue 05 章完成自检

```text
☐ src/composables/useProducts.js 与 useCart.js 存在
☐ App.vue import composable，不再内联大段 filter/cart 逻辑
☐ useCart 模块级 items，顶栏与列表共享 cartCount
☐ vite.config.js 与 jsconfig.json 配置 @ 别名
☐ onMounted 仅在需要 DOM/接口时使用
☐ 读过 Java 04：08 章将在 useProducts 内 axios 拉 /api/products
```
