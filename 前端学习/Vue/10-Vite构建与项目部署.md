...
**一句话**：`npm run build` 把 Vue 源码打成静态文件；Nginx 托管 `dist` 并反代 `/api` 到 Spring Boot——用户通过域名访问，无需 CORS（[计网 06 §11.3](../计算机网络/06-缓存Cookie与会话机制.md)）。
...
| 06 章 History 路由 | 部署必须 fallback，见 [06 §18](./06-Vue-Router路由管理.md) |
| Linux/Nginx 零基础 | 并行 [Java 09](../../后端学习/Java/09-LinuxDockerNginx部署基础.md) |

**后端衔接**：[Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) 8080 接口；[Java 06 MySQL](../../后端学习/Java/06-MySQL基础索引与事务.md) 生产数据；[Java 07 Redis](../../后端学习/Java/07-Redis核心原理与缓存实战.md) 可选缓存。
...
2. 本地 Nginx 或 preview 下刷新 `/products/1` 不 404。
...
### 5.1.1 Nginx `try_files` 逐行读

| 配置片段 | 含义 | 改错会怎样 |
|----------|------|------------|
| `root .../shop` | 静态文件根目录 = dist 上传位置 | root 错 → JS/CSS 404 白屏 |
| `try_files $uri` | 先找与 URL 同名的物理文件 | — |
| `$uri/` | 再找同名目录 | — |
| `/index.html` | 都没有则返回 SPA 入口 | **漏则刷新 /cart 404** |
| `location /api/` | API 与静态分流 | 与静态混 location 易冲突 |
| `proxy_pass http://127.0.0.1:8080` | 反代 Spring Boot | 端口错 → 502 |

**与 [Java 09 Nginx](../../后端学习/Java/09-LinuxDockerNginx部署基础.md)、[计网 06 同域部署](../计算机网络/06-缓存Cookie与会话机制.md) 一致。**
...
**Q：VITE_ 前缀有什么用？**  
只有 `VITE_` 开头变量会暴露给客户端 `import.meta.env`；密钥勿放前端 env。

**Q：build 后 API 还连 localhost 怎么办？**  
`.env.production` 应 `VITE_API_BASE_URL=/api`，重新 build；Nginx 反代同域。

**Q：preview 端口和 dev 不同？**  
preview 默认 4173，Serving 已打包 dist，更接近生产。

**Q：manualChunks 做什么？**  
把 element-plus、vue 等拆成独立 chunk，利于浏览器缓存。

**Q：子路径 `/shop/` 部署要注意什么？**  
`vite.config base: '/shop/'` + Router `createWebHistory('/shop/')` + Nginx alias 三者一致。

**Q：同域部署还要 CORS 吗？**  
不要。见 [计网 06 §11.3](../计算机网络/06-缓存Cookie与会话机制.md)；跨域才需 [Java 04 CorsConfig](../../后端学习/Java/04-SpringBoot核心开发.md)。

**Q：dist 要提交 Git 吗？**  
一般不提交；CI build 产物上传服务器或 OSS。

---

## 17.1 部署验收 Network/DevTools 步骤表

| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | `npm run build && npm run preview` | 4173 可访问 | build 报错先修 |
| 2 | F12 Network 过滤 JS | `/assets/index-*.js` 200 | base 路径错 → 404 |
| 3 | 直接访问 `/cart` 刷新 | 不 404，Vue 页正常 | 缺 try_files |
| 4 | 登录 | `/api/login` 200，同域无 CORS 红字 | proxy_pass 错 |
| 5 | Application → 搜 localhost | 打包 JS 无 `:8080` 硬编码 | 改 .env.production 重 build |
| 6 | Console | 无 Mixed Content | HTTPS 页须 HTTPS API |
| 7 | 对比 dev | preview 无 HMR，行为接近生产 | — |

**后端**：Spring Boot 8080 由 Nginx 反代；数据来自 [Java 06 MySQL](../../后端学习/Java/06-MySQL基础索引与事务.md)（若已接库）。

### 17.2 开发 vs 生产对照（与 08 章、[计网 06](../计算机网络/06-缓存Cookie与会话机制.md)）

| 维度 | 开发 `npm run dev` | 生产 Nginx |
|------|-------------------|------------|
| 前端端口 | 5173 | 80/443 |
| API 路径 | Vite proxy `/api` | Nginx `location /api` proxy_pass |
| 跨域 | 无（同源 5173） | 无（同域） |
| CORS | 通常不配 | 通常不配 |
| 环境变量 | `.env.development` | `.env.production` build 时注入 |
| 路由 fallback | Vite 内置 SPA | `try_files ... /index.html` |

### 17.3 shop-vue 10 章完成自检

```text
☐ npm run build 成功，dist/ 含 index.html + assets
☐ .env.production VITE_API_BASE_URL=/api
☐ npm run preview 或 Nginx 刷新 /products/1 不 404
☐ Network 无 localhost:8080 硬编码
☐ /api/login 同域 200
☐ 对照 Java 09 Nginx 配置可复用
```
...
1. Vite dev 为什么比 Webpack 启动快？
2. `npm run build` 产出什么？存在哪？
3. SPA History 模式刷新 404 的根因与 Nginx 解法？
4. `.env.production` 何时生效？能 build 后改吗？
5. `VITE_API_BASE_URL=/api` 在生产意味什么？
6. try_files 最后一项 `/index.html` 做什么？
7. gzip 对什么资源有效？
8. **动手**：build 后在 preview 完成登录流程。
9. **动手**：Network 确认无 localhost:8080 硬编码。
10. **综合**：对比 dev proxy 与生产 Nginx 反代（[计网 06](../计算机网络/06-缓存Cookie与会话机制.md)）。

### 18.1 自测参考答案

1. dev 用 ESM 按需编译；Webpack 常先全量打包再启动。
2. 静态 dist：index.html + assets/*.js/css（带 hash）。
3. 服务器找物理 `/cart` 文件不存在；`try_files $uri $uri/ /index.html`。
4. build 时注入；不能 build 后改，须重 build。
5. 浏览器请求同域 `/api/xxx`，Nginx 转发后端。
6.  fallback 到 SPA 入口，由 Vue Router 接管路径。
7. text/js/css/json 等文本；图片已压缩格式收益小。
8. preview 4173 登录→列表→加购无 Console 红字。
9. Sources/Network 搜 `:8080` 应无匹配（除注释）。
10. dev：Vite 5173 转发；生产：Nginx 80 同域 /api → 8080，均无浏览器 CORS。
...
1. **build = 打包成品**：dev 是装修现场，用户只能看成品 dist。
2. **Nginx = 大门 + 收发室**：静态页直接给；/api 转 Spring Boot（[Java 09](../../后端学习/Java/09-LinuxDockerNginx部署基础.md)）。
3. **try_files = 找不到房间就回大厅**：SPA 路由靠 index.html，否则刷新 404。
