...
<!-- 修改说明: 2026-06-30 按 EXPANSION-STANDARD 扩充 §0 导读、store 逐行读、DevTools、FAQ 12 题、闭卷自测、费曼检验 -->

## 0. 读前导读（零基础也能跟上）

> **读者假设**：06 章已有多页面路由，但登录态、购物车数据散落在各组件和 localStorage。本章用 **Pinia** 集中管理「跨页面共享状态」。

### 0.1 用一句话弄懂本章

**一句话**：Pinia 是 Vue 3 的**全局仓库**——登录 token、购物车 items 存进 store，任意页面 `useUserStore()` / `useCartStore()` 读写，改一处全站同步。

**生活类比**：

| 概念 | 类比 |
|------|------|
| **Store** | 商场中央收银台总账：各柜台（页面）都查同一本账 |
| **state** | 账本上写的数字：token、购物车件数 |
| **getters** | 账本上的小计：totalCount、isLoggedIn |
| **actions** | 收银员操作：setLogin、add、logout |

**为什么重要**：08 章 Axios 拦截器从 userStore 读 token 调 [Java 04 登录接口](../../后端学习/Java/04-SpringBoot核心开发.md)；06 章路由守卫从 store 读 `isLoggedIn` 替代裸 localStorage。
...
| 水平 | 建议 |
|------|------|
| 06 章路由/守卫不熟 | 先完成 [06-Vue-Router路由管理](./06-Vue-Router路由管理.md) |
| ref/computed 不熟 | 回 [02-模板语法与响应式原理](./02-模板语法与响应式原理.md) |
| 已会 06 章 | **从 §3 安装 Pinia 跟做 §5～§12** |

**后端衔接**：

| Java 章 | 与 Pinia 的关系 |
|---------|----------------|
| [04 Spring Boot](../../后端学习/Java/04-SpringBoot核心开发.md) | `POST /api/login` 返回 token → `userStore.setLogin` |
| [06 MySQL](../../后端学习/Java/06-MySQL基础索引与事务.md) | 购物车/订单持久化到库；07 章先 localStorage，11 章可同步服务端 |
| [07 Redis](../../后端学习/Java/07-Redis核心原理与缓存实战.md) | 后端 session/token 存 Redis；前端 store 存 access token 副本 |

---

### 0.3 本章知识地图（☐→☑）

- [ ] 理解何时用 Pinia vs props vs composable
- [ ] 用 `defineStore` 定义 Setup 风格 store
- [ ] 组件中 `useXxxStore()`、`storeToRefs` 解构
- [ ] 完成 userStore + cartStore，含 localStorage 持久化
- [ ] 路由守卫从 Pinia 读 `isLoggedIn`
- [ ] DevTools Pinia 面板查看 state 变化
- [ ] 闭卷自测 ≥ 8/10
...
|------|------|
| 概念 + 安装 §1～§4 | 1 小时 |
| userStore + cartStore §5～§11 | 2.5 小时 |
| 守卫联动 + 生产案例 §12～§19 | 1.5 小时 |
| DevTools + 自测 | 45 分钟 |
...
1. 登录后导航栏显示用户名，刷新仍保持（localStorage hydrate）。
2. 商品页加购，顶栏角标实时 +1，购物车页同步。
3. DevTools → Pinia → user/cart store 可见 state 变化。
4. 未登录访问 `/cart` 被守卫拦截（读 `userStore.isLoggedIn`）。

---

### 0.6 核心术语三件套

**术语（Pinia）**：Vue 3 官方状态管理库，替代 Vuex，无 mutations，API 更直观。
**生活类比**：全商场共用的中央账本——谁改库存，所有柜台立刻看到。
**为什么重要**：跨路由共享 token、购物车；08 章拦截器与 06 章守卫的标准数据源。
**本章用到的地方**：全文。

**术语（storeToRefs）**：从 store 解构 state/getters 时保持响应式的工具函数。
**生活类比**：从账本复印「当前页」——直接撕页（解构 ref）会断线，storeToRefs 保持连线。
**为什么重要**：模板里用 `totalCount` 必须响应式，否则 UI 不更新。
**本章用到的地方**：§8.2、§9 AppHeader。

**术语（Setup Store）**：用 `defineStore(id, () => { ref... return })` 定义，与 Composition API 一致。
**生活类比**：和 script setup 同一套语言——ref、computed、function 写在一起。
**为什么重要**：Vue 3 新项目默认写法；与 05 章 composable 思维连续。
**本章用到的地方**：§4、§5、§6。
...
| 行号/字段 | 含义 | 改错会怎样 |
|-----------|------|------------|
| `TOKEN_KEY` 常量 | localStorage 键名统一 | 多处硬编码字符串易拼错 |
| `ref(localStorage.getItem(...))` | 启动时从磁盘恢复 token | 漏则刷新后 isLoggedIn 变 false |
| `computed(() => !!token.value)` | 有 token 即已登录 | 用 `token` Truthy 可能把空串当 true |
| `setLogin(payload)` | 同时写 ref + localStorage | 只改 ref 则刷新丢失 |
| `logout()` | 清内存 + 清 storage | 只清 ref 则守卫仍读旧 token |
| `fetchProfile` | 08 章用 token 拉用户信息 | 占位即可，08 章实现 |

**与 Java 04 对齐**：`setLogin` 的 `payload.token` 来自 `POST /api/login` 响应 `data.token`（见 [Java 04 §49 JWT](../../后端学习/Java/04-SpringBoot核心开发.md)）。
...
- 后端 [Java 07 Redis](../../后端学习/Java/07-Redis核心原理与缓存实战.md) 存 session 时，前端仍可能持有一份 access token 副本
...
| 行号/字段 | 含义 | 改错会怎样 |
|-----------|------|------------|
| `loadCartFromStorage()` | 启动 hydrate 购物车 | 漏则刷新后 cart 空 |
| `try/catch JSON.parse` | 防止 localStorage 被手改坏 | 无则 parse 抛错白屏 |
| `exist.qty++` | 同商品合并数量 | 改 push 重复行 |
| `persist()` | 每次 mutation 后写盘 | 漏则刷新丢加购 |
| `updateQty(..., 0)` | qty≤0 走 remove | 漏则出现 0 元商品 |
| `totalPrice` computed | 响应式合计 | 放普通变量 UI 不更新 |

**11 章进阶**：登录后 `syncFromServer()` 从 [Java 06 MySQL](../../后端学习/Java/06-MySQL基础索引与事务.md) 拉购物车，与本地 merge（§18）。
...
### 16.1 DevTools + Application 联调步骤表

| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | 登录 admin | Pinia **user** → token 有值 | setLogin 未调用 |
| 2 | Application → Local Storage | `shop_token`、`shop_username` 存在 | KEY 常量与写入不一致 |
| 3 | 加购一件商品 | Pinia **cart** → items 长度 +1 | 未走 cartStore.add |
| 4 | AppHeader 角标 | 与 cart.totalCount 一致 | 未用 storeToRefs |
| 5 | F5 刷新 | user/cart state 从 storage 恢复 | 未 hydrate 或 persist |
| 6 | 退出登录 | user token 空；cart 可选清空 | logout 未清 storage |
| 7 | 未登录访问 /cart | 守卫 redirect login | 守卫仍读 localStorage 而非 store |

**08 章预览**：登录后 Network 请求带 `Authorization`，Pinia user.token 应与 Header 一致（[Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) 鉴权）。

### 16.2 Pinia 与 Java 后端职责分界

| 层级 | 前端 Pinia | 后端 Java |
|------|------------|-----------|
| 登录态 | token、username 副本 | [Java 04](../../后端学习/Java/04-SpringBoot核心开发.md) 签发 JWT/UUID |
| 购物车 MVP | items + localStorage | 11 章可落 [Java 06 MySQL](../../后端学习/Java/06-MySQL基础索引与事务.md) |
| 会话失效 | logout + 跳登录 | [Java 07 Redis](../../后端学习/Java/07-Redis核心原理与缓存实战.md) TTL / 黑名单 |
| 列表缓存 | 一般不放 store | Redis 缓存热点商品（07 章后端） |

### 16.3 shop-vue 07 章完成自检

```text
☐ main.js app.use(pinia) 在 mount 前
☐ userStore + cartStore 含 persist
☐ AppHeader 显示 displayName、cart 角标
☐ 路由守卫读 userStore.isLoggedIn
☐ DevTools Pinia 可见 state 变化
☐ 08 章将在拦截器读 userStore.token
```
...
有 XSS 风险。进阶方案：httpOnly Cookie + SameSite、短 token + refresh token。初学阶段先理解流程；后端 [Java 07 Redis](../../后端学习/Java/07-Redis核心原理与缓存实战.md) 可存 session。

### Q7：Setup Store 和 Options Store 怎么选？

新项目一律 Setup Store，与 `<script setup>` 一致。维护老 Vuex 项目可能见到 Options Store。

### Q8：cartStore 为什么要手写 persist？

初学理解「内存 + 磁盘」两层；生产可用 `pinia-plugin-persistedstate`（§15）。

### Q9：守卫里 useUserStore 报错 getActivePinia？

`app.use(pinia)` 必须在 `mount` 前；导航时 app 已挂载，一般正常。若在独立 JS 文件顶层调用 store 会报错。

### Q10：Pinia 和 [Java 06 MySQL](../../后端学习/Java/06-MySQL基础索引与事务.md) 购物车如何同步？

MVP 购物车存 localStorage；登录后可 `GET /api/cart` 拉服务端（11 章）；写库由后端负责，前端只调 API。

### Q11：401 时为什么要 store.logout？

清 token 后 isLoggedIn 变 false，守卫与 UI 自动同步；见 §17 与 08 章响应拦截器。

### Q12：05 章 useCart 和 07 章 cartStore 关系？

05 composable 是台阶；多页面 + DevTools + 持久化应升级 Pinia。08 章加购后可能再调 [Java 04 POST](../../后端学习/Java/04-SpringBoot核心开发.md) 接口。
...
1. 什么场景该用 Pinia 而不是 props 或 composable？
2. 为什么不能直接解构 `const { totalCount } = useCartStore()`？
3. `storeToRefs` 和直接解构 actions 有何区别？
4. userStore 为何要在 setLogin 里写 localStorage？
5. 路由守卫如何从 Pinia 判断登录态？
6. Setup Store 与 Options Store 结构有何不同？
7. 两个 tab 页签 logout 如何同步？（提示：storage 事件）
8. **动手**：DevTools Pinia 面板确认加购后 cart.items 变化。
9. **动手**：刷新页面后 token 与购物车仍保留。
10. **综合**：从登录到带 token 请求，Pinia 与 08 章 Axios 拦截器如何协作？

### 26.1 自测参考答案

1. 跨路由、跨多层组件共享的业务态：用户、购物车、权限。
2. 直接解构 state/getters 丢响应式，UI 不更新。
3. state/getters 需 storeToRefs；actions 是普通函数可直接解构。
4. 刷新 Pinia 内存清空，localStorage 可 hydrate 恢复登录态。
5. `const userStore = useUserStore(); if (!userStore.isLoggedIn) redirect login`。
6. Setup 用 ref/computed/函数 return；Options 用 state/getters/actions 选项。
7. 监听 `window storage` 事件，其他 tab 清 token 时本 tab logout。
8. 加购 → F12 Vue → Pinia → cart → items 数组多一项。
9. setLogin/persist 正确则 F5 后 user.token、cart.items 仍在。
10. login API 返回 token → setLogin → 请求拦截器读 userStore.token 加 Bearer。
...
1. **中央账本**：登录名、购物车不再每个页面各记各的，全站查同一 store。
2. **刷新不丢**：重要数据写 localStorage，开页时读回来灌进 store。
3. **保安认账**：路由守卫问 userStore「登录了吗？」，08 章快递员（Axios）贴 token 单。
