...
<!-- 修改说明: 2026-06-30 按 EXPANSION-STANDARD 扩充 §0 导读（盒模型=快递盒）、DevTools Box Model overlay、FAQ≥12、闭卷自测、费曼检验、衔接 Vue 09 -->

## 0. 读前导读（零基础也能跟上）

> **读者假设**：你已读过 [03-CSS 选择器与文本样式](./03-CSS基础语法选择器与文本样式.md)，能给元素加颜色、边框、内外边距。本章解决「元素到底占多大、为什么挤在一起、怎么叠在页面上」——这是写任何页面布局前的必修课。

### 0.1 用一句话弄懂本章

**一句话**：每个 HTML 元素都是一个**盒子**；搞清盒模型四层（content / padding / border / margin）、显示模式、浮动与定位，你就能解释「为什么宽了 50px」「为什么塌了」「角标怎么贴右上角」。

**生活类比——盒模型 = 快递盒**：

| CSS 概念 | 快递盒类比 | 本章位置 |
|----------|------------|----------|
| **content 内容区** | 盒子里的商品本身 | §2～§3 |
| **padding 内边距** | 商品与纸箱壁之间的气泡膜 | §4 |
| **border 边框** | 纸箱硬壳，看得见也占地方 | §5 |
| **margin 外边距** | 两个快递箱之间的空隙 | §6 |
| **box-sizing** | 「标 200g」指净重还是连箱带膜一起称 | §8 |
| **BFC** | 独立打包区——里面的 margin 不跟外面混 | §10 |
| **float 浮动** | 杂志排版：图片靠左，文字绕开 | §18 |
| **position 定位** | 在货架上贴标签：相对原处挪 / 钉在角落 / 粘在视口 | §22～§27 |
| **z-index** | 谁叠在谁上面——像一摞透明玻璃板 | §28 |

**为什么重要**：Vue [09-Element Plus 布局](../Vue/09-Element-Plus与UI工程化.md) 里的 `el-dialog` 遮罩、`el-badge` 角标、`position: fixed` 顶栏，底层全是本章概念；不懂盒模型，改 Element 组件样式会越改越乱。
...
| 水平 | 建议 |
|------|------|
| 刚学完 CSS 03 | 从 §2 盒模型图解开始，每个示例用 DevTools Box Model 对照 |
| 只会写 `margin` 硬推间距 | 重点 §6 margin 合并、§10 BFC、§31 排查清单 |
| 已会 Flex 一点 | 本章 float 了解即可；定位 §22～§28 必须扎实 |
| 目标 Vue 路线 | 本章 + [05 Flex/Grid](./05-CSS布局FlexGrid响应式与动画.md) 后再开 [Vue 01](../Vue/01-Vue入门与环境搭建.md) |

---

### 0.3 本章知识地图（☐→☑）

- [ ] 能画 content / padding / border / margin 四层，区分 content-box 与 border-box
- [ ] 会心算总宽度，全局使用 `box-sizing: border-box`
- [ ] 能解释垂直 margin 合并的 3 种情况及解决办法
- [ ] 知道 BFC 是什么，会用 `display: flow-root` 包住浮动
- [ ] 区分 block / inline / inline-block 及 `display: none` vs `visibility`
- [ ] 会写 relative + absolute 角标、fixed 按钮、居中弹窗
- [ ] 理解 z-index 与层叠上下文，不盲目写 `99999`
- [ ] 会用 DevTools Box Model overlay 调试尺寸与间距
- [ ] 独立完成 §32 角标卡片 + FAB + 弹窗实战
- [ ] 闭卷自测 ≥ 8/10
...
| 阶段 | 时间 |
|------|------|
| 盒模型 + box-sizing §2～§8 | 2 小时 |
| margin 合并 + BFC §9～§10 | 1.5 小时 |
| 显示模式 + overflow §11～§17 | 1 小时 |
| 浮动 + 清除浮动 §18～§21 | 1.5 小时 |
| 定位 + z-index §22～§28 | 2 小时 |
| DevTools 实操 + §32 实战 + 自测 | 1.5 小时 |

---

### 0.5 可验证成果

1. 在 DevTools 选中任意卡片，读出 content / padding / border / margin 四层像素值。
2. 独立写出带 `position: absolute` 角标的商品卡片（父级 `relative`）。
3. 实现右下角 `fixed` 按钮 + 更高 `z-index` 的居中弹窗。
4. 向同学解释：两个 `div` 上下各 `margin: 30px` 和 `20px`，实际间距是多少、为什么。

---

### 0.6 核心术语三件套

**术语（盒模型 Box Model）**：CSS 把每个元素看作由内容区、内边距、边框、外边距组成的矩形区域。
**生活类比**：快递盒——商品在内，气泡膜、纸箱、两箱间距各算一层。
**为什么重要**：所有宽高、间距、溢出问题的根源；[05 Flex/Grid](./05-CSS布局FlexGrid响应式与动画.md) 也是在分配盒子空间。
**本章用到的地方**：§2～§8 全文。

**术语（BFC 块级格式化上下文）**：一块独立排版区域，内外 margin 合并、浮动包含等行为与普通流不同。
**生活类比**：独立打包台——这台子上的箱子高度自己算，不会跟隔壁台子塌在一起。
**为什么重要**：解决浮动塌陷、父子 margin 穿透的经典钥匙。
**本章用到的地方**：§10；清除浮动 §21。

**术语（层叠上下文 Stacking Context）**：像一层透明玻璃板；板内比 z-index，板与板之间整块上下叠，子元素再高也盖不住更高板上的低 z-index 元素。
**生活类比**：一摞透明文件夹——先比哪本文件夹在上，再比文件夹里的便签。
**为什么重要**：`el-dialog`、`el-dropdown` 盖住普通内容全靠层级；Vue [09 §6](../Vue/09-Element-Plus与UI工程化.md) AdminLayout 的 header z-index 要低于弹窗。
**本章用到的地方**：§28；多层级弹窗 §41。
...
### 32.2 主示例逐行读（角标 + 弹窗核心 CSS）

| 行号/选择器 | 含义 | 改错会怎样 |
|-------------|------|------------|
| `* { box-sizing: border-box }` | 全局 border-box，width 含 padding/border | 去掉后 `width:280px` 卡片可能因 padding 变宽溢出 |
| `.card { position: relative }` | 为角标提供定位参照 | 角标可能相对 `body` 飞到页面左上角 |
| `.card__badge { position: absolute; top: -8px }` | 角标叠在卡片外缘 | 无 absolute 则角标挤占文档流，把标题顶下去 |
| `.fab { position: fixed; z-index: 100 }` | 钉在视口右下角 | 用 absolute 会随页面滚动消失 |
| `.modal { position: fixed; inset: 0; z-index: 1000 }` | 全屏遮罩层 | z-index 低于 FAB 时按钮会露在遮罩上面 |
| `.modal__panel { top:50%; transform:translate(-50%,-50%) }` | 在遮罩内视觉居中 | 只写 top/left 50% 不 translate 会偏到右下角 |
| `.modal { display: none }` / `.is-open` | 默认隐藏，类名切换显示 | 一直 `display:block` 则页面一打开就被遮罩挡住 |
...
### Q14：padding 能写负值吗？margin 呢？

**padding 不能为负**（无效或被当作 0）；**margin 可以为负**，用于重叠、微调，但容易破坏布局，初学慎用。间距不够优先调单侧 margin 或改用 `gap`（05 章 Flex/Grid）。

### Q15：为什么全站要加 `* { box-sizing: border-box }`？

写 `width: 100%` 或固定宽时，padding/border 不会额外撑大盒子，心算布局与响应式更简单。Element Plus 组件内部也普遍按 border-box 设计；个别老插件若依赖 content-box 需单独覆盖。
...
## 42. DevTools 盒模型 Overlay 八步实操

> 在 Chrome / Edge 中操作；Firefox 的 Layout 面板类似，Box Model 在规则面板下方。

| 步骤 | 你的动作 | 预期看到什么 | 若不对 |
|------|----------|--------------|--------|
| 1 | 用浏览器打开 §32 实战 HTML（或任意带 `.card` 的页面），按 **F12** | 开发者工具在右侧或底部打开 | 试 **Ctrl+Shift+I** 或右键「检查」 |
| 2 | 点左上角 **选择元素** 图标（箭头），再点页面上的卡片 | Elements 树高亮对应 `article.card` | 点偏了会选中子元素，向上找父级 `.card` |
| 3 | 右侧切到 **Computed**（计算后样式）面板，向下找到 **Box Model** 示意图 | 四层同心矩形：content（蓝/青）、padding、border、margin | 没有则把 Computed 面板拉宽或向下滚动 |
| 4 | 把鼠标悬停在 Box Model 的 **margin** 区域（最外圈） | 页面上该元素外围出现**橙色**半透明 overlay | 橙色 = margin；绿色 = padding；蓝色 = content |
| 5 | 点击 Box Model 里 **padding** 数字，临时改成 `30`，回车 | 卡片内容与边框距离变大，总宽可能变（content-box 时） | 若总宽不变检查是否 `border-box` |
| 6 | 在 Styles 里给 `.card` 加 `outline: 2px dashed red` | 红色虚线标出 border 外缘，便于和 margin overlay 区分 | `outline` 不占布局空间，不影响盒模型总宽 |
| 7 | 选中 `.card__badge` 角标，Computed 看 `position: absolute`；向上点父级 `.card` 看 `position: relative` | 角标相对卡片定位；改父级 `top` 角标跟着动 | 父级无 `relative` 时角标可能相对 `body` 定位 |
| 8 | 打开弹窗，选中 `.modal`，看 `z-index: 1000`；再选 `.fab` 看 `z-index: 100` | 弹窗 overlay 盖住 FAB；Layers 面板（Chrome）可见层叠顺序 | 若 FAB 在上，检查是否在不同层叠上下文 |

### Box Model overlay 颜色速记

| DevTools 高亮色 | 对应层 | 是否占背景色 |
|-----------------|--------|--------------|
| 蓝色 / 青色 | content | 是 |
| 绿色 | padding | 是 |
| 黄色（部分版本） | border | 边框线可见 |
| 橙色 | margin | 否（透明） |
...
| 症状 | 先查 |
|------|------|
| 比预期宽 40px | `padding-left/right` 各 20？`border` 宽度？`box-sizing` 值？ |
| 上下间距调不动 | 兄弟 margin 合并？试只改一侧或给父级 `flow-root` |
| 父元素高度为 0 | 子元素是否全 `float`？父级是否 BFC？ |
| 角标飞到页面左上角 | absolute 祖先谁第一个非 `static`？ |
| 弹窗盖不住按钮 | 比较**层叠上下文**根节点的 z-index，不是子元素 |

### 盒模型心算练习（动手 5 分钟）

```css
.quiz-box {
  box-sizing: content-box; /* 先 content-box，再改 border-box 对比 */
  width: 100px;
  padding: 15px;
  border: 5px solid #333;
  margin: 10px;
...
| 练习 | 你心算 | DevTools Computed 验证 |
|------|--------|------------------------|
| content-box 总宽（含 padding+border） | 100+30+10 = **140px** | Box Model 最外 border 边到边 |
| 改为 border-box 后内容区宽 | 100-30-10 = **60px** | 蓝色 content 区应变窄 |
| margin 改 30px 后总宽 | **仍 140px**（margin 在外层） | 橙色 overlay 变厚，border 边到边不变 |

---

## 43. 与 Vue 09 Element 布局的衔接

学完本章，读 [Vue 09 — Element Plus 与 UI 工程化](../Vue/09-Element-Plus与UI工程化.md) 会更顺：

| 本章概念 | Element Plus 中的体现 | Vue 09 位置 |
|----------|----------------------|-------------|
| `position: relative` + `absolute` | `el-badge` 角标、`el-image` 遮罩层定位 | §5 组件示例 |
| `position: fixed` + `z-index` | `ElMessage`、`el-dialog` 遮罩盖住页面 | §8 Dialog |
| `box-sizing: border-box` | `el-form`、`el-card` 内边距不撑破宽度 | §5～§6 |
| Flex（预习 05 章） | `el-container` / `el-header` / `el-aside` / `el-main` 后台骨架 | **§6 AdminLayout** |
| margin 合并 | 表单项间距用 `el-form-item` 的 margin 规范，少手写冲突 | §5 el-form |
| 层叠上下文 | 下拉 `el-select` 弹出层 z-index 需高于表格 | §24 响应式布局 |

**建议路径**：本章盒模型 + 定位 → [05 Flex/Grid](./05-CSS布局FlexGrid响应式与动画.md) → Vue 01～08 → **Vue 09 §6 用 `el-container` 搭 AdminLayout**，你会认出「这就是 Flex 列布局 + fixed 顶栏思维」。

---

## 44. 闭卷自测

1. 盒模型从里到外四层分别叫什么？各起什么作用？
2. `box-sizing: content-box` 与 `border-box` 下，`width: 200px` + `padding: 20px` 各表示多宽？
3. 两个块级兄弟 `margin-bottom: 30px` 与 `margin-top: 20px`，实际间距是多少？为什么？
4. 列举 3 种触发 BFC 的常用方式。
5. `display: none`、`visibility: hidden`、`opacity: 0` 在「占空间」和「可点击」上各有什么不同？
6. 浮动元素为什么会导致父元素高度塌陷？常用解决办法是什么？
7. `position: absolute` 的参照元素是谁？`fixed` 与 `absolute` 的核心区别？
8. 为什么子元素 `z-index: 9999` 仍可能被别的元素挡住？
9. **动手**：写 HTML+CSS，300px 宽卡片 `margin: 0 auto` 居中，右上角 `absolute` 角标「NEW」。
10. **动手**：用 DevTools 读出某元素 padding 四边计算后像素值，并临时改一个边看页面变化。
11. **综合**：`sticky` 标题不生效，列出至少 3 条排查项并说明原因。
12. **综合**：说明 Vue 09 的 `el-dialog` 为什么需要本章的 `fixed` + 高 `z-index` 知识。

### 44.1 自测参考答案

1. content（内容）、padding（内边距）、border（边框）、margin（外边距）；由内到外包裹，margin 隔开相邻盒子。
2. content-box：内容区 200px，总宽 200+40=240px（无 border 时）；border-box：总宽 200px，内容区缩小为 160px。
3. **30px**；垂直相邻块级 margin 合并，取较大值而非相加。
4. 示例：`display: flow-root`；`overflow: hidden/auto`；浮动/绝对定位元素自身；Flex/Grid 子项等。
5. `none` 不占位不可点；`visibility: hidden` 占位不可点；`opacity: 0` 占位仍挡点击（除非 `pointer-events: none`）。
6. 浮动脱离文档流，父级计算高度时忽略浮动子项；`flow-root`、clearfix、`overflow: hidden` 等触发 BFC 包含浮动。
7. 最近 `position` 非 `static` 的祖先，否则初始包含块；`fixed` 相对视口，`absolute` 相对定位祖先。
8. 处在较低层叠上下文的「玻璃板」里，无法越过更高上下文整块板子。
9. 父 `.card { position:relative; width:300px; margin:0 auto }`，子 `.badge { position:absolute; top:8px; right:8px }`。
10. Computed → Box Model 四边数字；双击可编辑看 live 效果。
11. 未设 `top`；祖先 `overflow: hidden`；父高度不足滚动；`display` 不合适等。
12. 遮罩需全视口 `fixed` 并盖住页面所有 fixed 元素，必须正确设置 z-index 与层叠上下文，否则弹窗被挡或点穿。
...
1. **盒模型 = 快递盒**：商品是内容，气泡膜是 padding，纸箱是 border，两箱间距是 margin；写 `width` 要想清是称净重还是连箱带膜（box-sizing）。
2. **定位 = 贴标签**：relative 在原位挪一步但占位还在；absolute 钉在父盒子角落（角标、弹窗内容）；fixed 钉在屏幕（返回顶部、遮罩）；sticky 滚动时粘在顶部。
3. **BFC + 层级**：独立打包台解决塌陷和 margin 穿透；z-index 像一摞玻璃板——先比哪块板在上，再比板里的便签；Vue 后台弹窗盖按钮就靠这个。

> 下一章：[05 — CSS 布局、Flex、Grid、响应式与动画](./05-CSS布局FlexGrid响应式与动画.md)。用 Flex/Grid 做页面主骨架，本章的定位负责「叠在上面」的组件。

---

*文档版本：扩充版 v2 | 建议学习时长：4~6 小时（含练习与 DevTools）*