...
MYSQL_DSN=root:root123@tcp(127.0.0.1:3307)/study?charset=utf8mb4&parseTime=True&loc=Local
...
| 配置项 | 类型 | 作用 |
|--------|------|------|
| `HTTP_ADDR` | 字符串 | Gin 监听地址，例如 `:8080` |
| `BASE_URL` | 字符串 | 创建短链时拼接 `short_url` 的前缀 |
| `MYSQL_DSN` | 字符串 | MySQL 连接串 |
| `REDIS_ADDR` | 字符串 | Redis 地址，例如 `127.0.0.1:6379` |
| `CACHE_TTL` | `time.Duration` | Redis 缓存有效期，例如 `1h`、`30m` |
| `CODE_LEN` | 正整数 | 随机短码长度 |
| `MAX_RETRIES` | 正整数 | 短码碰撞时的最大重试次数 |

`HTTP_ADDR` 和 `BASE_URL` 要一起改。比如改到 9090：
...
```text
config: config CODE_LEN must be a positive integer
```
...
```text
config: config REDIS_ADDR is required
```
...
1. 当前项目启动时从哪里读取配置？
2. `CACHE_TTL=1h` 为什么要转换成 `time.Duration`？
3. 为什么配置文件缺字段时要拒绝启动？
4. 改 HTTP 端口时，为什么还要一起改 `BASE_URL`？
